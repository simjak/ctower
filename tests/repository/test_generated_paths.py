...
_ATOMIC_WRITE_SCRIPT = """import sys
from pathlib import Path
from tools.checks._impl.generated import atomic_write_generated_text
atomic_write_generated_text(Path.cwd(), Path(sys.argv[1]), sys.argv[2])
"""


async def _execute_atomic_writer(
    checkout: Path,
    ...
    if process.returncode is None:
        raise RuntimeError("completed atomic-writer subprocess has no return code")
...
class AtomicGeneratedWriteTests(unittest.TestCase):
    root = Path(__file__).parents[2]

    def test_atomic_write_rejects_symlinked_generated_root(self) -> None:
        with (
            ...
            self._copy_atomic_module(root)
            result = self._run_atomic_writer(root, Path("generated/escape.txt"), "escape\n")

            self.assertNotEqual(result.returncode, 0, result)
            self.assertFalse((outside / "escape.txt").exists())

    def test_atomic_write_rejects_nested_parent_and_leaf_symlinks(self) -> None:
        with (
            tempfile.TemporaryDirectory() as root_name,
            tempfile.TemporaryDirectory() as outside_name,
        ):
            ...
            parent_result = self._run_atomic_writer(
                root, Path("generated/linked/escape.txt"), "parent escape\n"
            )

            outside_leaf = outside / "leaf.txt"
            outside_leaf.write_text("baseline\n", encoding="utf-8")
            (generated / "leaf.txt").symlink_to(outside_leaf)
            leaf_result = self._run_atomic_writer(root, Path("generated/leaf.txt"), "leaf escape\n")

            self.assertNotEqual(parent_result.returncode, 0, parent_result)
            self.assertNotEqual(leaf_result.returncode, 0, leaf_result)
            self.assertFalse((outside / "escape.txt").exists())
            self.assertEqual(outside_leaf.read_text(encoding="utf-8"), "baseline\n")

    def test_atomic_write_rejects_authored_and_lexical_output_escapes(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            self._copy_atomic_module(root)
            authored = root / "authored.txt"
            authored.write_text("baseline\n", encoding="utf-8")

            for relative in (
                Path("authored.txt"),
                Path("generated/../authored.txt"),
                Path("../outside.txt"),
            ):
                with self.subTest(relative=relative):
                    result = self._run_atomic_writer(root, relative, "escape\n")
                    self.assertNotEqual(result.returncode, 0, result)

            self.assertEqual(authored.read_text(encoding="utf-8"), "baseline\n")

    def test_atomic_write_replaces_regular_file_and_creates_safe_parents(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            self._copy_atomic_module(root)
            relative = Path("generated/nested/output.txt")

            first = self._run_atomic_writer(root, relative, "first\n")
            second = self._run_atomic_writer(root, relative, "second\n")
            ...
            self.assertEqual(tuple(output.parent.glob(".output.txt.*")), ())

    def test_atomic_write_rejects_non_directory_parent_and_non_regular_leaf(self) -> None:
        with tempfile.TemporaryDirectory() as root_name:
            root = Path(root_name)
            self._copy_atomic_module(root)
            generated = root / "generated"
            generated.write_text("not a directory\n", encoding="utf-8")
            parent_result = self._run_atomic_writer(root, Path("generated/output.txt"), "blocked\n")

            generated.unlink()
            generated.mkdir()
            (generated / "output.txt").mkdir()
            leaf_result = self._run_atomic_writer(root, Path("generated/output.txt"), "blocked\n")

            self.assertNotEqual(parent_result.returncode, 0, parent_result)
            self.assertNotEqual(leaf_result.returncode, 0, leaf_result)

    def _copy_atomic_module(self, checkout: Path) -> None:
        implementation = checkout / "tools/checks/_impl"
        implementation.mkdir(parents=True)
        for package in (
            checkout / "tools/__init__.py",
            checkout / "tools/checks/__init__.py",
    ...
    def _run_atomic_writer(
        self, checkout: Path, relative: Path, content: str
    ) -> subprocess.CompletedProcess[str]:
        environment = os.environ.copy()
        environment.pop("PYTHONPATH", None)
        return asyncio.run(_execute_atomic_writer(checkout, environment, relative, content))
