...
if TYPE_CHECKING:
    ...
    from repository._release_telemetry import GraphTelemetryProbe
else:
    from tests.repository._release_history import RawHistory, RawHistoryReader
    from tests.repository._release_telemetry import GraphTelemetryProbe
...
class ReleaseFoundationTests(unittest.TestCase):
    ...
    def _assert_pre1_feature_history(self) -> RawHistory:
        probe = GraphTelemetryProbe(self.root, _PRE1_FEATURE_COMMIT).capture()
        sys.stdout.write(f"{probe.telemetry}\n")
        sys.stdout.flush()
        graph_problems = [*probe.capture_problems, *probe.raw_history.problems]
        if graph_problems:
            self.fail(
