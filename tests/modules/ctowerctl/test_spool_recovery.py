...
from ctowerctl.spool._recovery import Session, recover_session, utc_text
...
def test_automatic_compaction_preserves_corrupt_disposition(
    ...
    spool.discard(corrupt.stored.sequence, "reviewed corrupt", artifact_digest=artifact.artifact_digest)
    assert spool.status().health == "healthy"
...
def test_bulk_transition_repair_bypasses_per_record_session_scans(
    ...
    with spool._tree.locked(spool._config.lock_timeout_seconds) as store:
        ...
        for index in range(_BULK_REPAIR_COMMANDS):
            command = _command(f"repair-{index}")
    ...
    monkeypatch.setattr(Session, "move", lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError))
