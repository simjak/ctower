...
def test_inbox_notify_warm_recovery_meets_fleet_bounds(
    ...
    executor.execute.return_value = ReplayResponse(status_code=403, problem_code="synthetic_barrier")
...
def test_compaction_survives_corruption_and_scan_bound_crash(
    ...
    artifact = next(item for item in spool.list_entries() if item.sequence == corrupt.stored.sequence)
    ...
    assert status.chain_status == "healthy" and status.accepted_count == 0
