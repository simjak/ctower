...
def test_every_ticket_operation_addresses_by_display_key_without_foreign_disclosure(
    ...
    _seed_ticket_row(tenant, project_key="manibo", display_key="MNB-1")
...
def test_a_key_two_projects_wear_after_a_reauthored_prefix_refuses_instead_of_guessing(
    tenant: TenantFixture,
) -> None:
    _apply_prefix_bundle(tenant)
    actor = Actor(tenant.commander_id, tenant.tenant_id, PrincipalKind.COMMANDER)
    record = PostgresRecord(tenant.database.runtime_dsn)
    command = _command(tenant, "ambiguous")
    captured = Work(record).create_ticket(
        actor, command, telemetry=telemetry_for(actor, command.client_command_id)
    )
    assert isinstance(captured, TicketCommandResult)
    _seed_ticket_row(tenant, project_key="manibo", display_key="CTW-1")

    ambiguous = record.tickets.resolve_display_key(
        actor, "CTW-1", telemetry=telemetry_for(actor, uuid4())
    )
    addressed = record.tickets.get(
        actor, captured.ticket.ticket_id, "ctower", telemetry=telemetry_for(actor, uuid4())
    )

    assert isinstance(ambiguous, RecordProblem)
    assert (ambiguous.code, ambiguous.status) == ("tenant-scope-denied", 404)
    assert isinstance(addressed, Ticket)
    assert addressed.display_key == "CTW-1"


def _seed_ticket_row(tenant: TenantFixture, *, project_key: str, display_key: str) -> None:
    """Insert one ticket in a project this tenant's commander holds no seat for."""
