...
# carries no editable install, so ctower_api, ctowerctl, the generated names, and the host
# kernel resolve only through these paths (pytest's pythonpath ini augments just the
# in-process interpreter). The trailing kernel root is the host fallback only; ref-bound
# children always get the target ref's own kernel prepended ahead of it.
