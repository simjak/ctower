...
def _append_expired_work_item_alarms(
    connection: psycopg.Connection[dict[str, object]],
    tenant_id: UUID,
    actor_principal_id: UUID,
    now: datetime,
    ...
    rows = connection.execute(
        """
        SELECT work_item_id, routine_ref, scheduled_for, window_ends_at,
            escalation_seat, gate_evidence
    ...
    for row in rows:
        kind: RoutineAlarmKind | None = None
        if not _valid_gate_evidence(row["gate_evidence"]):
            kind = RoutineAlarmKind.DEGRADED_READ
        elif cast(datetime, row["window_ends_at"]) <= now:
            kind = RoutineAlarmKind.MISSED_WINDOW
        if kind is None:
            continue
        existing = connection.execute(
            """
            SELECT 1 FROM routine_work_item_alarms
            WHERE tenant_id = %s AND routine_ref = %s
              AND scheduled_for = %s AND kind = %s
            """,
            (tenant_id, row["routine_ref"], row["scheduled_for"], kind.value),
        ).fetchone()
        if existing is not None:
            continue
        alarm = _append_work_item_alarm(
            connection,
            tenant_id,
            actor_principal_id,
            cast(UUID, row["work_item_id"]),
            str(row["routine_ref"]),
...
def _append_work_item_alarm(
    ...
    alarm_id, command_id, event_id, outbox_id = _alarm_ids(
        tenant_id, work_item_id, scheduled_for, kind
    )
    ...
    _commit_work_item_fact(
        ...
        request_payload,
        now,
        "runtime.routine-work-item-alarms",
    )
...
def _alarm_ids(
    ...
    return (
        ...
        _stable_uuid7(
            scheduled_for,
            b"routine-work-item-alarm-command",
            tenant_id.bytes,
            alarm_id.bytes,
        ),
        _stable_uuid7(
            scheduled_for,
            b"routine-work-item-alarm-event",
            tenant_id.bytes,
            alarm_id.bytes,
        ),
        _stable_uuid7(
            scheduled_for,
            b"routine-work-item-alarm-outbox",
            tenant_id.bytes,
            alarm_id.bytes,
        ),
    )
...
def _insert_alarm(
    ...
    connection.execute(
        ...
        INSERT INTO routine_work_item_alarms (
            alarm_id, tenant_id, routine_ref, scheduled_for, work_item_id,
            escalation_seat, kind, recorded_at, event_id
