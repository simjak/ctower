...
def _registration(binding: GitLabSyncBinding) -> ConnectorRegistration:
    return ConnectorRegistration(
        registration_key=binding.integration_key,
        ...
        initial_cursor=_cursor_token(GitLabCursor(binding.import_updated_after, 1, 0)),
        page_size=binding.page_size,
        poll_interval=binding.poll_interval,
        label_map=tuple(
            ConnectorLabelMapping(source=source, target=target)
            for source, target in binding.label_map
        ),
    )


def _cursor_token(cursor: GitLabCursor) -> ConnectorCursorToken:
...
def _gitlab_cursor(token: ConnectorCursorToken, project_event_cursor: int) -> GitLabCursor:
    try:
        value = json.loads(token.value)
    except json.JSONDecodeError as error:
        raise GitLabSyncError("GitLab cursor token is invalid") from error
    if not isinstance(value, dict) or value.get("schema") != "ctower.gitlab-cursor/v1":
        raise GitLabSyncError("GitLab cursor token is invalid")
    try:
        updated_after = datetime.fromisoformat(str(value["updated_after"]))
        page = value["page"]
    except (KeyError, TypeError, ValueError) as error:
        raise GitLabSyncError("GitLab cursor token is invalid") from error
    if isinstance(page, bool) or not isinstance(page, int):
        raise GitLabSyncError("GitLab cursor token is invalid")
    return GitLabCursor(updated_after, page, project_event_cursor)


def _external_issue(issue: GitLabIssue) -> ExternalIssue:
    return ExternalIssue(
        connector_kind="gitlab-issue",
        external_ref=issue.source_ref,
        title=issue.title,
        description=issue.body,
        source_labels=issue.labels,
        reporter_reference=f"@{issue.reporter.username}",
        reporter_display_name=issue.reporter.name,
        external_state=cast(Literal["opened", "closed"], issue.state),
        display_url=issue.web_url,
        updated_at=issue.updated_at,
    )


def _gitlab_issue(issue: ExternalIssue) -> GitLabIssue:
    project_id, issue_iid = _external_identity(issue.external_ref)
    reporter_reference = issue.reporter_reference
    if not reporter_reference.startswith("@"):
        raise GitLabSyncError("GitLab reporter reference is invalid")
    return GitLabIssue(
        project_id=project_id,
        iid=issue_iid,
        title=issue.title,
        body=issue.description,
        labels=issue.source_labels,
        reporter=GitLabReporter(reporter_reference[1:], issue.reporter_display_name),
        state=issue.external_state,
        web_url=issue.display_url,
        updated_at=issue.updated_at,
    )


def _connector_link(link: GitLabIssueLink) -> ConnectorLink:
    return ConnectorLink(
        tenant_id=link.tenant_id,
        registration_key=link.integration_key,
        revision_digest=link.revision_digest,
        connector_kind="gitlab-issue",
        external_ref=link.source_ref,
        ticket_id=link.ticket_id,
        thread_id=link.thread_id,
        display_url=link.web_url,
    )


def _gitlab_link(link: ConnectorLink) -> GitLabIssueLink:
    project_id, issue_iid = _external_identity(link.external_ref)
    return GitLabIssueLink(
        tenant_id=link.tenant_id,
        integration_key=link.registration_key,
        revision_digest=link.revision_digest,
        project_id=project_id,
        issue_iid=issue_iid,
        ticket_id=link.ticket_id,
        thread_id=link.thread_id,
        web_url=link.display_url,
    )


def _connector_receipt(receipt: GitLabCloseReceipt) -> ConnectorReceipt:
    if not receipt.issue_closed:
        raise GitLabSyncError("GitLab receipt did not confirm the closed state")
    return ConnectorReceipt(
        command_id=receipt.delivery_id,
        comment_created=receipt.comment_created,
    )


def _gitlab_receipt(receipt: ConnectorReceipt) -> GitLabCloseReceipt:
    return GitLabCloseReceipt(
        delivery_id=receipt.command_id,
        comment_created=receipt.comment_created,
        issue_closed=receipt.issue_closed,
    )


def _close_command(command: CloseExternalIssue) -> GitLabCloseCommand:
    return GitLabCloseCommand(command.command_id, command.comment)


def _connector_claim(claim: GitLabSyncClaim) -> ConnectorClaim:
    return ConnectorClaim(
        cursor=_cursor_token(claim.cursor),
        project_event_cursor=claim.cursor.project_event_cursor,
...
def _gitlab_claim(claim: ConnectorClaim) -> GitLabSyncClaim:
    return GitLabSyncClaim(
        cursor=_gitlab_cursor(claim.cursor, claim.project_event_cursor),
        owner_id=claim.owner_id,
        fence=claim.fence,
        expires_at=claim.expires_at,
    )
...
def _external_identity(external_ref: str) -> tuple[int, int]:
    ...
    if (
        ...
        raise GitLabSyncError("GitLab external reference is invalid")
