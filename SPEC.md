...
Retiring a fixed fleet-beat Routine is a protected operator/Commander command that appends one immutable,
tenant-scoped retirement fact naming the active revision and removes only its mutable scheduling trigger in
the same transaction. The accepted retirement is the scheduling cutover: later registration cannot
reactivate that reference, while every prior revision, occurrence, effect, event, and command result remains
history. Unknown and foreign-tenant references share one non-enumerating typed refusal. Boot verification
continues to require every authored pack unless that exact reference is retired for every current tenant;
an unaccounted missing pack still fails closed.
...
| <a id="ac-ops-07"></a>AC-OPS-07 | Agent/profile/runner/routine revisions are immutable, attributable, and reflected in Fleet; deleting/disabling config preserves historical runs/costs. Retiring a fixed fleet-beat Routine appends a tenant-scoped fact, removes only its active trigger, cannot be undone by later registration, and permits an absent authored pack at boot only when every current tenant has retired that exact reference. | Revision/tombstone tests, live retire/reboot transcript, missing-active-pack negative, and Fleet capture |
...
| <a id="ac-ops-13"></a>AC-OPS-13 | A revision-pinned Routine with cron/timezone materializes each logical due occurrence once across duplicate scans and scheduler/outbox restart; UTC and local civil time, DST gap/repeat result, concurrency/catch-up decision, component pins, and every queued/coalesced/skipped/refused outcome remain inspectable. Long downtime obeys its explicit cap and cannot silently flood jobs. Once a fleet-beat retirement commits, scans and later registration attempts create no further occurrence for that tenant/reference; unknown and foreign retirement targets refuse by the same stable typed name. | Fake-clock timezone/DST matrix, duplicate/crash/replay test, revision-edit/retirement isolation, typed-refusal negatives, occurrence ledger and Fleet capture |
...
| CT-I1-019 | Replace the five existing Mission Control fleet-beat crontab schedules with fixed `ctower.beat.*@1` Routine revisions. Pin each exact operator-authorized minute/hour set and schedule timezone plus the full canonical prompt in immutable revision custody; the digest and sprint schedules use literal Europe/Vilnius hours so daylight-saving changes preserve operator intent. Registering a corrected digest retires only that tenant's prior active trigger while preserving every old revision, occurrence, and effect. Transactionally emit one full-prompt beat effect per queued occurrence; expose operator-only registered-routine/effect reads; and deliver through a sibling Mission Control consumer whose append-only ledger permits one DIRECTOR injection and zero replay injections. Expose protected operator/Commander retirement as a new immutable tenant-scoped fact that transactionally removes only the active trigger, prevents later registration or scans from producing occurrences, returns one non-enumerating typed refusal for unknown/foreign targets, and authorizes boot without the retired pack only when every current tenant has retired the reference. Mission Control removes any hard-coded retired-beat suppression only in its separately owned lane after Tower state is live-proven. Add no beat consumption claim until a separately authorized authenticated lane-binding ceremony exists. | CT-I1-006; CT-I1-018; D52; D53; D57; issues #433 and #457 operator rulings | Engineer + independent Review; Director confirms live receipt | Authored Runtime/HTTP contracts; kernel Runtime; five Routine packs; generated clients; `ctower-api`; `ctowerctl`; Mission Control consumer/provisioning | AC-OPS-07; AC-OPS-13; RED-first schedule/contract/PostgreSQL/API/CLI/consumer suites including summer/winter timezone, corrected-revision activation, live retirement suppression, typed unknown/foreign refusal, and absent-retired-pack reboot proof; active registered routine list; full-prompt health occurrence/effect/consumer/DIRECTOR transcript; replay with zero reinjection; named missing-session/list/ledger failures; retired-crontab proof; same-candidate docs | `uv run pytest tests/contracts/runtime/test_beat_dispatch_contract.py tests/modules/runtime/test_schedule.py tests/acceptance/increment-1/test_beat_dispatch_effect.py -q && just check && just verify` |
