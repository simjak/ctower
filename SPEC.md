...
| <a id="ac-cap-06"></a>AC-CAP-06 | Each fleet-layer defect class demonstrated on 2026-08-11 carries a regression that fails before its fix and passes after: a seat whose assigned model is capped is NOT reported as healthy — liveness includes can-it-run-its-assigned-model, so a capped-model seat is typed unusable rather than alive; an account whose credential is present but empty fails a completeness assertion by name rather than surfacing an exception; and an injection aimed at a seat that cannot answer fails a delivery assertion rather than reporting a delivered receipt. No fix for these classes ships without its failing-then-passing test. | RED-first evidence per class at the pre-fix and post-fix heads: capped-model-seat liveness vector, empty-token completeness vector, and unanswerable-seat delivery vector |
