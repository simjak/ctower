...
the narrow standing GitLab and GitHub Issue co-source integrations, additive mission-control notification transport,
...
The complete base contract in [`docs/specs/operator-requests.md`](docs/specs/operator-requests.md) is
accepted by D46 and clarified by D47. Its seven `OR-01..OR-07` invariants are canonical as
`INV-81..INV-87` below. The tested development candidate implements those stable-work seams in disposable
verification tenants; it has no portfolio authority until the CT-I1-015 epoch passes.
...
Every new capture compares its text with every accepted open, non-rejected, non-duplicate Request in the
same Project. Comparison is an in-process deterministic `ctower.local-hashed-subword/v1` embedding: NFKC
casefolding, Unicode word and adjacent-word features, character 3/4/5-grams, fixed 2,048-dimension SHA-256
feature hashing, and cosine similarity at the fixed `0.72` threshold. It downloads no model, invokes no
process or provider, reads no secret, and has no network/egress path. The highest-scoring candidate wins,
with the oldest `R` number breaking a tie. The new Request is always captured. A match appends one immutable
link readable from both Requests without changing the candidate's version or facts and returns exactly
`captured as R-new, resembles R-old (status), linked — say same to merge.` Exact replay returns that same
acknowledgement and link; a nonmatch returns `captured as R-new.`

Resemblance never merges. Only the operator's authenticated `request same` against the new Request's
accepted persisted link, or the existing commander-authorized `DUPLICATE` triage command, may append a
merge. The immutable merge fact retains both Request identities, `R` numbers, verbatim UTF-8 contents and
digests, original capture timestamps, the exact merge wording, actor, command, trigger, and merge time.
Every other command leaves merge provenance absent. The resemblance link remains inspectable after merge;
the source Request becomes `DUPLICATE` while the canonical Request is untouched.
...
81. <a id="inv-81"></a>**INV-81 — Request capture is one burst-safe durable act.** An authorized project, nonempty text, and idempotency key are sufficient. The server validates authority/prohibited data, reserves replay, performs only the bounded local resemblance comparison of [INV-91](#inv-91), allocates UUIDv7 and the next tenant-wide `R` number, and atomically commits the inbound event, Request facts, optional resemblance link, native alias, command result, audit event, and outbox before any downstream or external analysis. Exact replay returns the original result; changed-digest reuse refuses. Acceptance waits for the required off-host acknowledgement; `durability_pending` and transport ambiguity never render as accepted.
...
90. <a id="inv-90"></a>**INV-90 — The morning digest is one truthful disposable projection.** One operator-only artifact key names each Europe/Vilnius civil date. The artifact renders four ordered sections from accepted Request and Ruling reads: one terse note per still-open persisted resemblance pair, open Requests with their complete record-derived decision briefs, the prior civil day's Rulings with executions derived only from typed Request relations, and related Ticket timeline links plus current proof counts. A merged, rejected, done, missing, or pending endpoint never renders as an open pair. Every source and section carries `complete|partial|unknown`, its visible count, a nullable total, and exact unreached scopes, while the artifact carries both source watermarks; a failed source, unresolved relation, or unread proof count is explicit `UNKNOWN` and never a measured zero or inferred fact. An authoritative absent relation is an empty execution set. The fold and renderer add no store, scheduler, identity, authority, or egress. Delivery uses the existing Mission Control notification rail after its durable rail-1 append and mirrors through rail 2 only where the authenticated identity resolves. Slack/Hermes remains absent, and only the director may retire the interim schedule after a verified switch.
91. <a id="inv-91"></a>**INV-91 — Request resemblance speaks and merge authority is explicit.** Every capture compares locally against every accepted open same-Project Request using the one fixed deterministic hashed-subword embedding and threshold named in the Request contract. The new Request is never dropped or automatically merged. A qualifying best match atomically appends one immutable link visible from both records and the capture acknowledgement names both `R` references, the candidate's accepted state, and `say same to merge`; the candidate's facts and version stay unchanged. Only authenticated operator `same` against that accepted link or commander `DUPLICATE` triage may append merge provenance. That fact preserves both original verbatim contents, digests, capture timestamps, identities, trigger wording, actor, command, and merge time; every other path leaves both Requests independent. The embedding path invokes no external process, model provider, network, secret, or egress.
...
There is no tailer, dual write, corpus importer, general fuzzy-dedupe service, or automatic backfill. D57's
deterministic local same-Project Request comparison is the sole bounded resemblance path; it always captures
and never merges automatically. Mission Control and the applicable GitHub/GitLab records remain co-sources
throughout shadow operation. The ordinary command
...
| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-req-01"></a>AC-REQ-01 | One hundred parallel distinct native captures plus exact-key replays produce one off-host-accepted Request per distinct key, unique permanent tenant-wide `R` numbers, byte-equal replay outcomes, zero implicit Tickets, and no provider/projection/external analysis on the capture critical path beyond the bounded local comparison of AC-REQ-12. Changed-digest key reuse refuses, and injected commit/ACK/timeout/disconnect/restart faults never expose ambiguity as accepted or lose/reuse an accepted identity. | `tests/acceptance/increment-1/test_requests.py::test_capture_is_atomic_burst_safe_and_idempotent`, command/outbox/allocator queries, and fault trace |
...
| <a id="ac-req-09"></a>AC-REQ-09 | A seeded near-duplicate capture always creates a new Request, returns the exact speaking acknowledgement naming the older reference and accepted state, persists one immutable link readable from both Requests, leaves the original version and facts unchanged, and replays byte-identically. A seeded unrelated capture has no link and returns the plain captured acknowledgement. | `tests/acceptance/increment-1/test_requests.py::test_resembling_capture_speaks_links_and_operator_same_preserves_provenance`, `tests/acceptance/increment-1/test_requests.py::test_unrelated_capture_stays_unlinked_and_same_refuses_without_a_link`, real PostgreSQL/API transcript, and link/version queries |
| <a id="ac-req-10"></a>AC-REQ-10 | Authenticated operator `request same` may merge only the source of its accepted persisted resemblance link, and commander `DUPLICATE` triage remains the only other merge path. Both paths retain both Request identities, verbatim contents/digests, original capture timestamps, exact trigger wording, actor, command, and merge time; every other command appends no merge fact. | `tests/acceptance/increment-1/test_requests.py::test_resembling_capture_speaks_links_and_operator_same_preserves_provenance`, `tests/acceptance/increment-1/test_requests.py::test_request_triage_owner_relation_and_version_refusals_are_fail_closed`, protected HTTP/CLI contract, and immutable provenance query |
| <a id="ac-req-11"></a>AC-REQ-11 | The morning digest renders exactly one terse note for each accepted still-open persisted resemblance pair and names the new Request on which to say same. Done, rejected, merged, missing, pending, partial, and unknown endpoints never become a fabricated open pair or zero. | `tests/modules/projections/test_morning_digest.py::test_digest_names_each_open_resemblance_once_and_hides_merged_pairs`, `tests/acceptance/increment-1/test_morning_digest.py::test_real_morning_digest_composes_record_derived_briefs_and_links`, deterministic JSON/CLI transcript |
| <a id="ac-req-12"></a>AC-REQ-12 | Request resemblance uses only the documented deterministic in-process `ctower.local-hashed-subword/v1` embedding and fixed threshold over accepted same-Project open Requests. Dependency, process, secret, provider, socket, DNS, and egress inventories show no new external compute path; the exact source and candidate embedding digests are persisted with the link. | Request similarity unit/acceptance vectors, migration link query, dependency and egress diff inventory, and `just check`/`just verify` |
...
| CT-I1-018 | Deliver the native morning digest as one landable read-model candidate: one strict Europe/Vilnius artifact over accepted Request and Ruling sources; one terse note per open resemblance pair; complete record-derived open decision briefs; prior-day Rulings and typed Request executions; Ticket proof links; exact partial/unknown epistemics; generated HTTP/client and deterministic protected-CLI rendering; and one real existing-rail notification delivery. It adds no store, scheduler, identity, adapter, Slack/Hermes path, or director-cron mutation. | CT-I1-017; D51, D57; issues #402, #431 | Engineer + Engineering Manager + independent Review | Authored HTTP contract; kernel `projections/`; generated clients; `ctower-api`; `ctowerctl`; concept/reference docs | AC-DIG-01..03 and AC-REQ-11; RED-first projection matrix; real PostgreSQL/API/CLI transcript; one delivery ID and rail outcome; source/identity/egress/scheduler inventory; same-candidate docs | `uv run pytest tests/contracts/digest tests/modules/projections/test_morning_digest.py tests/modules/projections/test_morning_digest_routes.py tests/modules/ctowerctl/test_morning_digest.py tests/acceptance/increment-1/test_morning_digest.py -q` |
