...
| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-fleet-01"></a>AC-FLEET-01 | The tower persists a versioned seat-and-crew registry naming, per seat: project, substrate identity, and exactly one respawn owner. Registry and respawn-policy revisions are immutable Catalog components. A second claimed owner for a seat is refused by name, and no registry row stores a session-resume identity — resume identity is discovered at respawn time from seat transcript state. | Registry/policy contract tests; duplicate-owner and stored-resume refusal vectors; revision-custody proof |
| <a id="ac-fleet-02"></a>AC-FLEET-02 | Per-seat liveness state (alive/working/idle/dead/capped) derives only from typed probe and event ingest with last-evidence timestamps. Pane-rendered text is not a source for liveness or cap detection. Stale or absent evidence yields typed `unknown` with its age, never inferred health. | Liveness projection truth table; stale-evidence and absent-probe fixtures; the pane-render misclassification regression fixture |
...
| <a id="ac-fleet-05"></a>AC-FLEET-05 | The kill-drill (substrate server killed; registered owners rebuild every seat) and the reboot-drill (host boots; boot-enabled tower services come up; fleet bootstraps; all registered seats respawn including commander with discovered resume identity; routines fire) run as tower-run workflow tickets with frozen criteria, proven on an isolated substrate with synthetic registry before any live drill, and the event ledger records each full recovery as rows. A drill success flag alone cannot resolve. | Isolated-substrate kill and reboot drill transcripts; ledger recovery rows; boot-enablement inventory; discovered-resume proof; flag-only resolution refusal |
