...
89. <a id="inv-89"></a>**INV-89 — The morning digest is one truthful disposable projection.** One operator-only artifact key names each Europe/Vilnius civil date. The artifact renders three ordered sections from accepted Request and Ruling reads: open `operator-decision-required` Requests in the complete decision-brief shape, the prior civil day's Rulings with recorded Request executions, and related Ticket timeline links plus current proof counts. Every source and section carries `complete|partial|unknown`, its visible count, a nullable total, exact unreached scopes, and its source watermark; a failed or not-yet-recorded brief, execution link, source, or proof count is explicit `UNKNOWN` and never a measured zero or inferred fact. The fold and renderer add no store, scheduler, identity, authority, or egress. Delivery uses the existing Mission Control notification rail after its durable rail-1 append and mirrors through rail 2 only where the authenticated identity resolves. Slack/Hermes remains absent, and only the director may retire the interim schedule after a verified switch.
...
| CT-I1-017 | Deliver the native morning digest as one landable read-model candidate: one strict Europe/Vilnius artifact over accepted Request and Ruling sources; brief-shaped open decisions; prior-day Rulings and recorded executions; Ticket proof links; exact partial/unknown epistemics; generated HTTP/client and deterministic protected-CLI rendering; and one real existing-rail notification delivery. It adds no store, scheduler, identity, adapter, Slack/Hermes path, or director-cron mutation. | CT-I1-016; D50; issue #402 | Engineer + Engineering Manager + independent Review | Authored HTTP contract; kernel `projections/`; generated clients; `ctower-api`; `ctowerctl`; concept/reference docs | AC-DIG-01..03; RED-first projection matrix; real PostgreSQL/API/CLI transcript; one delivery ID and rail outcome; source/identity/egress/scheduler inventory; same-candidate docs | `uv run pytest tests/contracts/digest tests/modules/projections/test_morning_digest.py tests/modules/ctowerctl/test_morning_digest.py tests/acceptance/increment-1/test_morning_digest.py -q` |
