...
Ruling names its `request_id`. The Ruling relation is constrained to the same tenant and Project and appears
as `request_id` plus `request_reference` on Ruling reads; the Request brief then becomes `answered`, cites
the current Ruling, and no longer counts the decision blocker in derived state. A pending Ruling changes no
accepted read. A correction inherits the same Request relation, so it cannot detach the answer. The digest
surface consumes this read contract only through its separately activated work item and adds no behavior
here.
...
89. <a id="inv-89"></a>**INV-89 — Operator decision briefs are Request-record projections closed by Rulings.** An accepted Request whose latest accepted `operator-decision-required` blocker is active renders one complete brief from accepted Request facts only: a plain explanation, exact origin quote, three outcome choices with bounded completeness scores, one record-derived recommendation and reason, and one safe default and reason. No caller may supply or alter those fields, and a Request without that decision marker renders no brief. An answer is one accepted immutable Ruling linked to the same tenant, Project, and Request; pending Rulings do not answer, accepted reads expose the relation from both records, and every Ruling successor inherits it.
...
| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-brief-01"></a>AC-BRIEF-01 | A Request with the current accepted `operator-decision-required` marker exposes one complete brief: plain ELI, exact origin quote, exactly three outcome choices with bounded completeness scores, one recommendation with reason, one safe default with reason, open/answered status, and a ready-to-send rendering. | `tests/acceptance/increment-1/test_decision_briefs.py::test_decision_request_renders_complete_record_derived_brief_and_ignores_extras`, rendered output capture |
| <a id="ac-brief-02"></a>AC-BRIEF-02 | An accepted linked Ruling is visible from the Request and Ruling, removes only the decision blocker from derived state, and remains linked through successors. A pending linked Ruling does not answer, and a Request without the decision marker exposes no brief. | `tests/acceptance/increment-1/test_decision_briefs.py::test_linked_ruling_is_visible_from_request_and_ruling_and_resolves_decision`, `test_non_decision_request_renders_no_brief`, relation and event-link queries |
...
| <a id="ac-brief-04"></a>AC-BRIEF-04 | The exact candidate proves the new migration and accepted-only state transition on real PostgreSQL, keeps generated clients and authored contracts in sync, documents the Request and Ruling surfaces, passes every activated suite and full release verification, and receives independent maximum-effort judgment review. | Real PostgreSQL trace, generated-drift gate, `just verify`, same-head documentation check, independent review verdict |
