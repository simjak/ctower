...
stage or a non-engineering Workflow declares a different graph. In `engineering.software-factory@1` that
set resolves to `risk-derived-review`, `documentation`, and `release-preflight` — the review fact and the
documentation fact over preflight, which is what makes documentation unskippable at the boundary that
actually lands work rather than only inside the evaluator.
