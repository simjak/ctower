...
not reference or derive truth from a later one. The fifth-layer source is one strict
`workflows/<workflow-name>.yaml` resource inside a CompanyBundle. S7's visual stage editor and S8's YAML
view are two projections of that same resource. Neither owns a second DTO, parser, save path, or runtime
state. A save is a Git change followed by the ordinary authenticated CompanyBundle
...
`transitions` arrays. Every key-like scalar uses `^[a-z][a-z0-9._-]{0,127}$`; assertion strings are
nonempty and at most 512 Unicode scalar values; `widths` is a unique nonempty list of integers from 240
through 7680. All arrays preserve authored order and reject exact duplicate items.

| Source field | Strict v1 meaning |
|---|---|
| `apiVersion` / `kind` | Exact literals `ctower/v1` and `Workflow`. |
| `metadata.name` | Stable Workflow key. It becomes the Catalog component key; it is not a mutable display name. |
| `metadata.company` | Company key from layer 1. The containing bundle company must match it exactly. |
...
| `spec.stages[]` | Ordered authored stage entries. Each has unique `name`, one `owner`, and a nonempty ordered `evidence` list. Order is display order only; transitions own graph order. |
...
1. **Source-schema valid** means the strict YAML shape and closed vocabulary parse. The approved S8 mockup
   ...
   `build -> review` refusal, and `lastmachines`/`bh-loop` overlay spelling.
...
3. **Publishable/activatable** means the normalized payload also satisfies the full
   ...
   conformance, digest, authorization, and activation contract already specified here. Only this normalized
   immutable Catalog revision is executable, and every run pins it by revision and digest.
...
the satisfying Evidence verifier assignment interval. `stage.owner`, evidence key, project overlay,
principal name, custodian, and silence are all forbidden seat-inference inputs, and assigned and signing
seats remain separately visible when they differ. Workflow Definition files contain no ticket, run,
...
company identity, goals, projects, ticket schemas, workflows, execution/gate/evidence policies, profiles, personas, skills,
...
| <a id="ac-wf-28"></a>AC-WF-28 | One strict `ctower.workflow-definition/v1` source schema accepts the approved S8 Workflow YAML as source-schema valid and rejects duplicate/unknown YAML structure; semantic plan resolves the five configuration layers into one normalized `ctower.workflow/v1` payload or reports every unresolved/nonnarrowing reference without mutation. S7 visual -> canonical YAML -> visual and S8 YAML -> visual -> canonical YAML preserve the same normalized digest. Project overlays add only required slot contracts in deterministic base-then-overlay order; removal/relaxation, owner/graph/policy/seat mutation, unknown project/stage/evidence keys, divergent outgoing-edge `requires` sets, and base/overlay collisions each prevent publication. Source validation alone never marks a revision active. | Exact approved-mock source-positive fixture; duplicate/alias/tag/unknown-field negatives; five-layer resolution manifest; monotonic-overlay mutation matrix for two projects plus no-overlay base; visual/YAML semantic round-trip digest; unresolved compact-mock plan diagnostics; prior-pointer-unchanged activation proof; assigned/signing/unassigned D28 anti-inference fixtures |
