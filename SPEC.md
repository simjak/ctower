...
| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-cap-01"></a>AC-CAP-01 | Capacity is measured on the exact credential artifact the seat authenticates with — never on a copy, marker, or sibling record standing in for it — and a measurement whose target cannot be proven to be that artifact is typed `unproven`, never `alive` and never `capped`. Two independent probe generations read this system wrong in OPPOSITE directions (one inferring alive from a usage endpoint, one calling real models against pooled copies the seats do not use), so a capacity reading carries the identity of the artifact it measured and any consumer — automatic model restore and every drill gate included — refuses to act on `unproven`. Capacity state is otherwise keyed by the (harness, account, model) triple with its own state and reset time. No read, policy, or event expresses exhaustion at account granularity alone. A model capped on one account leaves every other model and account unchanged, and the declared account inventory per harness is versioned data, not code. | Triple-keyed projection truth table; the capped-model-alive-sibling-model fixture; measured-artifact identity assertions; both false-reading regressions (false-alive from an inferred endpoint, false-capped from an unused copy) as named fixtures; unproven-state consumer-refusal vectors; inventory revision-custody proof |
