...
classifier, each authenticated omnibox submit defaults to `discussion` and may explicitly select
...
permitted in I1. The target classification flow above remains a later deepening, not fabricated I1 behavior.
...
- **Browser:** I1 uses a React 19 client-only SPA, React Router 7 Declarative Mode, and Vite static output.
...
For browser use, Access owns this session boundary:
...
| <a id="ac-sec-12"></a>AC-SEC-12 | Browser login/session/logout, CSRF, expiry, revocation, and protected-command reauthentication derive tenant/principal server-side; missing/wrong/replayed origin, CSRF, session, credential, freshness, or scope performs zero mutation; no API bearer/session value enters browser JavaScript, DOM, URL, storage, trace, screenshot, log, or telemetry. | Access/HTTP fake-clock matrix, cross-tenant Playwright contexts, secret/redaction scan, CSO verdict |
...
6. Thin Home, Board, and contextual/direct-ID Ticket plus narrow real read-only Fleet and Analytics inside
the locked five-surface shell. Fleet renders authoritative I1 health contributors. Analytics renders only
the frozen operator-attention baseline/current sample with revision, cohort, digest/watermark, and
provisional/unknown state. I2 deepens both. Browser commands remain visibly unsent or durability-pending
until accepted.
...
| CT-I1-005 | Implement thin Home, Board, contextual/direct Ticket, and narrow real read-only Fleet and Analytics inside the five-surface shell, with trustworthy health, priority/owner/blocker controls, `activity_class` lane fold, proof, typed delivery, timeline, and visibly pending browser commands. | CT-I1-002..003, CT-L0-008..009 | Designer + UI QA; operator taste gate when material | `apps/ctower-web/src/surfaces/{home,board,ticket,fleet,analytics}/`; `routes.ts`; kernel `attention/`, `projections/` | Every-control UI QA, Board truth table, command reconnect, <10 s Home, and unknown screenshots | `uv run pytest tests/acceptance/increment-1/test_needs_you.py tests/acceptance/increment-1/test_board.py -q && pnpm run test:e2e` |
