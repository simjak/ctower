...
| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-cap-01"></a>AC-CAP-01 | Capacity state is keyed by the (harness, account, model) triple with its own state and reset time. No read, policy, or event expresses exhaustion at account granularity alone. A model capped on one account leaves every other model and account unchanged, and the declared account inventory per harness is versioned data, not code. | Triple-keyed projection truth table; the capped-model-alive-sibling-model fixture; inventory revision-custody proof |
