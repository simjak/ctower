...
| CT-I1-019 | Replace the five existing Mission Control fleet-beat crontab schedules with fixed `ctower.beat.*@1` Routine revisions. Pin each exact UTC minute/hour set and full canonical prompt in immutable revision custody; transactionally emit one full-prompt beat effect per queued occurrence; expose operator-only registered-routine/effect reads; and deliver through a sibling Mission Control consumer whose append-only ledger permits one DIRECTOR injection and zero replay injections. Add no beat consumption claim until a separately authorized authenticated lane-binding ceremony exists. | CT-I1-006; CT-I1-018; D52; issue #433 operator ruling | Engineer + independent Review; Director confirms live receipt | Authored Runtime/HTTP contracts; kernel Runtime; five Routine packs; generated clients; `ctower-api`; `ctowerctl`; Mission Control consumer/provisioning | AC-OPS-13; RED-first schedule/contract/PostgreSQL/API/CLI/consumer suites; five registered routine list; full-prompt health occurrence/effect/consumer/DIRECTOR transcript; replay with zero reinjection; named missing-session/list/ledger failures; UTC crontab proof; same-candidate docs | `uv run pytest tests/contracts/runtime/test_beat_dispatch_contract.py tests/modules/runtime/test_schedule.py tests/acceptance/increment-1/test_beat_dispatch_effect.py -q && just check && just verify` |
