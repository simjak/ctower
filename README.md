...
| First-class Requests that always capture the operator's words, speak when an accepted open same-Project Request resembles them, preserve the link, and merge only after explicit operator `same` or commander duplicate triage | General semantic search, model-service embeddings, or automatic duplicate merging |
