...
| A board with six lanes, the full history of every ticket, a per-project delivery view, and a generated-client typed accepted-event feed scoped and cursor-bound to one project, all rebuilt from recorded facts | Fleets of agents working in parallel, with permissions that expire and automatic resume after a crash |
