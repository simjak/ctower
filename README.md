...
| Five fixed fleet-beat schedules with immutable full-prompt effects, operator-only reads, and an operator-only terminal retirement command that cannot be undone by a worker restart or an older application binary | A general routine editor, general routine retirement, caller-selected beat targets, or a beat consumption/lane-binding ceremony |
