...
## 2026-08-10 — scheduled dream `019fe965-f100-76be-b6cc-388404e2df47`
...
What happened → The repaired consumer processed the pending effects, produced the expected digest, and
passed a source-content cross-check. The collision-safe patch then reached Mission Control main with tests
for safely reaping landed output, preserving unpushed work, resuming pushed work, suffixing unsafe name
collisions, and retaining named failures. Durable rule → A successful retry is recovery evidence, not
...
What happened → A post-replay probe met an unbound dream lane and recorded the exact unbound disposition;
no writer ran and the pending work remained recoverable for a later authorized binding. Durable rule → A
missing one-way trust binding must produce an attributable, replayable refusal before execution, never a
...
What happened → The unbound-lane probe labeled its notification as the director even though a different
process emitted it; the record caught the mismatch and ordered a tooling cure. Durable rule → An operational
message derives its addressable sender from the authenticated or substrate-observed emitter; free-form
...
- Request acceptance remains **UNENCODED in running behavior**: #400 is open with a live build lane and no
  completion candidate at the cutoff. Owners: ctower Commander and Engineer; due by
  `2026-08-10 12:00 Europe/Berlin`.
- Operational sender attribution remains **UNENCODED** as described above. Owners: dream consumer Engineer
  and ctower Commander; due by `2026-08-10 12:00 Europe/Berlin`.
- Executable-remedy enforcement remains **UNENCODED**: #357 and #358 are still open. Owner: ctower
  Commander; due to land or re-scope by `2026-08-10 12:00 Europe/Berlin`.

The earlier recurring-output gap is no longer in this register: the first lesson in this block names its
landed prevention and replay evidence. The operator-only dream binding remains an open handoff, not an
unencoded engineering claim.

## 2026-08-11 — scheduled dream `019fee8c-4d00-77d8-b729-e6f7821eecba`

Record window: after the prior dream output at `2026-08-10T05:20:01Z` through the scheduled
`2026-08-11T02:00:00Z` cutoff, project `ctower` only.

### A cross-record invariant belongs below every writer

What happened → The first Request-authority candidate derived one Ticket's holder by comparing version
counters owned by different Requests. Independent review reproduced two active claimants, and also showed
that an application-only repair would leave direct SQL as a bypass. Durable rule → An invariant spanning
aggregates needs one database-enforced chokepoint and a generated-history test that exercises both the normal
command and the lowest available bypass; counters local to different aggregates are never a global clock.
Encoding → [PR #412](https://github.com/simjak/ctower/pull/412), migration
`0059_request_authority.sql`,
[`test_request_relation_exclusivity.py`](tests/acceptance/increment-1/test_request_relation_exclusivity.py),
and Mission Control
`coordination/2026-08-10_1136--review-412-fable-r2--b1-b4-verify.status.md` preserve the
database trigger, version-churn matrix, direct-SQL refusal, and independent GREEN recheck.

### A human-facing recurrence needs authorization, not only idempotent work

What happened → Replaying a dream could safely recover its project output while still repeating the two
human-facing sends. The consumer repair introduced an append-only claim/authorization/emission ledger and
proved that a replay launches neither sender. Durable rule → Idempotent artifact production and at-most-once
human delivery are separate contracts: claim before work, authorize exactly at the send boundary, record the
output digest after delivery, and make replay a loud zero-send terminal result. Encoding → Mission Control
commit `61b88833fb144695dec694313d2ace099b6e5a72`,
`tools/ctower-dream-consumer`, `tests/test_ctower_dream_consumer.py`, and
`coordination/2026-08-10_1636--engineer-dream-dedup--idempotent-emission.status.md` carry the
`CLAIMED → AUTHORIZED → EMITTED → REPLAY` contract and its same-day, new-artifact, and later-day proofs.

### A coverage inventory must make absence durable

What happened → The complete 14-feature audit found every unit cell represented, but no feature had an E2E
that drove a supported running Ctower instance at the cutoff; in-process clients, fixture echoes, browser page
loads, and deferred suites were not promoted into passing evidence. Durable rule → Coverage reporting names
the feature, layer, exact required selector, and live boundary independently; every absent cell receives one
owned ticket and remains a visible gap until that exact layer is exercised. Encoding →
[PR #460](https://github.com/simjak/ctower/pull/460) preserves the reviewed matrix; Mission Control
`board/ctower-agreed-structural-decisions.md` Decision 11, and
`coordination/2026-08-11_0136--review-460-fable--coverage-matrix.status.md` preserve the
42-cell denominator, the 25-present/17-gap accounting, and the one-to-one gap-ticket register.

### Mandatory controls must traverse the credential-bearing lifecycle

What happened → The first exact-head GitHub connector security pass found green named tests that never reached
the connected rotation/revocation lifecycle and a taint fixture that failed before credentials existed. The
repair made each required path reachable, fail-closed, and independently rechecked; the candidate remained
unmerged at this cutoff. Durable rule → A mandatory control test proves the real production composition and
asserts that every intended lifecycle branch was reached; an early refusal can be safe behavior without being
evidence for downstream containment. Encoding →
`tests/modules/integrations/test_github_security_controls.py`,
`apps/ctower-api/src/ctower_api/connector_loop.py`, and Mission Control
`coordination/2026-08-11_0200--cso-r429-reverify2--cured-head.status.md` preserve the six-path
reachability matrix, exact-head clearance, and head-change invalidation rule.

### Current UNENCODED register at the cutoff

- The prior Request-acceptance gap is now encoded in running development behavior: [#400](https://github.com/simjak/ctower/issues/400)
  closed through merged PR #412 with server-owned identity, durable acknowledgement, read-back, and the
  fail-closed cutover fence. This removes it from the register without claiming CP3-D or portfolio authority.
- Operational sender attribution remains **UNENCODED** in Mission Control tooling. Owner: dream-consumer
  Engineer with ctower Commander. Due: land a substrate-owned sender identity and impersonation regression, or
  record the exact blocker, by `2026-08-11 12:00 Europe/Berlin`. Source: Mission Control
  `board/ctower-migration-status.md:2538`.
- Executable-remedy enforcement remains **UNENCODED**: [#357](https://github.com/simjak/ctower/issues/357)
  and [#358](https://github.com/simjak/ctower/issues/358) are still open. Owner: ctower Commander. Due:
  land or explicitly re-scope the durable executable, invocation, regression, and blast-radius gate by
  `2026-08-11 12:00 Europe/Berlin`.

The recurrence collision and Request acceptance gaps are now encoded. The two rows above remain overdue and
must not disappear from the digest merely because their original deadlines passed.
