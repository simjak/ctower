...
function namesOne(payload: Readonly<Record<string, unknown>>, named: ReadonlySet<string>): boolean {
...
export function staleNamersOf(
  ...
  return document.resources.filter(
    (candidate) =>
      idOf(candidate.component) !== idOf(resource.component) && namesOne(candidate.payload, earlier)
  );
}
