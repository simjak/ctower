...
export function routeTo(correspondents: readonly InboxCorrespondent[], seatKey: string): Route {
  ...
  // The correspondent read serves its rows `ORDER BY seat_key`, so this is the
  // record's own order; sorting it again would overrule it client-side.
 ...
 * seat_key`, so a re-sort here would impose a second answer on an ordered one.
 */
