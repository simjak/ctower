...
import { Screen, screenFromSearch } from "./t030/Screens";
...
export function Gallery(): ReactElement {
  // A whole screen, when the address asks for one. T-030's Configuration tab is
  // an assembly rather than a component, so the bench draws it inside the real
  // shell; `?screen=…` picks which of the three, and no address at all keeps the
  // components.
  if (new URLSearchParams(window.location.search).has("screen")) {
    return <Screen which={screenFromSearch(window.location.search)} />;
  }
  return (
