...
function Moves({
  ...
  return (
    <div className="space-y-1 border-t border-line pt-3">
      {moves.length === 0 ? null : <Named what={moves} says="Changes with this edit" />}
      {behind.length === 0 ? null : <Named what={behind} says="Stays on the older version" />}
    </div>
  );
}
...
function Named({
  ...
  return (
    <p className="m-0 text-xs text-muted">
      {says}: {what.map((resource) => nameOf(resource)).join(" · ")}
    </p>
  );
}
