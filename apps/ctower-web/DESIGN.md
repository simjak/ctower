...
| 2026-08-24 | **Zero technical text on a rendered surface.** No component key, `@revision`, digest, schema name, wire enum, uuid, `repository:` reference or file path renders by default — anywhere. A value the record needs and the operator never typed is derived, or it sits behind a control that says `Developer details` | Operator standing rule, verbatim: "do not leave any technical text on UI". It is not a copy preference: an identifier a person cannot act on is noise wearing the authority of a fact, and this console's whole claim is that everything on it is real to the person reading it |
