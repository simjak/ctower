...
def ruling_parser(parser: argparse.ArgumentParser) -> None:
    ...
    _command_id(append)
    relation = append.add_mutually_exclusive_group()
    relation.add_argument("--request-id", type=UUID)
    relation.add_argument("--supersedes", dest="supersedes_ruling_id", type=UUID)
