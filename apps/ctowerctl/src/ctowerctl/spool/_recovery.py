...
class RecoverySnapshot:
    """Process-local verified state reusable while every durable file identity is unchanged."""
...
def recover_session(
    ...
    *,
    ...
    stored_cursor = _read_recovery_cursor(store, keys)
...
def snapshot_session(session: Session) -> RecoverySnapshot:
    """Capture immutable verified values after the session's cursor is checkpointed."""
...
def _read_recovery_cursor(store: LockedStore, keys: KeySet) -> RecoveryCursor | None:
