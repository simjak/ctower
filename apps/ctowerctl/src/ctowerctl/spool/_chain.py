...
class RecoveryCursor(BaseModel):
    """Authenticated progress through a specific durable chain state."""
...
class RecoverySnapshot:
    """Verified state reusable only while every durable file identity is unchanged."""
...
class _VerifiedSession(Protocol):
    metadata: Metadata
    head: Head
    keys: KeySet
...
def read_recovery_cursor(store: LockedStore, keys: KeySet) -> RecoveryCursor | None:
...
def load_or_create_head(store: LockedStore, keys: KeySet) -> Head:
...
def snapshot_session(session: _VerifiedSession) -> RecoverySnapshot:
    return RecoverySnapshot(
        ...
        recovery_cursor=session.recovery_cursor,
        maximum_record_bytes=session.maximum_record_bytes,
        torn_evidence=session.torn_evidence,
    )
...
def snapshot_matches(
    ...
    snapshot_files = tuple(
        sorted(
            (
                *(record.stored for record in snapshot.records),
                *(record.stored for record in snapshot.corrupt_records),
            ),
    ...
    return (
        ...
        and snapshot_files == stored_records
    )
