...
Recovery checkpoints a metadata-MAC-authenticated chain cursor and reuses a process-local verified snapshot only while the signed head and every durable file identity remain unchanged; any mismatch reopens and authenticates the chain. Ordinary spool sessions automatically compact wholly old accepted history to its authenticated anchor at the existing retention horizon, while quarantine still requires explicit disposition.
