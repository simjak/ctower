...
function List({
  ...
  return (
    <div className="main" style={{ gridColumn: 1, gridRow: 1 }}>
      <div className="stack">
        {entries.map((entry) => (
...
function BoardBody({
  ...
  return (
    <>
      ...
      <main className="page">
        <div className="wrap">
          {/* the rail is written first so a phone, where `.cols` is not a grid
              and order is the document's, reads the lane counts and the filter
              before three hundred cards rather than after them. On the desk the
              explicit placement puts it back on the right */}
          <div className="cols" style={{ paddingTop: "16px" }}>
            <BoardRail entries={fromSource} shown={shown} selection={selection} />
