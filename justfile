...
# `git ls-files` is visible; an empty or partial corpus refuses instead of reporting a clean
# scan of nothing. `no leaks found` is only trustworthy alongside the corpus count.
secrets-intended-tree:
    @scan_root="$(mktemp -d)"; file_list="$(mktemp)"; config_path="$(pwd -P)/.gitleaks.toml"; trap 'rm -rf -- "$scan_root" "$file_list"' EXIT; git ls-files --cached --others --exclude-standard -z > "$file_list"; listed=0; scanned=0; skipped=""; while IFS= read -r -d '' file_path; do listed=$((listed + 1)); target="$scan_root/$file_path"; mkdir -p -- "$(dirname -- "$target")"; if [[ -L "$file_path" ]]; then readlink "$file_path" > "$target"; scanned=$((scanned + 1)); elif [[ -f "$file_path" ]]; then cp -p -- "$file_path" "$target"; scanned=$((scanned + 1)); else skipped="$skipped $file_path"; fi; done < "$file_list"; printf 'secret scan corpus: listed=%s scanned=%s\n' "$listed" "$scanned"; if [[ "$listed" -eq 0 || "$scanned" -ne "$listed" ]]; then printf 'secret scan corpus is empty or incomplete; refusing to report a clean scan (skipped:%s)\n' "${skipped:- none}" >&2; exit 1; fi; cd "$scan_root"; {{gitleaks}} dir . --config "$config_path" --no-banner --redact --verbose
