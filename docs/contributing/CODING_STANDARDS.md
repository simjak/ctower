...
`just check` is the warm gate. `just verify` repeats it and adds full repository policy, generated drift, coverage, E2E/conformance/acceptance/chaos suites, history secret scan, and a clean-diff proof. Both are non-mutating. CI invokes these commands rather than maintaining parallel rule implementations.

`tools/checks/expected-suites.toml` is the only verification-scope manifest. Its active phase and ordered backlog phases determine which suites are required. A current suite that is missing, has no executable tests, is malformed, contains an unexpected skip, times out, or returns nonzero blocks verification. Commands are argv arrays executed without a shell. A later suite is reported as `not_yet_required`; it is never executed, represented by an empty placeholder, or counted as passing. A backlog owner activates a suite by advancing this manifest in the same reviewed change; the stable `just verify` command does not acquire a parallel suite list.
