...
repository's separate read-only development UI is not installed or served by this
...
| To try ctower on real but low-value work | Yes, with reconstructible work only |
| A team-shared service reachable over a network | No — it binds `127.0.0.1` and creates no external listener |
| A browser UI from this install | No — the repository's read-only development UI is a separate surface |
...
privileged bearer values live in the OS keyring; no unit, config file, environment variable, or
...
4. run `expose-cli` to add `ctowerctl`, `ctl`, and `ctower-shadow-ctl` to the owning account's path and write
   the owner-only instance catalog;
5. `database-up` to create the primary and standby pair and apply migrations under an advisory lock;
6. `install-units`, then `bootstrap` for the first tenant and its initial human authorities; and
7. `observe` to confirm the result.
...
After `expose-cli`, drive the instance through `ctower-shadow-ctl`, `ctowerctl`, or `ctl`. The shadow name is
a wrapper pinned to this instance; the other two resolve the same owner-only instance catalog when exactly
one instance is declared. All three use the command surface in the [CLI reference](reference/cli.md) and
read authority from one bounded stdin line.

The runbook also provides named local database undo:

```bash
ctower-private-vps checkpoint
ctower-private-vps checkpoint list
ctower-private-vps restore --checkpoint-id CHECKPOINT_ID
```

A checkpoint is an immutable local database artifact with an append-only integrity ledger. Restore replaces
the one application database only after the runbook verifies the checkpoint identity, recorded schema, and
serving generation. It is a recovery tool for this development host, not an off-host backup,
production recovery claim, or product-domain delivery checkpoint.
...
material for a deliberate recovery decision — not permission to improvise a restore.
...
6. health, telemetry, alerting, and service ownership that fail closed on unknown state;
7. promotion or replacement of the read-only development UI with a supported browser product, or an explicit
