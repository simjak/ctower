...
| <a id="ac-role-01"></a>AC-ROLE-01 | For an active project-seat grant, the server resolves exactly one authenticated role fact by joining the grant's `principal_id`, tenant, project, and stable `seat_key` to the active CompanyBundle assignment subject and its pinned assignment/component and seat-catalog revisions. The configured assignment subject is the role identity; the role key is never accepted from a command, inferred from a principal ID, display name, model, harness, profile, crew name, stage name, or silence. The fact is a derived read over the existing seat/assignment authorities, not a second crew/role store, principal class, grant, credential, or bearer format. Missing, ambiguous, revoked, expired, stale, or foreign-scope seat/assignment resolution is an explicit `role-resolution-unavailable` refusal with zero mutation. | Authenticated project-seat/CompanyBundle join matrix; current-role positive and missing/ambiguous/revoked/expired/stale/foreign negatives; revision-pinned derived-read/rebuild proof; no-new-store/principal/grant/bearer inventory |
| <a id="ac-role-02"></a>AC-ROLE-02 | Every role-bearing stage transition and stage attempt receives the role fact resolved from the acting Actor's authenticated project seat before policy/predicate success is accepted. The Workflow run and transition evaluation pin the resolved participant/role digest, preserve the exact seat, assignment subject, scope, and source revisions used, and never replace that fact with caller text or a later bundle/seat revision. A missing or unknown role fact never becomes an empty member set or a permissive default. | Accepted member transition and restart/replay/reassignment traces; transition-evaluation and run-manifest snapshots; changed-assignment/revocation invalidation matrix; caller-supplied-role and empty-default negatives |
| <a id="ac-role-03"></a>AC-ROLE-03 | If a Workflow stage declares role keys, the authenticated role fact must be a member of that stage's declared set and every existing edge, assignment, evidence, gate, scope, and predicate condition must also pass. A non-member is refused before authoritative transition mutation with the named `role-not-permitted` refusal, alongside the existing `transition-not-declared` and `predicate-unsatisfied` refusals (wire problem codes `workflow-role-not-permitted`, `workflow-transition-not-declared`, and `workflow-predicate-unsatisfied`). The refusal records the exact unmet role/stage and one transition evaluation, changes no stage/job/gate/Evidence/delivery fact, and cannot be bypassed by a caller-supplied role, profile, model, harness, or crew label; the same matrix proves an authenticated member can proceed when all other conditions pass. | Real-PostgreSQL permitted/non-member transition pair; exact refusal payload and zero-diff query; transition-evaluation audit; caller-identity/profile/model/harness/crew-label bypass negatives; existing transition/predicate refusal parity |
| <a id="ac-role-04"></a>AC-ROLE-04 | The Execution Policy `participant_resolution` schema is an optional, unique list of `{stage_key, role_keys}` entries. Each `stage_key` must name a declared Workflow stage and each `role_keys` member must already be declared by that stage; the effective set is the Workflow stage role set narrowed by the policy entry. A policy may remove eligible roles but may not add a role, stage, edge, capability, or participant, may not duplicate or make a stage mapping ambiguous, and may not turn an empty/unknown effective set into permission. Unknown, duplicate, widened, or otherwise non-narrowing mappings are refused before publication or dispatch; absent resolution never acts as a wildcard for a role-bearing stage. | Strict policy schema positive/negative vectors; stage-role subset and unknown/duplicate/widening matrix; publication/dispatch zero-mutation proof; cross-package arbitrary-role-key fixture; `participant_resolution` consumer trace |
...
| CT-I1-036 | Deliver the authenticated project-seat crew/role record as a derived read over the existing seat catalog, project-seat grant, and CompanyBundle assignment authorities: resolve one revision-pinned role fact for the acting principal; make Workflow stage-role membership available to transition evaluation; add the closed `participant_resolution` stage-to-role narrowing contract; and refuse non-members by name without transition mutation. The current `ctower:<role>` assignment subjects and matching stable seat keys are deployment data, not a fixed product roster. This candidate adds no crew/role store, principal class, grant, credential/bearer format, product route, live-instance mutation, or policy-only decoration; it is the authority prerequisite for package-defined execution-policy enforcement in CT-I2-006. | CT-I1-003; CT-I1-009; D28; D31; D68; operator order R3000 item 5 | Engineer + Engineering Manager + CSO + independent QA/Review | Authored role/participant contracts; existing Catalog/CompanyBundle, Access, and Workflow Interfaces; `packs/policies/execution/` consumer contract; generated API/CLI only where existing reads require it | AC-ROLE-01..04; RED-first authenticated seat/assignment resolution, permitted/non-member transition, refusal/state-diff, revision/revocation, and participant-narrowing proofs; current-seed role inventory; no-new-store/principal/grant/bearer scan | `uv run pytest tests/contracts/workflow tests/modules/workflow/test_execution_policy.py tests/acceptance/increment-2/test_workflow.py -q` |
