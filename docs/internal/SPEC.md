...
| <a id="ac-tm-13"></a>AC-TM-13 | Ticket/API/CLI query and Board card surfaces expose the display key. Every CLI position that accepts a Ticket reference accepts either the canonical UUID or `PREFIX-N`; display-key lookup resolves only through the authorized `(project, number)` index and never discloses a foreign Project. | Generated API/Board snapshots; UUID/display-key CLI parity transcript; same-project lookup and foreign-project refusal |
