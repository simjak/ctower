...
| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-rwi-01"></a>AC-RWI-01 | An accepted Routine fire appends exactly one Inbox work item addressed to the Routine's owning seat, carrying the Routine identity, the window it belongs to, the gate evidence and watermark it stood on, and one Knowledge reference; it writes nothing to any session and the candidate retains no session-targeted dispatch path. Exact replay appends no second item. | Real-PostgreSQL fire-to-item trace; strict item contract round-trip; exact-replay zero-diff; session-write and target-session inventory proving the path is absent, not merely unused |
| <a id="ac-rwi-02"></a>AC-RWI-02 | The item references what the seat must produce as a Knowledge record and embeds no instruction text of its own. Editing that Knowledge record changes what the next item points at without any Routine or item change. An item whose payload carries embedded specification text is refused. | Pointer-resolution trace; Knowledge-edit-then-fire fixture proving inheritance; embedded-instruction refusal matrix; item payload scan for specification text |
| <a id="ac-rwi-03"></a>AC-RWI-03 | While the previous window's item for a Routine is unconsumed, the next fire is suppressed and appends one typed suppression fact naming the blocking item; it never silently skips and never appends a second open item. Once the item is consumed, the following window fires normally. | Open-item/consumed-item matrix over real PostgreSQL; suppression-fact snapshot; one-open-item invariant assertion; resume-after-consumption trace |
| <a id="ac-rwi-04"></a>AC-RWI-04 | The owning seat closes an item with a receipt referencing the delivered artifact; the closure is an append-only fact carrying the reference and server time. A closure without an artifact reference, a closure by a seat that does not own the item, and a re-closure of a closed item each refuse with zero mutation. | Receipt-closure trace; missing-reference, foreign-seat, and double-closure refusal matrix; append-only storage inspection |
| <a id="ac-rwi-05"></a>AC-RWI-05 | A window that ends with its item unconsumed raises exactly one typed alarm fact to the declared escalation seat, naming the Routine, the window, and the unconsumed item; the alarm is idempotent per window and never substitutes for the item itself. A partial or failed read of item state produces a typed degraded alarm rather than a clean no-alarm. | Window-boundary matrix including on-boundary vectors; alarm idempotency proof; degraded-read fixture; escalation-address authorization check |
| <a id="ac-rwi-06"></a>AC-RWI-06 | Every Routine already migrated under CT-I1-033 is re-pointed to the item path in this same candidate, and each carries catch-parity evidence naming both its host twin's execution and its own item-plus-receipt before the twin is retired. Host-side retirement remains the external custodian's act. | Per-Routine re-point inventory; catch-parity report per migrated Routine; host-twin non-interference proof |
...
| CT-I1-035 | Deliver Routine work items as one landable candidate per D62 and D67: an accepted Routine fire appends exactly one Inbox work item for the owning seat carrying Routine identity, window, gate evidence and watermark, and one Knowledge reference, with the session-targeted dispatch path REMOVED rather than kept beside it; the item embeds no specification text so policy lives once in Knowledge; a typed gate over recorded Inbox state suppresses firing while the prior window's item is unconsumed and records the suppression as a fact; closure requires a receipt referencing the delivered artifact and refuses foreign-seat, missing-reference, and double closure; a window ending unconsumed raises one idempotent typed alarm to the declared escalation seat, degraded rather than clean on a partial read; and every Routine migrated under CT-I1-033 is re-pointed in this same candidate with catch-parity evidence before any host twin retires. Adds no product UI, principal, session write path, second Inbox store, or expression language. | CT-I1-003; CT-I1-033; D62; D66; D67; operator orders R3000, R3016 | Engineer + independent QA/Review + CSO addressing/authorization review; director confirms live receipt and alarm | Authored routine-item/receipt/alarm contracts; kernel `runtime/`, `record/`, `knowledge/`, `projections/`; generated clients; `ctower-api`; `ctowerctl`; the five migrated Routine registrations; `docs/concepts/routines.md` and Inbox/CLI/API references | AC-RWI-01..06; RED-first fire/pointer/suppression/receipt/alarm/re-point suites; real PostgreSQL fire-to-item-to-receipt trace; session-write absence inventory; catch-parity reports; same-candidate docs | `uv run pytest tests/contracts/routine_items tests/modules/runtime/test_routine_items.py tests/acceptance/increment-1/test_routine_items.py -q` |
