...
| CT-I2-014 | Implement the bounded chat/steer HTTP surface against the runner service. Keep the closed operation inventory to the cockpit proposal's justified crew/liveness/spec reads, typed transcript read/stream, composer input, and steer/ACK; expose serving-model evidence, harness/spec/capability identity, liveness and watermarks; make command IDs, refusal reasons, cursor gaps, and pending-versus-folded durability explicit; and close authored OpenAPI, generated Python/TypeScript clients, contract counters, and reference documentation together. No registration, guard dry-run, credential mint/reference/keep-or-evict, workspace-change, pull-request, public product page, navigation entry, or browser UI is activated by this row. The operator-principal authority for steer and the typed transcript vocabulary remain named prerequisites. | CT-I1-041..044; accepted harness-adapter phase activation at `tools/checks/expected-suites.toml` (`active_phase = "CT-I1-043"`); CT-I2-004; CT-I2-013; D72; D75; operator-cockpit design §8 as proposal-stage source | Engineer + API/CLI + CSO + independent Review/QA | `contracts/http/openapi.yaml`; generated Python/TypeScript clients; `apps/ctower-api/`; `apps/ctowerctl/`; `docs/reference/http-api.md`; HTTP contract/counter/docs and Increment 2 acceptance suites | AC-CHAT-01..07; four-inventory parity; project authorization/redaction; cursor/gap/replay; composer/steer ACK and refusal matrix; pending-to-folded projection trace; no credential values; no public-page or browser-surface claim; one `deferred` manifest-fixture disposition per new AC; R3114/R3117 review-round ledger proves at most three current-digest rounds and records any post-round-3 residue only as one-line tech debt unless it is security, data-loss/corruption, production-breaking, or gate-failing | `uv run pytest tests/contracts/http tests/acceptance/increment-2/test_chat_steer_http.py -q && python -m tools.codegen --root . --check && just check && just verify` |
