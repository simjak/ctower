...
| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-inbox-01"></a>AC-INBOX-01 | A generated-client and protected-CLI roundtrip sends one native inbox message, records recipient-only `delivered` and `read` acknowledgements in order, and queries exact per-message `sent -> delivered -> read` state including immutable event IDs and server timestamps. Direct `read` records both missing facts atomically; exact replay returns the same result. D44 and D45 additionally permit only `ctower-ui`'s server-mediated dogfood send box for this existing command: no browser credential/authority, only `{"text","thread_id","to"}` with a server-resolved recipient and no claimed sender, one Idempotency-Key reused across bounded retries and across a sender-pressed retry of the same unconfirmed message, terminal problem detail rendered as human copy, the accepted message rendered in the same document, and a non-accepted `durability_pending` answer rendered as no message at all with the draft kept. D55 additionally permits only that surface's compose control over the existing pair-grouped notification command: a recipient chosen from the server-listed addresses and re-checked against that list server-side, where that list is the set the command accepts inside the reader's own persisted project grant, so a seat key two seats share, the reader's own seat, and every address when the reader holds no seat are offered by neither the list nor the command, while an address in a project the reader holds no grant on is absent from the list although the command still accepts it — discovery is a project-scoped read under [INV-69](#inv-69) and delivery is tenant-wide ([D73](DECISIONS.md)); only `{"text","to"}` with no claimed sender and no caller-chosen thread, the server-derived pair thread so a second compose continues one conversation, and the same three renderings with the picked seat kept beside the draft. | Real PostgreSQL API/CLI send/ack/read-state transcript, authority/event query, replay assertion; a real-PostgreSQL correspondents/compose proof that reads and sends in one registry state holding a two-project shared seat key, plus the pair-grouping proof; `ctower-ui` request-shape/idempotency/refusal/durability-state tests and a rendered 375/768/1440 browser round trip covering the accepted and non-accepted answers of both boxes |
