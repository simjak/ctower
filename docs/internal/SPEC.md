...
| CT-I1-032 | Deliver the external-estate record imports as one landable candidate per D62 and D66: execute the activated OR-06 Request-ledger cutover through the existing signed-manifest machinery of AC-REQ-06 under the D66 window authority; add operator-authority manifest-bound imports for inbox history, agreed-decision Rulings, knowledge documents, and one new append-only company-record aggregate for accountability rows; every tier idempotent by its natural key with typed parity reports emitted before any source closure, unknown owners mapped to an explicit source-only state, and no principal, seat, or grant minted by any import. Source freezes, fence ordering, and the archive act stay with the external custodian. | CT-I1-003; D30; D62; D66; operator order R3000 | Engineer + independent QA/Review + CSO imported-tenant-content boundary review; director parity acknowledgment per surface | Authored import-manifest contracts; kernel `record/`, `knowledge/`, `work/` import paths behind existing Interfaces; `ctowerctl` migration commands; company-record aggregate + migration SQL; parity-report artifacts | AC-REQ-06 execution evidence; AC-IMP-01..04; RED-first manifest/idempotency/refusal suites; real PostgreSQL import traces; parity reports per tier | `uv run pytest tests/contracts/estate_imports tests/modules/migration/test_estate_imports.py tests/acceptance/increment-1/test_estate_imports.py -q` |
