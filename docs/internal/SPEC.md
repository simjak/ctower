...
| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-chat-01"></a>AC-CHAT-01 | The HTTP surface is a closed operation inventory limited to the cockpit proposal's justified G1–G4 paths: crew/liveness/spec reads, typed transcript read/stream, composer input, and steer/ACK. Registration, guard dry-run, credential mint/reference/keep-or-evict, workspace-change, pull-request creation, and any other control operation are absent rather than hidden behind a generic endpoint. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; OpenAPI operation count and generated-client registry diff reserved for the active CT-I2-014 implementation |
...
| <a id="ac-chat-06"></a>AC-CHAT-06 | Every read and write is project-authorized and strict. Response shapes expose pending versus folded durability, source watermark, and unknown/refusal reasons; no HTTP, generated client, transcript, log, fixture, or public response contains a credential value or raw secret-bearing reference. The browser has no new route or credential authority in this row. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; secret-pattern scan and authorization/redaction response matrix reserved for the active CT-I2-014 implementation |
