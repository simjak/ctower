...
### Operator human-session read surface

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-login-01"></a>AC-LOGIN-01 | A configured private-origin OIDC login verifies discovery issuer, authorization-code/PKCE state and nonce, and the returned `(issuer, subject)` against one active operator-issued human role binding; names, email, role claims, and browser-supplied project keys never authorize a read, while an unbound or disabled identity refuses with the established `auth-identity-unresolved` or `auth-role-denied` code. Operator outcome: he logs in as the human identity provisioned for him rather than as a browser-selected account. | Configured-provider and fake-provider exchange matrix; issuer/subject-binding positive and claim/project-key substitution negatives; operator-only binding mutation proof; exact RFC 9457 refusal snapshots |
| <a id="ac-login-02"></a>AC-LOGIN-02 | After a successful exchange, the server uses the existing opaque, bounded, revocable human session plus CSRF cookie at the pinned private origin; no provider token or API bearer enters browser JavaScript, DOM, URL, Web Storage, trace, screenshot, log, or telemetry, and every non-GET browser command retains the existing exact-Origin/CSRF checks. Operator outcome: he can return to the surface without an agent typing a credential while the browser holds no reusable bearer authority. | Cookie-attribute and token-disposition scan; browser-context redaction capture; exact-Origin/CSRF positive and negative matrix; session TTL and replay vectors |
| <a id="ac-login-03"></a>AC-LOGIN-03 | Every human-session read resolves exactly one server-side Actor under [INV-73](#inv-73) and then evaluates the same active Project-grant predicate under [INV-69](#inv-69) used by machine credentials before selecting project data, with the human binding's tenant and exact project keys as that predicate's input and the existing portfolio-operator rule as its only defined aggregate exception; no role-only, UI-only, or session-only allow path exists, and a missing, ambiguous, revoked, or foreign binding returns `project-scope-denied` with zero rows and zero mutation. Operator outcome: he sees only the Projects and source facts his operator-issued binding allows. | Human-session versus project-seat authorization equality matrix; all three project keys plus foreign-tenant and revoked-binding negatives; shared-predicate/route inventory; before-and-after state digests |
| <a id="ac-login-04"></a>AC-LOGIN-04 | If a source, projection, or authorization proof is unavailable, partial, or stale, the read preserves the existing typed `STATE_UNKNOWN`/unavailable result with its source, reason, and watermark instead of returning zero, empty, or an all-clear value, while independently authorized Projects remain distinguishable. Operator outcome: he can tell “not reached” from “no records,” so missing crew or report facts do not look complete. | Source outage/partial/stale matrix over each existing operator read; API/UI snapshots proving unknown is not empty; per-Project partial-read and watermark evidence; no-all-clear scan |
| <a id="ac-login-05"></a>AC-LOGIN-05 | Logout revokes the current session and clears its cookies; expiry, explicit session or binding revocation, provider disablement, and session mismatch refuse the next read with the exact `auth-session-invalid` or `reauthentication-required` code, serve no cached read, and never let an old or replayed cookie recover. Operator outcome: signing out or removing access takes effect on the next request, including from a stale tab. | Logout/revocation/fake-clock matrix; old-cookie replay after each revocation cause; cache-absence assertion; exact refusal snapshots and zero-diff proof |
| <a id="ac-login-06"></a>AC-LOGIN-06 | The existing private `ctower-ui` real-read screens consume the human-session path through the server-side read adapter, while its machine-token mode remains a separately configured path and is never silently substituted as human-session evidence; the slice adds no public ingress, self-signup, new environment variable, feature flag, or CT-I2-005 product route. Operator outcome: he opens ctower himself and inspects recorded request, crew, and report facts without an agent proxying the view or widening access. | Same-head human-session UI/API transcript over Board, Ticket, Project Delivery, roster/cadence, and Inbox reads; machine-token substitution negative; browser-secret absence scan; route/ingress/env/flag inventory; `pnpm --filter @ctower/ui typecheck` |
...
ctower has no ticket API yet. The 53 stable IDs below—9 L0 preconditions, 32 I1 items, and 12 I2
...
| CT-I1-037 | Deliver the private operator human-session read handoff as one landable candidate: consume CT-I1-013's configured provider registry, PKCE exchange, opaque session/CSRF transport, and operator-issued pinned human role binding to resolve one Actor for the existing `ctower-ui` read adapter; pass every project-scoped read through the same server-side Project-grant predicate as machine credentials using the binding's exact tenant/project keys; preserve typed unavailable/`STATE_UNKNOWN` reads; revoke and expire sessions fail closed on the next request; and keep browser tokens, bearer credentials, self-signup, public ingress, new environment variables, feature flags, and CT-I2-005 product routes out. Provider and human-role provisioning remain operator-only configuration/administration, not browser authority. | CT-I1-009; CT-I1-010; CT-I1-012; CT-I1-013; D30; D31 | Engineer + QA/UI QA + CSO authorization/isolation review; operator verifies the private read outcome | Existing `ctower-api` auth/session and read paths; `apps/ctower-ui/` server-side read adapter; generated contracts only where the existing auth/read shapes require them; operator-surface documentation and auth/read tests | AC-LOGIN-01..06; RED-first identity/session/project-predicate/unknown/revocation/browser-redaction suites; human-session versus machine-grant equality trace across the existing read screens; exact refusal snapshots; no-new-ingress/env/flag inventory; same-candidate UI typecheck | `uv run pytest tests/modules/access/test_human_login.py tests/modules/access/test_human_role_binding.py tests/modules/console/test_browser_auth.py tests/acceptance/increment-1/test_auth_http_flow.py -q && pnpm --filter @ctower/ui typecheck` |
