...
| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-run-16"></a>AC-RUN-16 | Increment 2 has one long-running runner owner per configured target, delivered by a systemd unit and driven by the authoritative runner registry. Desired-to-observed reconciliation is idempotent; a duplicate, stale, or fenced owner cannot become active; restart resumes reconciliation without a second owner, cron loop, ad-hoc shell loop, or service-per-noun reconciler. | `systemd-analyze verify deploy/private-vps/development/systemd/ctower-development-runner.service`; `uv run pytest tests/conformance/runner tests/acceptance/increment-2/test_runner_service.py -q`; owner/restart/duplicate trace with exact unit inventory |
| <a id="ac-run-17"></a>AC-RUN-17 | Every spawn resolves the registry-pinned HarnessSpec, Supervisor, target, workspace, telemetry, build digest, and CredentialPool binding. The merged binding is the only launch/submit path; the normalized plan and targets pass the current Runtime CommandGuard at the final pre-dispatch boundary; unknown, incompatible, revoked, digest-mismatched, or unfenced input performs zero process action and returns a typed refusal. | `uv run pytest tests/conformance/runner tests/conformance/harness tests/acceptance/increment-2/test_runner_dispatch.py -q`; binding/guard/refusal vectors; generated-client import-boundary check |
| <a id="ac-run-18"></a>AC-RUN-18 | Liveness is emitted by service-owned observation cycles with independent timestamps and watermarks. Working, capped with a reset or unknown reset, saturated, dead-auth, stale, gapped, and unknown are distinct typed outcomes; process exit, a vendor heartbeat, pane silence, or a missing footer cannot assert success, completion, or a healthy pool. | `uv run pytest tests/conformance/runner/test_liveness.py tests/acceptance/increment-2/test_runner_liveness.py -q`; deterministic-clock stale/gap/saturation vectors; response-schema enum and watermark assertions |
| <a id="ac-run-19"></a>AC-RUN-19 | `collect` crosses the boundary only as strict, revision-pinned typed artifacts and a typed transcript vocabulary closed by the binding contract. Harness-private transcript/session formats remain inside the binding; kernel, reporter, projection, CLI, and Board code never parse them. Missing, stale, gapped, unreadable, or vocabulary-unknown collection yields explicit unknown or refusal and never a completion claim. | `uv run pytest tests/conformance/runner/test_collect.py tests/conformance/harness/test_transcript_boundary.py tests/acceptance/increment-2/test_runner_collect.py -q`; schema/codegen parity; forbidden-import and raw-format inventory |
| <a id="ac-run-20"></a>AC-RUN-20 | `writeback` uses the generated authenticated API client and the seat's own project-seat credential with only the declared capture, transition, and evidence scopes; the runner never connects to Record/Postgres or appends protected verdict/resolution/freeze truth. `teardown` is checkpoint, park, or reap with a typed acknowledgement and fence; saturation and loss preserve the declared checkpoint order, and stop/restart/replay cannot double-apply a lifecycle fact. | `uv run pytest tests/conformance/runner/test_writeback_teardown.py tests/acceptance/increment-2/test_runner_recovery.py -q`; database-privilege and import-boundary diff; duplicate/restart/teardown trace |
| <a id="ac-run-21"></a>AC-RUN-21 | The credential fixture preserves the sealed seam topology: each Hermes profile has five subscriptions — three separately minted Codex accounts plus z.ai and Alibaba — yielding the three ordered provider rungs `codex` to `glm` to `qwen`; `claude-code` has three per-seat credential homes with one rung per home. The pool interface distinguishes configure/observe from managed perform, has no copy verb, stores references and typed state only, and emits no credential value; the unresolved managed-store boundary blocks activation rather than being guessed. | `uv run pytest tests/conformance/harness/test_credential_topology.py tests/conformance/harness/test_pool_states.py -q`; topology/count/layer assertions; secret-pattern scan over generated responses, logs, fixtures, and status artifacts |

### Chat and steer HTTP surface

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-chat-01"></a>AC-CHAT-01 | The HTTP surface is a closed operation inventory limited to the cockpit proposal's justified G1–G4 paths: crew/liveness/spec reads, typed transcript read/stream, composer input, and steer/ACK. Registration, guard dry-run, credential mint/reference/keep-or-evict, workspace-change, pull-request creation, and any other control operation are absent rather than hidden behind a generic endpoint. | `uv run pytest tests/contracts/http/test_openapi.py tests/contracts/http/test_operation_inventory.py tests/acceptance/increment-2/test_chat_steer_http.py -q`; OpenAPI operation count and generated-client registry diff |
| <a id="ac-chat-02"></a>AC-CHAT-02 | Crew discovery returns only project-authorized rows and exposes the serving model with its evidence source, harness/spec identity, capability set, liveness classification, observed-at timestamp, and watermark. Missing or conflicting evidence is rendered as unknown or refusal; no row exposes a credential value, raw credential reference, or inferred healthy/idle state. | `uv run pytest tests/contracts/http/test_crew_inventory.py tests/acceptance/increment-2/test_chat_crew_list.py -q`; response-schema snapshots; tenant/project isolation and redaction assertions |
| <a id="ac-chat-03"></a>AC-CHAT-03 | Transcript read and stream return ordered typed turns, thinking/tool/interrupt facts, elapsed and source timestamps, cursor position, gap/replay state, and durability state without parsing a harness-private pane or session format. A stale or gapped stream is explicit unknown or refusal, never silently truncated or treated as current. | `uv run pytest tests/contracts/http/test_transcript_stream.py tests/conformance/harness/test_transcript_boundary.py tests/acceptance/increment-2/test_chat_transcript.py -q`; cursor/replay/gap vectors; raw-pane import denial |
| <a id="ac-chat-04"></a>AC-CHAT-04 | Composer send creates one strict operator command with client-command id, project/crew target, normalized input, capability/authority result, and source watermark. The response distinguishes accepted-pending, folded, and refusal; an acknowledgement is not success until the durable command id is returned, and exact replay is idempotent. | `uv run pytest tests/contracts/http/test_composer_commands.py tests/acceptance/increment-2/test_chat_composer.py -q`; OpenAPI request/response examples; replay and pending-to-folded trace |
| <a id="ac-chat-05"></a>AC-CHAT-05 | Steer uses the same typed command path rather than pane keystrokes, and returns a durable command id or an exact refusal for missing capability, non-working lane, stale liveness, fence conflict, or the still-unsettled operator-principal authority rule. It never reports a local interrupt as a folded result and never accepts a runner-side operator or commander credential as a substitute for the seat writeback credential. | `uv run pytest tests/contracts/http/test_steer_commands.py tests/conformance/runner/test_steer_ack.py tests/acceptance/increment-2/test_chat_steer.py -q`; refusal matrix; command-id ACK/fence trace |
| <a id="ac-chat-06"></a>AC-CHAT-06 | Every read and write is project-authorized and strict. Response shapes expose pending versus folded durability, source watermark, and unknown/refusal reasons; no HTTP, generated client, transcript, log, fixture, or public response contains a credential value or raw secret-bearing reference. The browser has no new route or credential authority in this row. | `uv run pytest tests/contracts/http/test_auth_redaction.py tests/acceptance/increment-2/test_chat_steer_security.py -q`; secret-pattern scan; authorization/redaction response matrix |
| <a id="ac-chat-07"></a>AC-CHAT-07 | The implementation closes all four inventories together: authored OpenAPI, generated Python/TypeScript clients, contract operation counters, and reference documentation agree byte-for-byte on operations, schemas, refusal codes, and pending/folded response examples. The cockpit document remains proposal-stage input; no public product page, navigation entry, or browser UI claims this behavior before its owning implementation and browser gate are active. | `python -m tools.codegen --root . --check && python -m tools.checks.traceability --root . --check`; `uv run pytest tests/contracts/http/test_openapi.py tests/contracts/http/test_codegen.py tests/contracts/http/test_docs_inventory.py -q`; generated diff and docs-link inventory |
