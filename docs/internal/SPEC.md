...
The worker plane is replaceable. I2 wraps local `bin/mux`/tmux behind a durable runner protocol and pins each attempt's harness, supervisor, target, workspace, telemetry, and build digest. The runner service is one long-running, registry-driven owner process delivered by systemd; it reconciles desired state, observes liveness, dispatches only through the merged Harness/Runtime boundaries, collects typed artifacts; transports transcript observations through a separate typed runner vocabulary, writes back through authenticated generated APIs, and tears down or checkpoints without touching record-tier persistence. Tmux is optional same-host continuity, never ticket identity or proof. Jobs, fenced leases, structured cursors, replay, steer, cancel, and checkpoint are durable; vendor sessions are hints. The bounded chat/steer HTTP surface exposes only crew/liveness/spec reads, typed transcript read/stream, composer input, and steer/ACK operations; its response shapes show pending versus folded durability and never contain credential values. Remote/image placement remains deferred until real Adapters earn its Seam.
...
implementation and evidence begin at I2.4. D75 retires the separate, non-product `ctower-ui` Inbox dogfood
boundary and its required suite; the retained design/reference files are inert, and the separate
`ctower-web` browser lane remains deferred.
...
The v1 ordinary channel set is closed to the existing project-seat CLI. It resolves an existing Actor and
calls the same strict command; it may not claim actor, owner, project authority, priority, triage, relation,
closure, or accepted state. A future product browser channel is not implemented or evidenced by I1 and must
earn its own I2.4 contract and trust boundary. The v1 exact design adds no new trust boundary and records
`no-new-boundary`. Slack/Hermes is not v1: it is an inactive later phase
...
For a role-bearing assignment, the authored fields `CompanyBundle.assignments[*].seat_key` and
`CompanyBundle.assignments[*].role_key` sit beside that entry's `subject`, `slot`, and component reference. The
operator writes that binding with the
...
stage skips the role checks. Thus undeclared + non-member is `workflow-transition-not-declared`,
predicate-unsatisfied + non-member is `workflow-stage-role-not-permitted`, and permitted member plus an
...
caller-supplied sender. Both forms of that listing are project-scoped reads under [INV-69](#inv-69): naming
no project asks for every project the caller's grant reaches rather than for the tenant, so the listing
never discloses a project's roster to a seat that holds no grant on it, while the transport underneath
stays tenant-wide ([D73](DECISIONS.md)).
...
| <a id="ac-tm-09"></a>AC-TM-09 | Every project component carries one authored `prefix` matching `^[A-Z]{2,5}$`; one CompanyBundle refuses duplicate prefixes and reserves `CT` and `R` for their existing identity domains. Record reads the active project component's prefix and never hard-codes a project-to-prefix table or fallback. | Project schema/pack vectors; duplicate/reserved CompanyBundle refusal; active-prefix SQL read and no-hardcoded-prefix inventory |
| <a id="ac-tm-10"></a>AC-TM-10 | Capture allocates one server-owned `PREFIX-N` display key inside the same transaction as the Ticket, from a monotonic sequence scoped to `(tenant, project)`. Concurrent captures cannot duplicate a `(project, number)` pair, exact replay returns the original key, and the UUID remains the canonical Ticket identity. | Real PostgreSQL capture/replay/concurrency trace; sequence and ticket uniqueness queries; UUID/display-key response parity |
| <a id="ac-tm-11"></a>AC-TM-11 | An assigned display key is immutable and is never renumbered. Gaps are valid and remain visible in the sequence history; direct mutation, delete/recreate, or prefix-change attempts refuse without changing the Ticket, event, command result, or sequence high-water. | SQLSTATE `55000` trigger proof; gap/no-renumber state diff; replay and append-only event query |
| <a id="ac-tm-12"></a>AC-TM-12 | The additive migration backfills pre-existing Tickets in deterministic `(created_at, ticket_id)` order for each configured Project, initializes the next-number high-water without collisions, installs the immutable trigger, and leaves UUID/event history byte-equivalent. Fresh-apply and pre-existing-row fixtures both pass on disposable PostgreSQL. | Fresh-apply and pre-existing-row disposable PostgreSQL migration traces; ordered backfill/high-water query; migration manifest and checksum |
| <a id="ac-tm-13"></a>AC-TM-13 | Ticket/API/CLI query and Board card surfaces expose the display key as a required member: a Ticket that carries no key states `null` and never omits it, so no client invents a default. Every CLI position and every ticket-scoped operation that accepts a Ticket reference accepts either the canonical UUID or `PREFIX-N`; display-key lookup resolves only inside the caller's authorized Project scope, refuses an ambiguous key rather than guessing, and answers an unknown key and a foreign-Project key identically so enumeration discloses no more than guessing a UUID. | Generated API/Board snapshots; UUID/display-key CLI and replay parity transcript; same-project lookup, mutation-by-display-key trace, and identical unknown/foreign-Project refusal |

### Native inbox

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-inbox-01"></a>AC-INBOX-01 | A generated-client and protected-CLI roundtrip sends one native inbox message, records recipient-only `delivered` and `read` acknowledgements in order, and queries exact per-message `sent -> delivered -> read` state including immutable event IDs and server timestamps. Direct `read` records both missing facts atomically; exact replay returns the same result. | Real PostgreSQL API/CLI send/ack/read-state transcript, authority/event query, and replay assertion |
...
| <a id="ac-comms-03"></a>AC-COMMS-03 | Project-qualified recipient addressing is discoverable and enforced: both correspondent reads evaluate the caller's persisted grant under [INV-69](#inv-69) before any address row, and they differ only in how a foreign project is answered. The unnarrowed listing names no project and binds its rows to every project the caller's grant reaches — unrestricted operator authority reaching every project the tenant registered a seat in — so a project the caller holds no grant on is absent rather than refused; naming a project is a separate project-qualified read that names exactly one project, and a project the caller holds no grant on refuses `project-scope-denied` rather than answering empty. The comms plane itself stays tenant-wide, so an address the unnarrowed listing withholds is still deliverable ([D73](DECISIONS.md)): send/notify require the project key, same-project resolution succeeds, and foreign/unknown/ambiguous/self addresses refuse without mutation; sender remains server-derived. | CLI/API project-filter RED/GREEN, unnarrowed grant-bound listing RED/GREEN, foreign-project `project-scope-denied` trace, projection read-inventory guard, refusal matrix, real PostgreSQL address/projection query |
...
### Request maintenance proposal queue

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-prop-01"></a>AC-PROP-01 | Creating or ranking a duplicate, completed-but-open, supersession, kill, or keep proposal appends one independently identified proposal fact and leaves every target Request stream, version, disposition, relation, blocker, proof link, and exact text byte-identical. The review queue derives only from proposal facts and is not writable Request state. | Before/after Request stream and byte digest for every proposal kind; proposal event/query trace; projection privilege inventory |
| <a id="ac-prop-02"></a>AC-PROP-02 | Every proposal records the existing proposer Actor/seat, source watermark, target Request identity and expected version, and at least one typed stable evidence pointer. A completed-but-open proposal cites the accepted shipped evidence needed by ordinary Request closure and cannot use semantic similarity as delivery proof. A supersession proposal carries both Request identities and quotes both exact Request texts; omitting or altering either quote or evidence pointer refuses the proposal with zero mutation. | Strict contract and real-PostgreSQL proposal append matrix; proposer attribution query; accepted-evidence closure fixture; both-text supersession round-trip and omission/tamper negatives |
| <a id="ac-prop-03"></a>AC-PROP-03 | Only the authenticated existing operator may confirm one exact `OPEN` proposal. Confirmation records the proposal decision and causes one ordinary Request mutation as a separately identified, replay-safe command/result with its own expected-version, authority, and evidence checks; the Request changes only when that command is accepted. Commander, project-seat, proposer, Routine, digest-reader, stale-version, replay-against-later-version, and failed-target-command fixtures produce no Request mutation, and no new principal or authority path exists. | Operator/Commander/seat/Routine authorization matrix; linked proposal-decision and Request-command query; replay, stale-version, target-refusal, and zero-diff proofs; principal and route inventory |
| <a id="ac-prop-04"></a>AC-PROP-04 | Rejecting an `OPEN` proposal appends the operator identity, server time, and optional reason, derives `REJECTED`, and preserves the proposal, evidence, quoted text, and target Request unchanged and queryable. Confirmed or rejected proposals cannot be deleted, reopened, or reused against another target version. | Reject/query/restore transcript; immutable-trigger `UPDATE`/`DELETE` refusals; terminal-state and cross-version reuse negatives |
| <a id="ac-prop-05"></a>AC-PROP-05 | Conflicting or incomplete evidence, uncertain duplicate similarity, unclear supersession, stale target state, or unprovable completion produces an `OPEN` proposal row with one typed ambiguity reason. It never guesses duplicate disposition, closure, supersession, kill/keep disposition, or success from silence; correcting the source facts creates new evidence and an explicit later decision rather than rewriting the ambiguity history. | Ambiguity truth table covering all five reasons; fail-honest API/CLI snapshots; Request zero-diff assertions; append-only later-evidence trace |
| <a id="ac-prop-06"></a>AC-PROP-06 | The remaining open Requests are ranked for the top-20 kill-or-keep view by current week-goal relevance, then older age, then operator-gated before fleet-owned work, and finally stable Request identity. The morning digest receives only counts by proposal kind/state plus an authorization-preserving pointer and source watermark for that exact top-20 view: no proposal row, Request row, identity, or text is embedded in the digest. | Deterministic ranking/tie matrix with 0/1/20/21/many rows; cross-project authorization negatives; digest JSON/text snapshots and list-content absence scan |
| <a id="ac-prop-07"></a>AC-PROP-07 | Similarity may create only an `OPEN` duplicate proposal and never a Request mutation. Before append and again before an operator-confirmed duplicate command, the server proves that both Requests' stable identities and exact texts remain independently preserved; any proposed merge, alias, overwrite, or disposition path that would drop either order's identity or text refuses by stable code with zero mutation. | Similarity-proposes-only fixture; exact-text/identity preservation query; destructive-merge mutation vectors; confirm-time race and stable-refusal assertions |

### Project-management planning and reminder

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-pm-01"></a>AC-PM-01 | Request capture remains the first durable act and every new Request derives `planning_stage=PLANNED` with `dispatched=false`, where the stage is a queue position rather than a completeness claim. A feature uses its Request plan for the outcome and separate plans for every fulfillment Ticket; every other Request and Ticket likewise requires a current revision binding its subject version, outcome, constraints/non-goals, dependencies, acceptance/proof obligations, owner, next action, author/source, and digest. A changed subject or dependency invalidates the affected revision, and dispatch before a current plan plus the accepted daily command refuses with a typed unmet condition and zero mutation. No separate Feature aggregate, priority, parent plan, Routine, model output, prose, or silence skips the gate. | RED-first Request/Ticket planning-state and revision contracts; capture/default query; feature Request-to-Ticket composition trace; invalidation matrix; pre-plan/pre-confirm dispatch refusals and zero-diff proof; forbidden-Feature storage/import inventory |
| <a id="ac-pm-02"></a>AC-PM-02 | Only the authenticated existing operator may issue one one-line confirm or re-rank command for an exact Europe/Vilnius date, artifact key, content digest, and complete source watermarks. Re-rank is an exact permutation of the proposed typed stable identities. The accepted command/result records actor, inputs, resulting order, and idempotency identity, derives `planning_stage=CONFIRMED` for only matching current plan revisions, and leaves `dispatched=false` until a later ordinary dispatch; it does not dispatch, confirm a maintenance proposal, or claim execution. Commander, project seat, proposer, Routine, digest reader, inserted/dropped/substituted identity, stale source, changed-body replay, and later-version replay produce no planning or work mutation. | Generated API/protected CLI one-line confirm/rerank transcripts; operator/Commander/seat/Routine authorization matrix; exact-permutation, idempotency, stale and replay fixtures; linked command/result query; Request/Ticket/proposal zero-diff assertions |
| <a id="ac-pm-03"></a>AC-PM-03 | For one civil date and named watermarks, the proposed stack is a pure deterministic fold over the current recorded week goal and authorized accepted record facts for every current actionable Request and Ticket, including untriaged Request captures, plus open proposals. Its total order is recorded current-week-goal relation first, operator action required before ordinary work, explicit `P0` then `P1` then `P2`, older continuous actionable time, and typed stable identity; a proposal inherits its target Request's recorded goal relation and priority. Authoritative absent relation is false, while unread or conflicting input is unknown. Identical facts rebuild byte-identically, and no model score, generated prose, similarity, caller order, or projection write supplies membership or rank authority. | Property/tie matrix across subject kinds, priorities, equal ages, and stable identities; randomized-input/rebuild byte comparison; complete/absent/unknown relation vectors; model/prose/caller-order injection negatives; projection privilege inventory |
| <a id="ac-pm-04"></a>AC-PM-04 | The exact rolling 24-hour view inlines one self-contained line for every operator-originated Request — identified by recorded requester/source provenance regardless of capturing seat — created or changed in the window and every typed state change on a Ticket linked to one or currently requiring operator action, carrying stable identity and Project, remembered ask/outcome, exact change and current state, owner, blocker, and next action. Raw comments, logs, evidence bodies, repeated events, and proposal/Request review lists render only counts plus authorization-preserving pointers at the same watermarks; no D59 list is embedded. The same projection renders R2951 for the seven preceding Europe/Vilnius civil dates grouped by Project, one row per stable Ticket with current typed resolution, and groups Requests only by accepted duplicate/canonical relation or identical stable source identity while preserving all identities, exact texts, occurrence count, and current resolution. Similarity and open proposals never deduplicate. | Deterministic 24-hour boundary and zero-memory text/JSON snapshots; self-contained-field and prohibited-content scans; counts/pointer authorization tests; seven-civil-date/DST fixtures; Project grouping, typed-resolution, accepted-relation/source-identity dedupe and similarity/open-proposal non-dedupe matrix |
| <a id="ac-pm-05"></a>AC-PM-05 | Exactly one `ctower.beat.project-management@1` pack at `packs/routines/ctower.beat.project-management/v1.yaml`, registered in `_PACK_PATHS` and `_EXPECTED_ROUTINE_REFS`, fires at the already ordered 07:47 Europe/Vilnius post-dream-cycle cadence and can only read and deliver the recorded artifact. After one live verified delivery, a director-owned migration receipt proves removal of exactly `LAST 24 HOURS — REQUESTS + TICKET UPDATES` (R2952), `TODAY'S PRIORITY STACK` (R2953), and `PAST 7 DAYS — DEDUPLICATED REQUESTS + TICKETS WITH RESOLUTIONS BY PROJECT` (R2951) from the interim path. All three are windows of the same ctower projection and ride the same beat; unrelated open-decision, execution, proof-close, and dream-analysis content remains director-owned. No second schedule or report authority exists. | Strict Routine pack/ref exact-count and summer/winter next-fire tests; prompt/effect custody and cannot-confirm/dispatch negatives; one live artifact delivery ID; before/after director section inventory and signed switch/retirement receipt; exact schedule/route/authority inventory |
| <a id="ac-pm-06"></a>AC-PM-06 | Missing, conflicting, stale, unauthorized, or unread week-goal, Request, Ticket, proposal, duplicate-relation, resolution, watermark, or plan facts preserve every visible fact, render the affected source and artifact `partial|unknown` with exact scope/reason, and make that artifact unconfirmable and its subjects ineligible for dispatch. Bounded output names every omitted scope and points to the complete typed view. Silence never becomes an empty list, guessed rank, guessed duplicate, guessed resolution, or success. | Complete/empty/partial/unknown cross-source truth table; conflict and authorization fixtures; bounded-output snapshot; confirm/dispatch refusal and zero-diff proof; no-false-empty/no-guessed-dedupe assertions |
| <a id="ac-pm-07"></a>AC-PM-07 | The first build candidate creates the outside-reader home `docs/concepts/project-management.md` and links it from `mkdocs.yml`; every later project-management build PR updates that page plus every affected CLI, API, Request, Ticket, digest, Routine, and shipped-coverage reference in the same candidate. Documentation names implemented versus planned behavior and passes documentation review checklist item 11; a generated inventory, code comment, follow-up ticket, or later docs PR cannot satisfy the gate. | Same-head docs-surface inventory and outside-reader walkthrough; `mkdocs` strict build/link check; implemented/planned truth comparison; checklist-item-11 verdict; omitted-surface and follow-up-docs refusal fixtures |
### Dream-cycle session mining

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-mine-01"></a>AC-MINE-01 | Claude `~/.claude/projects` JSONL, Codex rollout, and Hermes log readers each pass one shared extractor conformance suite through a small public Interface and emit the same strict `SessionDigest` schema. Every digest identifies its harness, extractor revision, source ordering and opaque evidence pointers, and contains the fixed decisions-and-reasoning, flaws/dead-ends, fleet-known rediscoveries, procedure-knowledge gaps, and documentation-gaps rubric. Under [AC-ARCH-01](#ac-arch-01), harness-specific filesystem or parser types do not cross the runner Interface or enter kernel imports. | Three real-reader conformance runs over positive/empty/multiline/unknown-field fixtures; strict schema and import-boundary inventory; rubric and opaque-pointer snapshots |
| <a id="ac-mine-02"></a>AC-MINE-02 | REDUCE clusters every accepted digest in one bounded nightly batch into strict Pattern Cards with stable pattern identity, scope, occurrence count, and evidence pointers. The append-only pattern ledger preserves first-seen, last-seen, lifetime count, every per-night count and source watermark, superseded/rejected clustering, and linked skill/document cures across rebuild and restart. A post-cure zero is appended only when the relevant night is complete; partial or unknown coverage cannot lower a count or claim zero. | Real-PostgreSQL two-night/restart/rebuild trace; stable-cluster and evidence-pointer matrix; linked-cure positive followed by complete-zero and partial-unknown counter-fixtures; UPDATE/DELETE refusals |
| <a id="ac-mine-03"></a>AC-MINE-03 | One accepted cursor per harness binds source ordering, extractor revision, Routine occurrence, and either the accepted sanitized digest or one typed parse skip. Each occurrence reads only the unprocessed suffix; restart, replay, duplicate discovery, and equal source timestamps emit no duplicate digest or Pattern Card count. A bounded overflow advances through the accepted prefix and leaves the visible deferred suffix eligible for the next occurrence, so unchanged history causes zero source rereads and zero MAP cost. | Per-harness first/incremental/restart/replay/equal-time cursor matrix; 1/2/many-session source-read counters; accepted-prefix/deferred-suffix trace; no-duplicate ledger proof |
| <a id="ac-mine-04"></a>AC-MINE-04 | Under [INV-32](#non-negotiable-invariants), before any MAP model or process submission, the local reader deterministically redacts credential material from parsed transcript content. Digests contain paraphrase plus opaque typed pointers, never raw credential text, raw transcript excerpts, or local paths. Credential canaries and secret-like negatives appear in no MAP request, digest, reducer batch, Pattern Card, ledger row, DREAM input, ordinary evidence artifact, log, error, or telemetry; redaction uncertainty records a typed skip with zero downstream submission. | Per-harness credential corpus and false-positive counter-fixtures; intercepted MAP/REDUCE/DREAM request bodies; durable-row/object/log/telemetry canary scan; uncertain-redaction zero-call assertion; independent CSO exact-head verdict |
| <a id="ac-mine-05"></a>AC-MINE-05 | Every `SessionDigest` is at most 500 tokens under the pinned tokenizer revision. One REDUCE invocation receives all and only the accepted digests in its batch and at most 30,000 digest tokens; it receives no raw transcript fields. A next digest that would exceed the bound and every later source item are reported as one deterministic deferred suffix with count, first opaque pointer, and unchanged eligibility for the next occurrence, never truncated, dropped, summarized twice, or sent through an unbounded context. | Pinned-tokenizer 499/500/501 and 29,999/30,000/30,001 boundary vectors; reducer request capture; deterministic overflow/restart trace; raw-field absence schema check |
| <a id="ac-mine-06"></a>AC-MINE-06 | DREAM receives only strict Pattern Cards from the accepted reducer receipt plus separately authorized Request/Ticket projections at named watermarks. Its input contains no raw transcript, session message, local path, Session Digest, or credential value, and a missing/partial/unknown reducer or projection source remains named in the input and output. Under [INV-01](#non-negotiable-invariants), DREAM may produce judgment and maintenance proposals only; it cannot mutate Request, Ticket, pattern-ledger, or source-cursor authority. | DREAM request schema and intercepted payload; raw-transcript/digest/path absence scan; source-completeness matrix; Request/Ticket/ledger/cursor before-after digests; proposal-only authorization negatives |
| <a id="ac-mine-07"></a>AC-MINE-07 | A malformed, unreadable, unsupported, missing-root, or redaction-uncertain session produces a typed per-harness skip carrying only opaque source pointer, extractor revision, reason, and count, and makes the night `partial`; no digest, Pattern Card, complete-empty claim, or guessed rubric value is emitted. Corrected parsing uses a versioned explicit reprocess that appends a later result without rewriting the failure or double-counting the session. | Five-reason all-harness failure matrix; skip-and-count API/CLI snapshots; zero-model-call and zero-guessed-row assertions; versioned reprocess append trace; complete-empty versus unavailable-root comparison |

### Ticket movement feed

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-move-01"></a>AC-MOVE-01 | Each accepted Ticket stage transition atomically emits exactly one canonical append-only `workflow.changed` transition event carrying stable Ticket identity, from/to stage, the existing Actor envelope, one stable transition-evaluation/evidence-manifest pointer, server time, and record position. Exact replay emits no duplicate, and a pending or refused transition emits no movement fact. The event is linked to the Ticket and contains only transition-specific facts, never an embedded exhaustive Ticket snapshot. | Real-PostgreSQL transition/event atomicity query; strict payload round-trip; accepted/pending/refused/exact-replay matrix; Ticket-link and no-exhaustive-snapshot schema/storage inspection |
| <a id="ac-move-02"></a>AC-MOVE-02 | One project-scoped authorization-preserving cursor read backs the generated API and protected CLI movement views. It derives only from canonical project events, orders and pages by record position under the existing project-feed contract, rebuilds byte-equivalently, and permits neither projection writes nor a second movement event kind or ledger. | Generated API/CLI equality and authorization matrix; 0/1/limit/limit+1 cursor fixtures; rebuild byte comparison; event-kind, persistence, and projection-privilege inventory |
| <a id="ac-move-03"></a>AC-MOVE-03 | For the prior Europe/Vilnius civil day, the digest carries deterministic movement counts grouped by Project and exact from/to stage plus an authorization-preserving pointer and source watermark for the exact view. It embeds no movement event row, Ticket identity, or Ticket text. | Summer/winter/DST civil-day boundary matrix; deterministic grouping snapshot; pointer authorization tests; digest event-row/identity/text absence scan |
| <a id="ac-move-04"></a>AC-MOVE-04 | A real Atom 1.0 endpoint returns `application/atom+xml` and validator-clean stable feed/entry IDs, server timestamps, authorized links, and deterministic pagination from the same cursor stream. It accepts only the existing direct-API Bearer/project-seat credential machinery resolved through the existing Actor and Project authorization checks; anonymous access, query-string credentials, feed tokens, new principals, cross-project entries, and Ticket-text payloads are absent. | Real authenticated Atom transcript and independent validator result; API/Atom cursor equality; Actor/Project/cross-project authorization matrix; route/principal/credential inventory; query-secret and Ticket-text leak scans; independent CSO exact-head verdict |
| <a id="ac-move-05"></a>AC-MOVE-05 | The future CT-I2 browser movement view consumes the same generated read contextually within the standing five-surface product boundary, never as a sixth primary route. API, CLI, digest pointer, Atom, and later browser fixtures name the same source watermark and agree on every authorized movement represented there. | Same-watermark cross-surface conformance fixture; generated-client-only browser import inventory; exactly-five-route assertion; cross-project and stale-watermark negatives |

### Routine work items

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-rwi-01"></a>AC-RWI-01 | An accepted Routine fire appends exactly one Inbox work item addressed to the Routine's owning seat, carrying the Routine identity, the window it belongs to, the gate evidence and watermark it stood on, and one exact immutable Knowledge `document_id`; it writes nothing to any session and the candidate retains no session-targeted dispatch path. The complete current closed-world registry is the denominator: exactly 17 references (3 `v1`, 4 `v2`, 5 `v3`, 5 `v4`) are enumerated in the same candidate, every reference is proven on the item path, and exact replay appends no second item. | Real-PostgreSQL fire-to-item trace; strict item contract round-trip; registry-derived 17/17 item-path inventory and target-session absence scan; exact-replay zero-diff |
| <a id="ac-rwi-02"></a>AC-RWI-02 | The item pins the exact immutable Knowledge snapshot it references and embeds no instruction text of its own. A policy change registers a new Knowledge snapshot through the existing append-only register operation and explicitly supersedes the Routine revision to pin the new `document_id` before a future fire; existing items and the old Routine revision remain unchanged. No in-place Knowledge edit, floating alias, or unpinned current-resolution path is allowed. An item whose payload carries embedded specification text is refused. | Immutable register trace with old/new document IDs; explicit Routine-revision supersession and future-fire trace; Knowledge update/delete refusal; embedded-instruction refusal matrix; item payload scan for specification text |
| <a id="ac-rwi-03"></a>AC-RWI-03 | While the previous window's item for a Routine has no accepted owning-seat receipt closure, the next fire is suppressed and appends one typed suppression fact naming the blocking item; Inbox `sent`, `delivered`, and `read` acknowledgements do not consume it. It never silently skips and never appends a second open item. Once the receipt is accepted, the following window fires normally. | Real-PostgreSQL open/read/no-receipt, receipt-closed, and next-window matrix; suppression-fact snapshot; one-open-item invariant; receipt-backed resume trace |
| <a id="ac-rwi-04"></a>AC-RWI-04 | The owning seat closes an item with an accepted receipt referencing the delivered artifact; the closure is the sole consumption transition and an append-only fact carrying the reference and server time. A closure without an artifact reference, a closure by a seat that does not own the item, and a re-closure of a closed item each refuse with zero mutation; Inbox read state alone never closes the item. | Receipt-closure trace; read-without-receipt non-consumption proof; missing-reference, foreign-seat, and double-closure refusal matrix; append-only storage inspection |
| <a id="ac-rwi-05"></a>AC-RWI-05 | For the stable window identity `(tenant_id, routine_revision_digest, scheduled_for)`, a window that ends without an accepted receipt reaches `confirmed-unconsumed` and raises exactly one ordinary typed alarm to the escalation seat bound by immutable scoped `escalation_seat_ref` on the active Routine revision, naming the Routine, window, and item; the ordinary alarm is idempotent and never substitutes for the item. The binding is published/superseded only by the authorized owner of that Routine revision, is never caller-selected, resolves to a non-revoked seat in the same company/project scope, and missing, stale, revoked, or foreign-scope resolution appends `escalation-unresolved` and refuses to claim delivery. A partial or failed item-state read appends a typed `degraded` observation in the same append-only alarm episode rather than a clean no-alarm. A later authoritative read resolves that episode to either the one ordinary alarm for confirmed-unconsumed or `recovered-receipted` with no ordinary alarm. Repeated partial reads, restart replay, and concurrent boundary attempts add no duplicate episode or ordinary alarm; “exactly one” counts ordinary missed-window alarm facts, not degraded/recovery observations. | Boundary matrix for receipt before/on/after boundary and read/no-receipt; immutable scoped escalation binding publication/supersession trace; same-scope live-seat resolution; absent/stale/revoked/foreign binding refusal matrix; degraded→confirmed-unconsumed and degraded→recovered-receipted vectors; sequential, restart, and concurrent attempts (at least two per same stable key) with one ordinary fact; final episode projection |
| <a id="ac-rwi-06"></a>AC-RWI-06 | Every Routine in the complete current closed-world registry—exactly 17 references (3 `v1`, 4 `v2`, 5 `v3`, 5 `v4`)—is explicitly re-pointed to or proven on the item path in this candidate; no registered reference is omitted. All ten active `v3`/`v4` registrations that carry `target_session` are superseded or removed from the active surface. The five Routines migrated under CT-I1-033 additionally each carry catch-parity evidence naming both its host twin's execution and its own item-plus-receipt before the twin is retired. Host-side retirement remains the external custodian's act. | Registry-derived 17/17 re-point/disposition inventory; target-session absence and supersession proof; 17 item-path fire/receipt traces; five host-twin catch-parity reports; host-twin non-interference proof |

### Tracked review records

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-rev-01"></a>AC-REV-01 | The tracked review-record manifest names the bounded source denominator and every normalized `record-*.md` file has exactly one final structured `verdict`, exactly one `head` identity, and exactly one `SIGNED-OFF` block. Multi-subject source evidence is split rather than represented by a plural head, and an unavailable source head is explicit rather than guessed. | Manifest source/record counts; repository structural test; one-file/one-verdict/one-head inventory; unavailable-head negative fixture |
| <a id="ac-rev-02"></a>AC-REV-02 | The record corpus contains normalized evidence only: no raw coordination transcript, URL, credential, bearer material, connection string, personal contact, or private project identity is copied. The intended-tree secret scan runs over the tracked corpus, and a detector-shaped canary proves the scan would fail before the clean result is reported. | Redaction scan with credential-shaped negative fixture outside the committed corpus; `secrets-intended-tree` corpus accounting; clean scan output; forbidden-content inventory |
| <a id="ac-rev-03"></a>AC-REV-03 | Review lanes perform no product write or gate mutation. The Commander lands a normalized record as the final commit before merge, and a record-only commit is mechanically proven by `git diff --name-only <sha>^ <sha>` to contain only `docs/internal/review-records/` paths. A record-only change does not invalidate a prior product gate because it changes no judged code. | Read-only source/record inventory; exact record-only commit path output; prior-head versus record-commit diff; no product-path mutation proof |
| <a id="ac-rev-04"></a>AC-REV-04 | The candidate rescues exactly the named CSO/security source scope and no wider coordination estate. It adds no product route, principal, runtime authority, environment variable, feature flag, secret value, public navigation entry, or gate waiver; a changed judged head requires a fresh record rather than an append to an existing file. | 31-source scope manifest; excluded-corpus disposition; no-new-surface inventory; exact-head filename and duplicate-incarnation negatives; gate configuration diff |

### Typed stall clocks

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-stall-01"></a>AC-STALL-01 | One append-only wait episode records exactly one typed reason (`review`, `operator-decision`, `external`, or `blocked-by-ticket`), server-committed `waiting_since`, owner, and reason-specific stable source pointer; a `blocked-by-ticket` wait names the blocking Ticket. Recheck and resolution are later facts, never an overwritten clock. The episode is linked to its Ticket and contains only wait-specific facts, never an embedded exhaustive Ticket snapshot. | Strict reason/pointer contract and real-PostgreSQL append trace; UPDATE/DELETE refusal; recheck/resolution history query; blocking-Ticket link fixture; Ticket-link and no-exhaustive-snapshot schema/storage inspection |
| <a id="ac-stall-02"></a>AC-STALL-02 | The stall floors are review at 24 hours; operator decision at 48 hours, after which the next beat regenerates the complete record-derived D50 brief; external at seven days, after which a typed recheck is due; and `blocked-by-ticket` at 24 hours without a blocking-Ticket transition, with immediate event-driven reevaluation when that Ticket moves. The internal-dependency floor is one full unattended workday because it is objectively recheckable from the movement feed. Clock evaluation creates no Ticket transition. | 23:59/24:00, 47:59/48:00, and 6d23:59/7d boundary vectors; blocking-Ticket no-movement/movement trace; complete brief snapshot; Ticket stream zero-diff assertion |
| <a id="ac-stall-03"></a>AC-STALL-03 | Each breached current episode appears once, batched into the owning Commander's next existing beat and the D60 project-management digest aging section, with stable Ticket/Project, reason, age, threshold, owner, recheck state, and next action. Repeated evaluations at identical source facts deduplicate; no breach creates an immediate page, new Routine, cadence, or page storm, and ordinary waiting alone derives neither Attention nor `human-waiting`. | Beat/digest snapshots and source-watermark equality; repeated-evaluation idempotency matrix; Routine/schedule/outbox and Attention-state zero-diff inventories; 0/1/many breached-episode batching fixture |
| <a id="ac-stall-04"></a>AC-STALL-04 | Missing, conflicting, stale, unreadable, unauthorized, or gapped reason, start, owner, dependency, movement, threshold, recheck, or watermark input is typed `unknown`, names every affected source and reason, and is surfaced alongside breached episodes. It never folds to non-breach, empty, calm, complete, or an invented timestamp. | Complete/breached/partial/unknown cross-source truth table; conflict/gap/authorization fixtures; no-false-calm and no-invented-time assertions; API/CLI/beat/digest snapshots |

### Commander beat worklist

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-beat-01"></a>AC-BEAT-01 | One generated authenticated read resolves the existing Commander Actor/custodian and returns all current owned nonterminal Tickets grouped by pinned workflow stage, every breached or unknown stall clock, and movement after the last accepted beat watermark, with per-source completeness and one current source watermark. | Commander/custodian authorization matrix; stage-group and ownership fixtures; complete/empty/partial/unknown contract snapshots; generated API/protected CLI parity |
| <a id="ac-beat-02"></a>AC-BEAT-02 | The existing D52 Mission Control sibling consumer starts every applicable Commander beat by calling the worklist through the generated authenticated control-plane client before DIRECTOR injection. It holds no Record-tier credential, imports no kernel persistence, and records the returned watermark in its existing append-only claim/delivery/replay ledger; this does not claim the separately authorized server-side consumption/lane-binding ceremony. | Real consumer preflight and DIRECTOR transcript; generated-client call capture; import/credential/process inventory; claim/delivery/replay ledger query; server-side lane-binding absence assertion |
| <a id="ac-beat-03"></a>AC-BEAT-03 | The current owned-Ticket set is re-derived in full on every beat; the prior watermark limits only the movement delta and never membership. Context compaction, restart, prompt truncation, model or harness replacement, and a missing local cursor therefore cannot drop a driven Ticket. | Full-set-before/after-beat matrix with quiet Tickets; watermark delta boundary fixtures; compaction/restart/truncation/model/harness substitution transcript; no-drop stable-identity assertion |
| <a id="ac-beat-04"></a>AC-BEAT-04 | An unavailable, stale, gapped, unauthorized, conflicting, partial, or unknown worklist produces a typed failed/partial/unknown beat preflight and never a contextless clean injection, empty-work claim, advanced watermark, or silent suppression. Recovery retries from durable ctower state and the external append-only delivery ledger. Existing Routine schedules are reused unchanged and no cadence is added. | Failure/recovery truth table; zero-injection/zero-empty-claim/unchanged-watermark assertions; durable retry and replay trace; Routine pack/ref/schedule exact-count inventory |

### Tool and knowledge catalog engine

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-cat-01"></a>AC-CAT-01 | The public repository contains only the catalog engine, authored contracts, explicitly public first-party packs, synthetic examples, and generic fixtures. Actual tenant tool, skill, persona, instruction, credential, and inventory content remains in private per-company storage. A repository-admission fixture that injects one artifact explicitly marked as tenant content fails by the stable name tenant-catalog-content-in-public-repository; generic secret, generated-file, or path checks are additional evidence and cannot substitute for this result. | Intended-tree content-class inventory; injected tenant-artifact negative naming the exact refusal; public-pack allowlist; secret/generated/path scans; independent CSO exact-head verdict |
| <a id="ac-cat-02"></a>AC-CAT-02 | Tool, skill, persona, Goal, and Project resources use the existing VersionedComponent Catalog Interface, CompanyBundle validate/plan/apply/export path, and one active bundle. COMPANY is the existing tenant scope with project=null: one exact entry is available across that tenant's authorized Project contexts. PROJECT carries one exact Project key and never crosses it. Existing Goal-to-Project and component-assignment references are preserved. A second Catalog, bundle, active pointer, scope store, revision service, permission service, or per-kind engine is absent. | COMPANY/PROJECT schema and authorization matrix; cross-tenant/Project negatives; CompanyBundle plan/apply/export round trip; Goal/Project exact-reference fixtures; interface, persistence, pointer, and dependency inventories |
| <a id="ac-cat-03"></a>AC-CAT-03 | Publication starts or appends one immutable revision lineage. A changed normative contract publishes a new authored schema version; changed content appends a new revision and digest; and supersession, deprecation, revocation, rollback, or active-pointer movement affects future resolution only. Every index row, read, materialization, invocation, and result identifies exact kind, key, revision, and content digest. Ambiguous, missing, incompatible, revoked, or digest-mismatched resolution refuses by name and cannot rewrite a prior pin or running result. | v1/v2 schema-version discipline fixture; append-only lifecycle and exact-byte resolver trace; active-pointer-before/after running-consumer matrix; index/read/materialization/invocation/result pin snapshots; named-resolution refusal vectors |
| <a id="ac-cat-04"></a>AC-CAT-04 | Operator-only CompanyBundle apply is the only publication path for COMPANY and PROJECT entries; a Commander may author or propose but cannot apply. The operator reads both scopes. Existing OIDC human commander and viewer bindings read COMPANY entries only in an authorized Project context and PROJECT entries only for a bound Project, with viewer read-only. Machine project-seat credentials have no browse or Catalog-admin surface, and a current runner job receives only its exact authorized pin. Invocation is still the intersection of existing caller command authority, Project, assignment, policy, and current job; catalog visibility grants no execution. No new principal, authority store, bearer format, or free-standing Catalog scope exists. | Operator/commander/viewer/project-seat/runner publish-read-invoke matrix; cross-tenant/Project and viewer-write negatives; exact-pin job manifest; principal/scope/route/store inventory; visibility-without-invocation proof; independent CSO verdict |
| <a id="ac-cat-05"></a>AC-CAT-05 | A typed invocation binds Actor, tenant and Project, exact entry revision and digest, named operation, strict input schema and digest, idempotency identity, policy, job, lease, fencing context, and typed output or refusal. The control plane authorizes and records; only runner-side code materializes and executes through the existing Runtime and CommandGuard boundary and submits a strict result through generated authenticated APIs. Runner exit alone cannot advance Workflow, satisfy Evidence, or grant an effect. Runner, provider, web, CLI, and generated clients have no Record-tier import, client, credential, or connection. | Strict invocation/result contract round trip; authorization/idempotency/lease/fence/guard refusal matrix; real runner transcript; Workflow/Evidence/effect zero-diff assertions; process/import/credential/connection inventory; generated-client-only submission capture |
| <a id="ac-cat-06"></a>AC-CAT-06 | Each private catalog source and scope has one accepted monotonic cursor binding source identity and ordering, source watermark, source-reader and mapping revisions, Routine or manual occurrence, bounded accepted prefix, and for every discovered item one strict staged entry or typed skip. A visible deferred suffix remains eligible. Restart, replay, equal timestamps, and duplicate discovery append no duplicate component revision. Missing, partial, stale, unreadable, unauthorized, conflicting, or gapped source truth remains named, cannot claim complete-empty coverage, cannot skip an unaccounted suffix, and cannot move the active pointer. External payloads are strict and contain secret references only, never secret values or untyped provider objects. | Cursor/watermark schema and accepted-prefix trace; restart/replay/equal-time/duplicate idempotency matrix; deferred-suffix and completeness truth table; active-pointer zero-diff failures; intercepted strict payload and secret-value/untyped-object absence scans; independent CSO sync verdict |
| <a id="ac-cat-07"></a>AC-CAT-07 | One catalog directory import path maps a tool/skill/persona directory tree through an explicit versioned schema map into exact scope, kind, key, schema, revision, digest, and provenance. Dry-run performs zero mutation and emits a canonical per-item create/reuse/supersede/refuse report, totals, and source watermark; apply uses the ordinary staged Catalog and sync path. Identical re-import is zero-revision idempotent reuse, changed content proposes one new revision, and ambiguous, missing, unsupported, or conflicting mappings refuse without partial publication. Migration of any real tenant inventory, preservation of source-specific quirks, and publication of tenant names, counts, or content are not exercised. | Versioned directory-tree/schema-map fixtures; byte-stable dry-run and Catalog zero-diff; first-import/identical-reimport/changed-content traces; ambiguous/missing/unsupported/conflict atomic-refusal matrix; no-bypass dependency inventory; public-evidence tenant-name/count/content absence scan |
| <a id="ac-cat-08"></a>AC-CAT-08 | The same build creates docs/concepts/catalog.md as the public concept home, adds one catalog first-use path to docs/quickstart.md, creates docs/reference/catalog.md for schemas, scopes, permissions, synchronization, invocation, refusals, and import, and links all three from mkdocs.yml. Every affected API, CLI, runner, and security reference is updated in that candidate. These public pages contain zero internal request numbers, ticket IDs, crew or seat labels, internal operational jargon, private repository paths or links, or private inventory facts. A same-head structural scan and an outside-developer concepts -> quickstart -> reference walkthrough block review when any page, navigation link, command, schema, permission, refusal, or boundary is missing. | MkDocs strict build and navigation/link check; public-doc forbidden-reference scan; API/CLI/runner/security documentation inventory; fresh outside-reader walkthrough transcript; docs-surface completeness manifest and same-head reviewer verdict |

### Knowledge catalog discovery

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-kcat-01"></a>AC-KCAT-01 | One generated authenticated `knowledge search` / `searchKnowledgeCatalog` read accepts one nonempty normalized query, one exact `org` or `project` scope, and bounded cursor/limit values, then returns zero or more matches from exactly the existing Knowledge-document, accepted-Ruling, and D63 Catalog `skill` sources at one named source watermark. Matching and ordering are deterministic, and each result carries a bounded source-derived title or excerpt plus its source kind, stable source identity, scope, recorded time, and typed currentness. Operator outcome: the operator and an agent can ask one supported place what the company knows about X without knowing a filesystem path. | Generated API and protected CLI equality transcript over all three source classes; empty/invalid/boundary query vectors; deterministic ranking and cursor replay; one-watermark result snapshot |
| <a id="ac-kcat-02"></a>AC-KCAT-02 | Every match points back to its authoritative source without copying authority: a Knowledge document carries its document ID and `source_ref` or an explicit absent value; a Ruling carries its Ruling ID, accepted request/source reference when present, and verbatim digest; a Catalog skill carries exact kind, key, revision, content digest, schema, and provenance. `recorded_at` is the source fact's committed time, not caller text or query time, and resolving a result uses the existing authorized source read. Operator outcome: every answer says where it came from and when it entered the system. | Cross-source result-to-source join trace; byte/digest equality against `knowledge_documents`, accepted Rulings, and exact Catalog revisions; caller-timestamp and forged-provenance negatives; source-link readback |
| <a id="ac-kcat-03"></a>AC-KCAT-03 | Currentness is a closed typed state of `current`, `superseded`, `deprecated`, `revoked`, or `unknown`, with a named reason and source pointer. A Catalog skill follows its accepted lifecycle and successor facts; an accepted Ruling with no successor is current and a Ruling with `superseded_by_ruling_id` is superseded; a source-ref Knowledge document is current only when it is the latest unambiguous accepted snapshot for that source identity, while a direct document without source identity, duplicate/conflicting source snapshots, or missing/stale/gapped/unreadable source truth is `unknown`. No result becomes current from absence, silence, or an unobserved external file. Operator outcome: the operator can distinguish current knowledge from a superseded or honestly unknown answer. | Real PostgreSQL current/superseded/deprecated/revoked/unknown matrix; duplicate-source and source-gap fixtures; no-false-current assertions; exact currentness reason/source-pointer snapshots |
| <a id="ac-kcat-04"></a>AC-KCAT-04 | The read reuses existing Actor, tenant, Project, and accepted-source authorization. Organization results are available only in an authorized Project context, Project results never cross the bound Project, and foreign tenant/Project queries return the established no-disclosure refusal without result rows or source pointers. Catalog visibility remains distinct from invocation authority, and no project-seat credential gains Catalog administration or execution authority. Operator outcome: one search is useful without leaking another company's or Project's knowledge. | Operator/commander/viewer/project-seat and cross-tenant/cross-Project matrix; 404/403 no-disclosure snapshots; durable state-diff and invocation-authority zero-diff proof |
| <a id="ac-kcat-05"></a>AC-KCAT-05 | Every response identifies one repeatable source watermark and is derived only from accepted source facts at that watermark. A later Knowledge snapshot, Ruling successor, Catalog revision/lifecycle change, source gap, or authorization change affects only later reads; restart, replay, and rebuild at one watermark reproduce the same semantic result set and currentness, while pending/refused/unaccepted writes never appear. Operator outcome: the answer is tied to a durable point in time instead of an unexplained stale cache. | Repeatable-read watermark transcript; append/revision/supersession and authorization-before/after matrix; restart/replay/rebuild byte-semantic comparison; pending/refused exclusion proof |
| <a id="ac-kcat-06"></a>AC-KCAT-06 | The Knowledge catalog is one derived read over the existing Knowledge, Ruling, Catalog, and Actor authorities. It adds no authoritative body store, duplicate document store, second Catalog, new `ComponentKind`, importer, pointer, principal, grant, credential, provider, or execution path; any disposable search structure contains only rebuildable references/terms and never replaces a source fact. Search and result resolution are reads and append no fact. Operator outcome: the company gets one place to discover knowledge without creating a second place that must be kept in sync. | Persistence/schema/import/process inventory; no-new-authority/component/principal/route execution scan; body-duplication and secret scan; read-only state-diff proof; rebuild-from-sources demonstration |
| <a id="ac-kcat-07"></a>AC-KCAT-07 | The same candidate documents the existing Catalog/Knowledge discovery path through the public concepts, quickstart, reference, generated HTTP, and protected CLI references, with the command and result fields sufficient for an outside developer or agent to query without private paths or tenant inventory facts. Strict documentation/navigation checks and a same-head walkthrough are required; planned behavior is not described as shipped. Operator outcome: agents use the supported catalog query instead of grepping private files or guessing an undocumented path. | `mkdocs build --strict`; navigation/link and public-forbidden-reference scan; generated-client/CLI/API inventory; fresh outside-reader concepts-to-quickstart-to-reference walkthrough |

### First-class workspaces

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-ws-01"></a>AC-WS-01 | One strict append-only workspace record has UUIDv7 identity, immutable `WORKTREE|PROJECT|COMPANY` type, the exact tenant/Project/Ticket binding permitted by that type, normalized start-directory intent, one current immutable mount-set revision, one `CredentialScopeRef` to existing authority, aggregate version, and lifecycle facts. A Ticket workspace-link fact carries only `{workspace_id, workspace_type}` and resolves and validates the referenced identity, type, tenant, Project, and Ticket binding without embedding a workspace snapshot. Invalid type/binding combinations, foreign scope, unknown workspace, and duplicate or mismatched links refuse with zero mutation. | Strict workspace/Ticket-reference contracts; real-PostgreSQL create/link/replay trace; UUID/type/binding truth table; foreign/unknown/duplicate/mismatch zero-diff matrix; no-embedded-snapshot schema/storage inspection |
| <a id="ac-ws-02"></a>AC-WS-02 | An accepted dispatch creates or exactly replays one ephemeral `WORKTREE` bound to exactly one Ticket and its immutable Project. Acceptance of a PR merge fact explicitly linked to that Ticket automatically appends exactly one `workspace.close_requested` fact causally linked to the merge fact and, where the Ticket moves, to D62's stable transition-evaluation pointer. Checkpoint and sole-work protection run before cleanup; the record derives `closing` until an authenticated current-lease/current-fence runner receipt appends `workspace.closed` or a typed failure. Pending or refused dispatch, a foreign or unlinked PR, branch ancestry, projection state, replay, missing checkpoint, runner exit, and cleanup uncertainty cannot create a duplicate or false close. | Real-PostgreSQL dispatch/PR-merge/movement/close-request/runner-receipt trace; exact replay cardinality; pending/refused/foreign/unlinked/ancestry/projection negatives; checkpoint sole-work refusal; closing-before-receipt and cleanup-failure snapshots |
| <a id="ac-ws-03"></a>AC-WS-03 | A `PROJECT` workspace is persistent, bound to exactly one tenant/Project, starts in that Project's normalized checkout context, and carries only exact PROJECT-scope Catalog pins for the same Project. Ticket completion or PR merge never closes it. Only an explicit authorized close request followed by checkpoint-safe current-lease/current-fence runner cleanup closes it; cross-Project Ticket link, mount, credential-scope use, remount, or cleanup refuses without mutation. | PROJECT lifecycle API/CLI/runner transcript; same-Project checkout and mount snapshots; Ticket-close/PR-merge zero-diff proof; explicit-close receipt; cross-Project link/mount/credential/remount/cleanup refusal matrix |
| <a id="ac-ws-04"></a>AC-WS-04 | A `COMPANY` workspace is persistent, bound to one tenant/company and one existing cross-Project authorization context, and carries exact COMPANY-scope Catalog pins with `project=null`. It may serve only Projects already authorized by the referenced existing scope and creates no company principal, portfolio principal, free-standing grant, or workspace authority store. Ticket completion or one Project's PR merge never closes it; only explicit authorized checkpoint-safe close with a current runner receipt does. | COMPANY lifecycle and multi-Project authorized-read transcript; project-null exact-pin snapshots; unauthorized-Project and cross-tenant refusals; principal/grant/store inventory; Ticket/PR zero-diff and explicit-close receipt |
| <a id="ac-ws-05"></a>AC-WS-05 | The control plane owns desired and accepted workspace record facts; the runner alone resolves the host path, creates or checks out bytes, materializes mounts, remounts, checkpoints, and cleans up, and reports strict typed idempotent leased/fenced results through generated authenticated APIs. Runner, provider, web, CLI, and generated clients have no Record-tier import, client, credential, or connection. Process/path existence, disappearance, exit, janitor observation, branch ancestry, provider state, or publication of a workspace record never advances lifecycle by itself or proves that a directory exists. | Generated API/runner conformance trace; desired-versus-observed record query; process/path/janitor/ancestry/provider mutation negatives; lease/fence/idempotency matrix; import/client/credential/connection inventory; no-directory-proof assertion |
| <a id="ac-ws-06"></a>AC-WS-06 | `CredentialScopeRef` is exactly `{authority_kind, authority_id, authority_revision}` referencing one already accepted grant or binding class and resolving through the existing Actor and Project authorization model. Every workspace operation re-intersects it with tenant/Project, Ticket or assignment, policy, current job/lease, and requested mount/action. Records, events, commands, fixtures, documentation, logs, and telemetry contain no credential value, bearer, vault result, new principal, new grant kind, authority store, or free-standing workspace scope; a runner receives only an existing short-lived handle at the authorized boundary. | Strict reference and resolution contracts; Actor/grant/binding authorization matrix; revoke/revision/tenant/Project/job/action negatives; intercepted runner command; credential-canary scan across rows/events/fixtures/docs/logs/telemetry; principal/grant/store inventory; independent CSO exact-head verdict |
| <a id="ac-ws-07"></a>AC-WS-07 | Every workspace mount entry carries D63's exact Catalog scope, Project key or null, kind, key, revision, and content digest. Type/scope rules are enforced and a `WORKTREE` pins the exact mount set authorized at dispatch. Catalog active-pointer movement never changes a live workspace. Remount is one explicit append-only request plus a typed runner result that creates a new immutable mount-set revision; exact replay emits no duplicate, failure leaves the prior accepted pins authoritative, and lifecycle reads and results identify the exact mount-set revision. | Exact-pin mount contract and resolver snapshots for all three types; active-pointer-before/after live-workspace proof; remount request/result/replay trace; failed-remount prior-pin equality; scope/Project/digest mismatch refusals; historical mount-set query |
| <a id="ac-ws-08"></a>AC-WS-08 | The workspace fact fold distinguishes requested, active, remounting, closing, closed, failed, and unknown. Missing, stale, gapped, conflicting, unauthorized, digest-mismatched, unfenced, or unreadable runner, PR, Ticket, Catalog, credential-scope, checkpoint, remount, or cleanup input names every affected source and reason and yields typed partial/unknown or refusal. It never derives active, unchanged, clean, closed, or safe-to-delete from absence or silence; retries resume from durable command/result identity and cannot duplicate a lifecycle fact. | Complete/state-transition/partial/unknown truth table; per-source stale/gap/conflict/auth/digest/fence/unreadable fixtures; no-false-active/unchanged/clean/closed/delete assertions; restart/retry/replay cardinality and recovery trace |
| <a id="ac-ws-09"></a>AC-WS-09 | The same build creates `docs/concepts/workspaces.md` as the public concept home, adds one first-use path to `docs/quickstart.md`, creates `docs/reference/workspaces.md` for record/reference schemas, type/lifecycle tables, mounts, credential-scope references, API/CLI/runner commands, refusals, and recovery, and links all three from `mkdocs.yml`. Every affected Ticket, Catalog, API, CLI, runner, and security reference updates in that candidate. Public pages contain zero internal request, decision, ticket, or acceptance IDs, crew or seat labels, fleet jargon, private paths or links, or private inventory facts. Strict docs build, same-head structural scan, and a fresh outside-developer concepts -> quickstart -> reference walkthrough block landing when any surface or boundary is missing. | MkDocs strict build and navigation/link check; public-doc forbidden-reference scan; Ticket/Catalog/API/CLI/runner/security documentation inventory; fresh outside-reader walkthrough transcript; docs-surface completeness manifest and same-head reviewer verdict |

### External-estate record imports

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-imp-01"></a>AC-IMP-01 | One operator-authority inbox-history import accepts a signed manifest over the frozen external inbox ledger and appends messages with preserved original timestamps, source sender/recipient identities resolved through the accepted seat-mapping plan, and initial read-state facts. Message identity derives deterministically from the source row; re-running any batch or the complete manifest appends zero new records. A non-operator caller, an unsigned or count-mismatched manifest, and an unmapped seat without a source-only disposition all refuse with zero mutation. | Strict manifest contract; real-PostgreSQL import/replay zero-diff trace; timestamp/read-state preservation snapshot; authority and manifest-tamper refusal matrix; source-only mapping fixture |
| <a id="ac-imp-02"></a>AC-IMP-02 | Agreed-decision documents import as byte-exact Rulings carrying original decision timestamps and one source-path provenance reference each; knowledge documents import through the existing knowledge surface with source-ref provenance and the original file timestamp carried as record metadata. Both imports are idempotent by source path identity and neither rewrites, reorders, or re-dates an existing record. | Byte-equality comparison per imported Ruling; provenance/timestamp field inspection; double-run zero-diff trace; existing-record immutability probe |
| <a id="ac-imp-03"></a>AC-IMP-03 | One append-only company-record aggregate stores typed accountability rows (escape ledger first) under tenant scope with a declared natural key per record type; import is operator-authority, manifest-bound, idempotent by natural key, and the aggregate accepts no credential material, production customer data, or other prohibited data class. | Strict company-record contract; real-PostgreSQL import/replay trace; natural-key dedup matrix; prohibited-class scan; tenant-scope refusal matrix |
| <a id="ac-imp-04"></a>AC-IMP-04 | Every import tier emits one typed parity report naming source row count, imported record count, per-batch digests, and sampled content hashes, and the report exists BEFORE any source-path closure act is taken by the external custodian. Unknown source owners appear in the report under an explicit source-only state; no import mints a principal, seat, or grant. | Parity-report contract and content check per tier; source-count equality assertion; source-only enumeration fixture; principal/seat/grant zero-diff inventory |

### Activity-gated Routines

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-rtn-01"></a>AC-RTN-01 | One strict append-only Routine registration record carries UUIDv7 identity, company or project scope, one owning seat, one schedule, and exactly one typed activity gate drawn from a closed gate set; registration, amendment-by-supersession, and retirement are recorded facts. An unknown gate type, foreign scope, or unowned registration refuses with zero mutation. | Strict Routine contract; real-PostgreSQL registration/supersession/retirement trace; closed-gate-set truth table; scope and ownership refusal matrix |
| <a id="ac-rtn-02"></a>AC-RTN-02 | Evaluation fires a Routine only when its schedule elapses AND its typed gate answers true against canonical ctower reads; each fire appends exactly one dispatch fact with the gate evidence and watermark it stood on; a false gate appends a typed skip fact, never silence; exact replay of an evaluation appends nothing. Failed or partial gate reads produce a typed degraded fact and never a clean fire. | Gate-true/gate-false/degraded matrix over real reads; dispatch/skip/degraded fact snapshots; exact-replay zero-diff; watermark presence assertion |
| <a id="ac-rtn-03"></a>AC-RTN-03 | Five representative external schedules — including both operator-facing report rails, one watcher, one janitor, and one sentinel — run end-to-end as registered Routines with catch-parity evidence: each host twin remains active until its Routine has fired once with recorded evidence, and the parity report names both executions. Host-side deletion is the external custodian's act and never a ctower effect. | Five registration fixtures; real fire evidence per Routine; catch-parity report; host-twin non-interference inventory |
| <a id="ac-rtn-04"></a>AC-RTN-04 | The gate set stays closed and typed with no general expression language, no product UI, no new principal, and no Routine-initiated host command execution in this candidate; every unmigrated external schedule is enumerated with its intended gate in the admitted backlog. | Gate-set closure inventory; route/principal/UI zero-diff; execution-path absence probe; backlog enumeration completeness check |

### Spawn-custody records

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-spwn-01"></a>AC-SPWN-01 | One strict append-only spawn record carries UUIDv7 identity, tenant/project scope, seat and crew references, one task reference, a worktree path intent, a substrate profile pin, and an optional workspace reference that stays null until first-class workspaces exist; lifecycle moves are appended transition facts through requested, accepted, running, and one terminal state, with no in-place mutation and no transition skipping. Payloads carry no credential value or other prohibited data class. | Strict spawn-record and transition contracts; real-PostgreSQL lifecycle trace; illegal-transition and mutation refusal matrix; prohibited-class scan |
| <a id="ac-spwn-02"></a>AC-SPWN-02 | The spawn driver records the spawn in ctower BEFORE creating any host session; when ctower is unreachable the driver appends the identical fact to one mode-0600 local spool, surfaces the pending-durability state explicitly, replays the spool to ctower on recovery, and removes only acknowledged entries. A spawn is never silently unrecorded. | Record-before-drive ordering proof; unreachable-path spool trace with permission check; replay/ACK/removal trace; surfaced-pending assertion |
| <a id="ac-spwn-03"></a>AC-SPWN-03 | External spawn-history rows import by source identity with latest-status-effective semantics and a parity report per AC-IMP-04 discipline; the set of never-terminated source rows forms the initial running set and nothing else does. | History import/replay zero-diff; latest-status truth table; initial-running-set derivation fixture; parity report check |
| <a id="ac-spwn-04"></a>AC-SPWN-04 | Fleet reconcile continues to read the external registry twin until one recorded parity proof shows ctower-derived reconcile input equal to the twin over an agreed window; before that recorded proof no host session lifecycle decision derives from ctower spawn reads. The read-path swap is a separately admitted act. | Parity-proof record and equality fixture; reconcile-input source inventory; pre-proof decision-path probe |
...
### Crew and role records

| <a id="ac-role-01"></a>AC-ROLE-01 | For an active project-seat grant, the operator-authored CompanyBundle assignment contains exact `seat_key` and `role_key` fields beside its assignment subject, slot, and component reference, written by `company bundle apply`; the grant resolves exactly one role fact only when `project_seats.seat_key` equals that authored seat key. The fact carries the assignment subject as role identity plus exact role key, tenant/project scope, and pinned bundle, assignment/component, and seat-catalog revisions/digests. Neither key is accepted from a command or inferred from a principal ID, display name, model, harness, profile, crew name, seat spelling, stage name, or silence. Missing, ambiguous, revoked, expired, stale, or foreign-scope resolution is `role-resolution-unavailable` with zero mutation; no second store, principal class, grant, credential, or bearer format is introduced. | RED-first authored-binding census; authenticated project-seat/CompanyBundle join matrix; current-role and missing/ambiguous/revoked/expired/stale/foreign negatives; revision-pinned rebuild proof; no-new-store/principal/grant/bearer inventory |
| <a id="ac-role-02"></a>AC-ROLE-02 | Every role-bearing stage transition and stage attempt receives a role fact from the acting Actor's authenticated authority plane before policy/predicate success is accepted. A transition checks the destination stage being entered; an attempt checks its current stage. The Workflow run and transition evaluation pin the role/participant digest, exact authority plane, role key, subject or human binding, scope, seat when present, and source revisions/digests used, and never replace them with caller text or a later authority revision. Project-seat and human-binding facts remain disjoint. A missing or unknown fact never becomes an empty member set or permissive default. | Accepted project-seat and human-binding traces; transition/attempt destination/current-stage snapshots; restart/replay/reassignment traces; authority-revision invalidation matrix; caller-supplied-role and empty-default negatives |
| <a id="ac-role-03"></a>AC-ROLE-03 | If a Workflow stage declares `role_keys`, the exact typed role fact must be a member and every existing edge, assignment, evidence, gate, scope, and predicate condition must also pass. After version/run checks, an undeclared edge wins over membership; for a declared edge, role resolution precedes membership and membership precedes predicate evaluation. A non-member is refused before authoritative transition mutation with `workflow-stage-role-not-permitted`, carrying the checked stage, source/destination when applicable, declared/effective role sets, actor plane, resolved role or null, and one evaluation. The combined matrix proves undeclared plus non-member returns `workflow-transition-not-declared`, predicate-unsatisfied plus non-member returns `workflow-stage-role-not-permitted`, and permitted member plus unsatisfied predicate returns `workflow-predicate-unsatisfied`; the refusal changes no stage/job/gate/Evidence/delivery fact and cannot be bypassed by caller role, profile, model, harness, crew label, or policy-only decoration. | RED-first combined-failure matrix; real-PostgreSQL permitted/non-member/role-unavailable transition and stage-attempt pairs; exact refusal payload and zero-diff query; transition-evaluation audit; existing transition/predicate refusal parity |
| <a id="ac-role-04"></a>AC-ROLE-04 | The Workflow stage `role_keys` field is optional, but when present is a nonempty unique list of exact `{authority_plane, key}` references; absent is role-neutral and empty is invalid, never a wildcard. Execution Policy `participant_resolution` is an optional unique list of `{stage_key, role_keys}` entries. Each entry names a declared stage and a nonempty unique subset of that stage's authored base set; the effective set preserves base order after filtering. A policy may remove eligible roles but may not add a role, stage, edge, capability, or participant, duplicate or ambiguously map a stage, target a role-neutral stage, or create an empty/unknown effective set. Unknown, duplicate, widened, or otherwise non-narrowing mappings refuse before publication or dispatch; absent resolution leaves a role-bearing base set unchanged. The Workflow base set, policy ref/digest, and effective set are pinned in the participant digest. | RED-first stage/policy schema vectors; source-schema and normalization/round-trip checks; stage-role subset and unknown/duplicate/widening/empty/role-neutral matrix; publication/dispatch zero-mutation proof; cross-package arbitrary-role-key fixture; `participant_resolution` consumer trace |
...
| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-fleet-01"></a>AC-FLEET-01 | The tower persists a versioned seat-and-crew registry naming, per seat: project, substrate identity, and exactly one respawn owner. Registry and respawn-policy revisions are immutable Catalog components. A second claimed owner for a seat is refused by name. No persistent artifact anywhere — registry row, respawn-policy revision, Catalog component, executor configuration, or any other stored payload — carries a session-resume identity; a candidate revision containing one is refused by name at publication, and resume identity is discovered at respawn time from seat transcript state as the only source. | Registry/policy contract tests; duplicate-owner refusal vectors; stored-resume publication-refusal vectors across every persistent payload kind; revision-custody proof |
| <a id="ac-fleet-02"></a>AC-FLEET-02 | Per-seat liveness state (alive/working/idle/dead) derives only from typed probe and event ingest with last-evidence timestamps. Account-cap status is a separate orthogonal typed fact — never a liveness state — carried with its own evidence timestamps and folded beside liveness in the operator answer. Pane-rendered text is not a source for liveness or cap detection. Stale or absent evidence yields typed `unknown` with its age, never inferred health. | Liveness projection truth table over the four states; orthogonal cap-fact fixtures; stale-evidence and absent-probe fixtures; the pane-render misclassification regression fixture |
| <a id="ac-fleet-03"></a>AC-FLEET-03 | Every observed seat or crew death, respawn claim, respawn result, cap event, and drill outcome appends one durable typed event with cause and claimant identity. Respawn claims serialize under one spawn-lock protocol fact; a claimant that is not the registered owner records stand-down and spawns nothing. | Ledger append/replay tests; dueling-claimant same-second race fixture with exactly one spawn and one stand-down row |
| <a id="ac-fleet-04"></a>AC-FLEET-04 | One deterministic read — generated API plus protected CLI rendering — folds registry, liveness, and ledger into the operator answer "is everything moving, anything capped or blocked" with per-seat rows and typed partial/unknown epistemics. Local executors consume respawn policy through the generated client only; no executor holds Record-tier access, and browser Fleet-surface deepening remains CT-I2-005-gated with no new public ingress and no sixth surface. | Read-model matrix over registry×liveness×ledger states; CLI transcript; executor import/egress boundary checks; surface and listener inventory |
| <a id="ac-fleet-05"></a>AC-FLEET-05 | The kill-drill (substrate server killed; registered owners rebuild every seat) and the reboot-drill (host boots; boot-enabled tower services come up; fleet bootstraps; all registered seats respawn including commander; routines fire) run as tower-run workflow tickets with frozen criteria, proven on an isolated substrate with synthetic registry before any live drill, and the event ledger records each full recovery as rows. The reboot-drill proves SESSION CONTINUITY, not merely discovery: the identity discovered at respawn resumes the same pre-reboot seat session, evidenced by pre/post session-identity equality per seat — a fresh session with a discovered identity fails the drill. Prerequisites: the credential artifact a rebuilt seat will actually read — the live ambient credential file, not the state-level account marker — is complete and real-call-verified before EITHER drill runs, and a drill started otherwise is refused. The two are distinct authorities and a drill gate that binds the marker binds the wrong one: a marker may name an account whose pooled copy is incomplete while the live file a new process inherits is sound, which is why seats rebuilt under exactly that condition recovered normally. The rotation path is the hazard the gate exists for, since rotation installs a pooled copy as the live file; a drill that could trigger or race rotation is refused unless the target pooled copy is complete and real-call-verified too. Additionally, every registered dynamic-crew respawn recipe persists in a reboot-durable location before registration; a recipe under a volatile path (such as `/tmp`) is refused at registration, and the reboot-drill is blocked until this holds. A drill success flag alone cannot resolve. | Isolated-substrate kill and reboot drill transcripts; ledger recovery rows; boot-enablement inventory; per-seat pre/post session-identity equality proof; volatile-path recipe registration-refusal vector; incomplete-live-credential drill-refusal vector; marker-versus-live-file distinction proof; rotation-target completeness vector; flag-only resolution refusal |

### Harness capacity and credentials

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-cap-01"></a>AC-CAP-01 | Capacity is measured on the exact credential artifact the seat authenticates with — never on a copy, marker, or sibling record standing in for it — and a measurement whose target cannot be proven to be that artifact is typed `unproven`, never `alive` and never `capped`. Attribution binds the same way: the (harness, account, model) triple a result is published under derives from the measured artifact's own proven identity — never from a separate marker, pointer, or sibling record — and a result whose measured identity cannot be bound to its published triple is itself `unproven`. Two independent probe generations read this system wrong in OPPOSITE directions (one inferring alive from a usage endpoint, one calling real models against pooled copies the seats do not use), so a capacity reading carries the identity of the artifact it measured and any consumer — automatic model restore and every drill gate included — refuses to act on `unproven`. Capacity state is otherwise keyed by the (harness, account, model) triple with its own state and reset time. No read, policy, or event expresses exhaustion at account granularity alone. A model capped on one account leaves every other model and account unchanged, and the declared account inventory per harness is versioned data, not code. | Triple-keyed projection truth table; the capped-model-alive-sibling-model fixture; measured-artifact identity assertions; measurement-to-attribution binding vectors (right artifact published under a marker-named wrong account is refused); both false-reading regressions (false-alive from an inferred endpoint, false-capped from an unused copy) as named fixtures; unproven-state consumer-refusal vectors; inventory revision-custody proof |
| <a id="ac-cap-02"></a>AC-CAP-02 | Every harness implements one shared typed capacity Adapter Interface — probe capacity, report credential completeness, describe supported models — proven by a single conformance suite that each registered Adapter passes. Adding a harness adds an Adapter and its conformance run and changes no core module. Probes are real substrate requests; rendered terminal text is never a capacity source. | Shared Adapter conformance matrix across every registered harness; real and fake Adapter runs through one suite; core-diff proof; pane-source absence check |
| <a id="ac-cap-03"></a>AC-CAP-03 | Credential facts are references and states only — present, complete, unexpired — per account, with pulls and rotations through the declared path. No secret value appears in any record, response, log, event, export, or fixture. An incomplete credential is a typed fact naming the account and the missing field, never an exception name or a silent skip, and a usable-account count below the declared inventory is reported before the next cap. Discovery of an incomplete credential pages EXACTLY ONCE and the state persists as a standing typed fact; a probe that cannot run never crash-loops its executor and never repeats its page, and an unrepaired incomplete credential remains visible in the operator answer for as long as it holds. | Credential-state contract tests; plaintext/secret scan across records, logs, and fixtures; `credential-incomplete` naming vectors; degraded-pool reporting fixture; page-once-at-discovery and no-crash-loop executor vectors with the standing-fact persistence proof |
| <a id="ac-cap-04"></a>AC-CAP-04 | Rotation selects by the exact triple the seat runs and never fires on a window whose self-clear costs less than the restart it would cause. When a seat is moved to a fallback model to survive a cap, the tower records the original model as desired state and restores it automatically once that triple reports clear; every substitution and restore appends a typed event. | Rotation criterion matrix including the self-clearing-window refusal; fallback-substitution and automatic-restore traces with ledger rows; no-permanent-drift assertion |
| <a id="ac-cap-05"></a>AC-CAP-05 | When any seat is capped, the tower itself answers through the CT-I1-022 operator read: which triple is exhausted, its reset time, what the seat was moved to, and what still moves, with typed partial/unknown epistemics. A delivery receipt for a session that no live model can answer is not an answer and does not satisfy this criterion. | Capped-seat operator answer transcript through API and protected CLI; unknown/partial epistemic vectors; receipt-is-not-an-answer negative |
| <a id="ac-cap-06"></a>AC-CAP-06 | Each fleet-layer defect class demonstrated on 2026-08-11 carries a regression that fails before its fix and passes after. Assigned-model operability is a SEPARATE typed fact derived from capacity — never a liveness state: AC-FLEET-02's four liveness states stand unchanged, and a seat whose assigned model is capped reports `liveness=alive` beside `assigned_model_operability=unusable`, which the composite operator answer must refuse to report as healthy or moving. An account whose credential is present but empty fails a completeness assertion by name rather than surfacing an exception, and an injection aimed at a seat that cannot answer fails a delivery assertion rather than reporting a delivered receipt. No fix for these classes ships without its failing-then-passing test. | RED-first evidence per class at the pre-fix and post-fix heads: the capped-model composite-answer vector proving liveness stays four-state while operability reads unusable, the empty-token completeness vector, and the unanswerable-seat delivery vector |

### Harness adapter seam

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-had-01"></a>AC-HAD-01 | The public Seam publishes only when two real bindings (`claude-code`, `hermes`) plus one deterministic fault-injection fake pass one shared conformance suite, with the fake injecting unacknowledged dispatch, a queued/collapsed-paste composer, a cap menu, context saturation, silent model substitution, dead auth, and pane loss. Registration additionally requires a complete credentials survey — native pool, native fallback, config surface and whether it is authored-config-only, identity proof, reset/window semantics, rotation cache semantics, subagent inheritance, and egress topology — and enforces **never both**: a binding declaring a native layer and also enabling ctower's own for that layer is refused. | Conformance run across three implementations, one suite; injected-fault matrix; publication refusal with one real binding; survey-completeness refusal; both-layers-enabled registration refusal; per-layer role table derived from surveys rather than harness names |
| <a id="ac-had-02"></a>AC-HAD-02 | Delivery is never assumed. `spawn` returns a receipt only after the binding's declared ACK predicate is observed; a queued-composer and a collapsed-paste fixture each return `harness-dispatch-unacknowledged` with zero session-start fact, zero assignment stamp, and zero evidence row. Input into a `working` lane requires the declared interrupt capability or delivers nothing, and steer counts as acknowledged only when the harness returns the durable command ID. | Per-binding composer fixtures; zero-mutation ledger assertion; ACK-predicate declaration in each `HarnessSpec`; mid-turn steer refusal; ACK-only-on-command-id assertion |
| <a id="ac-had-03"></a>AC-HAD-03 | `liveness` reports `served_model` from its per-binding evidence source. A launch/served mismatch appends one `model_changed` observation and overwrites no dispatch stamp; a source proving only the request is recorded as a conflict, never as serving truth; an unreadable substrate returns `substrate-unobservable:<probe>` and surfaces `STATE_UNKNOWN`; a seat self-report satisfies neither and refuses by name. | Three-source matrix (gateway log, transcript, launch argv); substitution, conflict, and unknown fixtures; self-report refusal |
| <a id="ac-had-04"></a>AC-HAD-04 | Cap and saturation are classified before any working marker and win over it: a pane at or past the declared window percentage is `saturated`, a limit/upgrade/out-of-credits pane is `capped`, and both count as not working while a spinner or timer advances. A healthy large-window lane at the same absolute token count is not flagged. Each binding's classifier is proven against captured real substrate output, and a coverage-style `95%` line in scrolled output does not trip it. | Captured-footer corpus per binding; percentage-versus-absolute counter-fixtures; precedence assertions; false-positive corpus |
| <a id="ac-had-05"></a>AC-HAD-05 | Every `writeback` fact attributes to one seat's own project-seat credential and resolves exactly one Actor. Facts outside `capture`/`transition`/`evidence` refuse by name with zero mutation; a Workflow stage change is emitted only as a request and its refusal is reported verbatim; an adapter presented with an operator or commander credential refuses rather than using it; a foreign project key returns `project-scope-denied` with zero disclosure. | Scope matrix over the three scopes; transition-request-not-transition trace; operator-credential refusal; foreign-project refusal with zero rows |
| <a id="ac-had-06"></a>AC-HAD-06 | `collect` derives artifacts from committed refs and durable records only. An uncommitted worktree returns `checkpoint-uncollectable` naming the dirty paths; no terminal capture, pane text, or session existence can fill an evidence slot; a run whose seat wrote no status artifact still yields a complete artifact set from its pushed branch. | Dirty-tree refusal fixture; evidence-slot inventory proving no capture-sourced satisfier; no-status-file collection transcript |
| <a id="ac-had-07"></a>AC-HAD-07 | Lane termination preserves work and continuation. A `saturated` or `capped` fact triggers `teardown(checkpoint)` — work committed and pushed, handoff carrying done / in progress / not started / next three steps — before the lane stops; a `park` carries a stated basis and explicit expiry, re-proves its basis, and fails loud on expiry or when a wake condition turns true; `reap` refuses while sole work is unpushed and refuses a `dead_auth` lane, which is preserved for resume, with one nudge offered before any replacement. For a binding whose survey says "no native fallback", cross-provider failover is a **new attempt** with its own pinned composition after a successful checkpoint, never an in-session swap. After pane, tmux-server, or host loss the adapter reconstructs from ctower state, rejects old epochs, and never treats pane disappearance as success. | Saturation-to-checkpoint trace; handoff-section assertion; park expiry and basis-broken fixtures; sole-work and dead-auth refusals; nudge-before-respawn ordering; failover-to-new-attempt digest diff plus in-session-swap refusal; kill/restart reconstruction with epoch rejection |
| <a id="ac-had-08"></a>AC-HAD-08 | An unknown harness value is carried byte-for-byte through spawn, liveness, and writeback, displayed as observed, included in equality and cross-checks, and never normalized, downgraded, or collapsed. No kernel, reporter, projection, CLI, or Board path imports or parses a harness-private transcript or session format; harness-private observation exists only inside a binding and crosses the seam as typed facts. | Unknown-harness carry/display fixture; import-boundary inventory; per-binding private-parser containment check |
| <a id="ac-had-09"></a>AC-HAD-09 | Nothing dispatches unless the composition and the guard both clear. An unknown, incompatible, revoked, or digest-mismatched `HarnessSpec` performs zero dispatch with no fallback to a generic process; every `spawn` obtains and enforces a current versioned CommandGuard decision for the exact normalized plan at its final pre-dispatch boundary, where `block` and `needs_operator` dispatch nothing and a receipt that cannot be durably recorded first yields zero dispatch; a changed plan, expired or replayed grant, or direct bypass fails closed. | Four fail-closed component fixtures; guard invocation trace per binding; zero-execution-on-block assertion; receipt-first ordering; replay/expiry/bypass refusals |
| <a id="ac-had-10"></a>AC-HAD-10 | Every pool entry carries **three orthogonal states** — `auth ∈ {healthy, lineage-dead, chain-burned}`, `quota ∈ {available, capped(reset_at), capped(reset_unknown), unfunded, unknown}`, `reach ∈ {ok, edge-challenged, unknown}` — with no path collapsing them, selectable only when all three are clear, and a mint moving only the `auth` axis so an `edge-challenged` entry is never routed to a mint, rotation, or restart. Entries are keyed by decoded identity, never by label; a `discovered` identity is non-selectable pending operator keep-or-evict; the Interface exposes **no copy verb**. With no selectable entry, `spawn` performs zero dispatch and refuses `credential-pool-exhausted` carrying `observed`, `meaning`, `action`, per-entry three-axis states, and the earliest known reset or explicit unknown — each diagnosis row mapping to its exact meaning/action pair, with a stale-cache pool-state refusal never classified as real exhaustion. Observation projects a strict named-field allowlist, so no credential value reaches any ledger row, status output, log, refusal, or telemetry event despite adjacent token fields. `limits()` returns per-entry rows carrying each account's own reset clock and never an aggregate substrate verdict, and `acquire` fails only when every entry is unselectable: a fixture pool holding two exhausted entries and one near-full entry acquires successfully and reports three distinct clocks. | Three-axis matrix including auth-healthy/quota-available/reach-challenged; mint-changes-only-auth and no-ceremony-for-reachability proofs; mislabelled-entry and duplicate-label identity fixtures; discovered-not-selectable assertion; Interface inventory proving no copy path; refusal-body assertions per diagnosis row; stale-cache misclassification fixture; adjacent-token projection scan; mixed-exhaustion pool acquiring from its healthy entry; per-account reset-clock rows with no aggregate verdict |
| <a id="ac-had-11"></a>AC-HAD-11 | `rotate` is incomplete until its declared cache-invalidation hook completes — `rotation-incomplete` otherwise, with no entry state recorded from a pre-hook observation — and is **refused rather than attempted** against an `edge-challenged` classification. For a ctower-provided pool, rotation writes the live credential back before swapping, refuses a snapshot older than the live generation, and enforces one live holder. `probe` is valid only when drawn from the pool's own entries, sized to the declared workload shape, taken after invalidation, and classified on the response body rather than the status line: a 403 with a challenge page is `reach: edge-challenged`, never `auth: lineage-dead`; a 200 with empty content is a hang, never capacity; a one-token probe, a constant, a default, or a hand-marked file each report `unknown`. The probe's product, endpoint, and **model** must be the ones seats run: a probe aimed at a different model reports `unknown` for the seats' rung rather than that rung's state, and a status-code-only classifier that maps 401, 402, 403, and 429 to one word is refused as a state source. A window returning to `available` is not selectable until it holds a full observation cycle, and a rotation during a live attempt changes nothing about that attempt. | Hook-incomplete refusal and pre-hook discard; rotate-refused-on-reachability fixture; write-back-before-swap ordering plus `refresh_token_reused` regression; generation-guard and concurrent-holder tests; challenge-page-versus-401 classification pair; empty-content-200 rejection; tiny-probe-then-402 fixture; correlated-failure inference over a shared-egress survey answer; default-alive-constant rejected; probe-target-mismatch reports unknown; four-status-codes-to-one-word classifier refused; flap hold-one-cycle; live-rotation no-op with next-attempt pin diff |
| <a id="ac-had-12"></a>AC-HAD-12 | The ledger and the generated configuration stay faithful to what happened. Spend is metered in **provider-native credit units** using the subscription's versioned per-model, per-direction weight table from the registry, attributed as credits by model × account alongside raw token counts, so "which model on which account drained this plan" is answerable directly; a fixture whose weights span 25× ranks a low-token high-weight lane above a high-token low-weight one, and a stale or missing weight table refuses rather than silently mispricing. Every rotation is metered at one context re-read and every fallback activation at two (switch and return); fallback is recorded turn-scoped with at most one activation per turn, never as a mode; a retry skipped against an unelapsed `reset_at` is an explicit fact, not an absence; aux-task spend is attributed separately from main-model spend under the same seat and attempt; a compression degradation is a recorded quality event. The registry declares the **desired topology per profile** — five subscriptions yielding three rungs — and reconciliation against actual emits typed drift findings in both directions: a desired-but-absent subscription is `missing` and routed to its declared enactment path (`operator-ceremony` for OAuth grants, `secret-reference` for API keys), never reported as ordinary unavailability or silently dropped from the chain, while a present-but-undesired entry is `unregistered` and non-selectable pending operator keep-or-evict. A reconciliation plan whose steps have a dependency order states it, so no proposed step retires a provider that a profile still resolves to. Each profile's `fallback_providers` and aux `fallback_chain` are generated from that topology as revision-pinned authored configuration, applied by the adapter and read by the engine, with ctower never writing `auth.json`; no environment variable can alter a chain; and layer identity is preserved — a same-provider rotation is never recorded as a cross-provider fallback, while a transient 429 respects an explicitly configured provider and a capacity error (402, daily-quota, connection) bypasses it. | Credit-unit arithmetic against a weight-table fixture with a 25× spread; credits-by-model×account attribution query; stale/missing weight-table refusal; cached-versus-fresh input pricing across a cache reset; cost-event arithmetic for rotation and round-trip fallback; turn-scoped-not-modal ledger shape; explicit reset-skip fact; main-versus-aux attribution split; degradation event; desired-versus-actual reconciliation over a partially-minted topology (19 of 24) proving `missing` findings rather than silence; enactment-path routing per subscription kind; `unregistered` keep-or-evict fixture; ordered-plan assertion that no step precedes its dependency; five-subscriptions-to-three-rungs generation fixture; registry-to-chain generation diff with revision/digest pins; env-override-ignored fixture; one-writer-per-file inventory; layer-attribution fixture; transient-versus-capacity ladder matrix |

### Runner service ownership and lifecycle

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-run-16"></a>AC-RUN-16 | Increment 2 has one long-running runner owner per configured target, delivered by a systemd unit and driven by the authoritative runner registry. Desired-to-observed reconciliation is idempotent; a duplicate, stale, or fenced owner cannot become active; restart resumes reconciliation without a second owner, cron loop, ad-hoc shell loop, or service-per-noun reconciler. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; owner/restart/duplicate trace reserved for the active CT-I2-013 implementation |
| <a id="ac-run-17"></a>AC-RUN-17 | Every spawn resolves the registry-pinned HarnessSpec, Supervisor, target, workspace, telemetry, build digest, and CredentialPool binding. The merged binding is the only launch/submit path; the normalized plan and targets pass the current Runtime CommandGuard at the final pre-dispatch boundary; unknown, incompatible, revoked, digest-mismatched, or unfenced input performs zero process action and returns a typed refusal. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; binding/guard/refusal vectors and generated-client import-boundary check reserved for the active CT-I2-013 implementation |
| <a id="ac-run-18"></a>AC-RUN-18 | Liveness is emitted by service-owned observation cycles with independent timestamps and watermarks. Working, capped with a reset or unknown reset, saturated, dead-auth, stale, gapped, and unknown are distinct typed outcomes; process exit, a vendor heartbeat, pane silence, or a missing footer cannot assert success, completion, or a healthy pool. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; deterministic-clock stale/gap/saturation vectors and response-schema assertions reserved for the active CT-I2-013 implementation |
| <a id="ac-run-19"></a>AC-RUN-19 | `collect` crosses the boundary only as strict, revision-pinned typed artifacts. Transcript observations use a separate strict typed transcript observation vocabulary over the runner seam, not `collect` or `writeback`. Harness-private transcript/session formats remain inside the binding; kernel, reporter, projection, CLI, and Board code never parse them. Missing, stale, gapped, unreadable, or vocabulary-unknown collection yields explicit unknown or refusal and never a completion claim. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check |
| <a id="ac-run-20"></a>AC-RUN-20 | `writeback` uses the generated authenticated API client and the seat's own project-seat credential with only the declared capture, transition, and evidence scopes; the runner never connects to Record/Postgres or appends protected verdict/resolution/freeze truth. `teardown` is checkpoint, park, or reap with a typed acknowledgement and fence; saturation and loss preserve the declared checkpoint order, and stop/restart/replay cannot double-apply a lifecycle fact. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; database-privilege, import-boundary, duplicate, restart, and teardown traces reserved for the active CT-I2-013 implementation |
| <a id="ac-run-21"></a>AC-RUN-21 | The credential fixture preserves the sealed seam topology: each Hermes profile has five subscriptions — three separately minted Codex accounts plus z.ai and Alibaba — yielding the three ordered provider rungs `codex` to `glm` to `qwen`; `claude-code` has three per-seat credential homes with one rung per home. The pool interface distinguishes configure/observe from managed perform, has no copy verb, stores references and typed state only, and emits no credential value; the unresolved managed-store boundary blocks activation rather than being guessed. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; topology/count/layer assertions and secret-pattern scan reserved for the active CT-I2-013 implementation |

### Chat and steer HTTP surface

| ID | Pass condition | Evidence capture |
|---|---|---|
| <a id="ac-chat-01"></a>AC-CHAT-01 | The HTTP surface is a closed operation inventory limited to the cockpit proposal's justified G1–G4 paths: crew/liveness/spec reads, typed transcript read/stream, composer input, and steer/ACK, plus exactly three harness-setup writes: `registerHarness`, `submitHarnessSurvey`, and `bindHarnessCredentialReference`. External-provider credential minting/rotation, guard dry-run, keep-or-evict, workspace-change, pull-request creation, and any other control operation are absent rather than hidden behind a generic endpoint. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; OpenAPI operation count and generated-client registry diff reserved for the active CT-I2-014 implementation |
| <a id="ac-chat-02"></a>AC-CHAT-02 | Crew discovery returns only project-authorized rows and exposes the serving model with its evidence source, harness/spec identity, capability set, liveness classification, observed-at timestamp, and watermark. Missing or conflicting evidence is rendered as unknown or refusal; no row exposes a credential value, raw credential reference, or inferred healthy/idle state. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; response-schema snapshots and tenant/project isolation/redaction assertions reserved for the active CT-I2-014 implementation |
| <a id="ac-chat-03"></a>AC-CHAT-03 | Transcript read and stream return ordered typed turns, thinking/tool/interrupt facts, elapsed and source timestamps, cursor position, gap/replay state, and durability state without parsing a harness-private pane or session format. A stale or gapped stream is explicit unknown or refusal, never silently truncated or treated as current. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; cursor/replay/gap vectors and raw-pane import denial reserved for the active CT-I2-014 implementation |
| <a id="ac-chat-04"></a>AC-CHAT-04 | Composer send creates one strict operator command with client-command id, project/crew target, normalized input, capability/authority result, and source watermark. The response distinguishes accepted-pending, folded, and refusal; an acknowledgement is not success until the durable command id is returned, and exact replay is idempotent. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; OpenAPI request/response examples and replay/pending-to-folded trace reserved for the active CT-I2-014 implementation |
| <a id="ac-chat-05"></a>AC-CHAT-05 | Steer uses the same typed command path rather than pane keystrokes, and returns a durable command id or an exact refusal for missing capability, non-working lane, stale liveness, fence conflict, or the still-unsettled operator-principal authority rule. It never reports a local interrupt as a folded result and never accepts a runner-side operator or commander credential as a substitute for the seat writeback credential. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; refusal matrix and command-id ACK/fence trace reserved for the active CT-I2-014 implementation |
| <a id="ac-chat-06"></a>AC-CHAT-06 | Every read and write is project-authorized and strict. Response shapes expose pending versus folded durability, source watermark, and unknown/refusal reasons; no HTTP response, generated-client response model, transcript, log, fixture, or public response contains a credential value or raw secret-bearing reference. AC-CHAT-14 may carry only its opaque non-secret reference and digest in the setup request, never bearer material, and never echoes that reference into a response or observability record. The browser has no new route or credential authority in this row. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; secret-pattern scan and authorization/redaction response matrix reserved for the active CT-I2-014 implementation |
| <a id="ac-chat-07"></a>AC-CHAT-07 | The implementation closes all four inventories together: authored OpenAPI, generated Python/TypeScript clients, contract operation counters, and reference documentation agree byte-for-byte on operations, schemas, refusal codes, and pending/folded response examples. The cockpit document remains proposal-stage input; no public product page, navigation entry, or browser UI claims this behavior before its owning implementation and browser gate are active. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; generated diff and docs-link inventory reserved for the active CT-I2-014 implementation |
| <a id="ac-chat-08"></a>AC-CHAT-08 | G2 adds read-only `listHarnessSpecs` and `getHarnessSpec` rows: one registry list and one get by `HarnessSpec` key/revision over the existing revision-pinned data. The read exposes the spec's key, revision, artifact/config digests, protocols, capabilities, ACK predicate, liveness sources, context-window percentage, probe, pool, survey, layers, and status; no `/v1/harness` path exists at the current head before CT-I2-014 activation, while the deferred setup rows AC-CHAT-12..14 define the bounded path that will be added later. This is a read over facts the record already stores; it adds no new authority and no new seam capability, never registers or dispatches a harness, and never exposes a credential value or raw credential reference. The four inventories for both operations are authored OpenAPI, generated Python/TypeScript clients, contract operation counters, and reference documentation, and they close together. Source: `docs/internal/design/ctower-app.md` §§1–3, proposal snapshot `origin/feat/cockpit-design@c3e4de48`. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; registry list/get schema, authorization, revision, and redaction traces reserved for the active CT-I2-014 implementation |
| <a id="ac-chat-09"></a>AC-CHAT-09 | G10 extends the existing `getTicket` read's `TicketResource` with the current `stage` and `workflow_ref` already present in `WorkflowReceipt` write receipts and `WorkflowChangedAuditPayload` audit facts. The read returns those facts without making the browser perform a transition or fold the audit stream, preserves the open stage-key contract and pinned workflow reference, and never invents an ordered stage vocabulary. This is a read over facts the record already stores; it adds no new authority and no new seam capability. The four inventories for the `getTicket` schema change are authored OpenAPI, generated Python/TypeScript clients, contract operation counters, and reference documentation, and they close together. Source: `docs/internal/design/ctower-app.md` §1 and `docs/internal/design/operator-cockpit.md` §8.2, proposal snapshot `origin/feat/cockpit-design@c3e4de48`. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; TicketResource schema/readback and open-stage/refusal traces reserved for the active CT-I2-014 implementation |
| <a id="ac-chat-10"></a>AC-CHAT-10 | G11 adds read-only `listTickets`, enumerating the tenant's tickets with enough per-row state for triage and filters for project, custodian, and workflow state. `getBoard` is the only existing portfolio read and is not a ticket inventory; the six keyed Ticket reads require an id the caller already holds. This is a read over facts the record already stores; it adds no new authority and no new seam capability and does not replace the Board projection. The four inventories for `listTickets` are authored OpenAPI, generated Python/TypeScript clients, contract operation counters, and reference documentation, and they close together. Source: `docs/internal/design/ctower-app.md` §§1–3, proposal snapshot `origin/feat/cockpit-design@c3e4de48`. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; list filtering, project authorization, watermark, and Board-versus-inventory traces reserved for the active CT-I2-014 implementation |
| <a id="ac-chat-11"></a>AC-CHAT-11 | G12 adds read-only views over the four writes-without-reads queues: outstanding attention findings, the intake queue, the seat-credential register, and poisoned outbox messages. The named operations are `listAttentionFindings`, `listIntakeEvents`, `listSeatCredentials`, and `listPoisonedOutboxMessages`; `project_seats` already exists when a credential is issued but is currently unserved and security-relevant. Each read carries its cursor, source watermark, completeness/gap state, and typed unknown or refusal; the seat register exposes no credential value or raw secret-bearing reference. This is a read over facts the record already stores; it adds no new authority and no new seam capability and adds no disposition or other write. The four inventories for every operation are authored OpenAPI, generated Python/TypeScript clients, contract operation counters, and reference documentation, and they close together. Source: `docs/internal/design/ctower-app.md` §§1–3, proposal snapshot `origin/feat/cockpit-design@c3e4de48`. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; queue cursor/completeness, project authorization, poison visibility, and credential redaction traces reserved for the active CT-I2-014 implementation |
| <a id="ac-chat-12"></a>AC-CHAT-12 | R3129 adds the strict HTTP write `registerHarness` (`POST /v1/harness/registrations`) over the kernel's `HarnessRegistry.register` ceremony. Its request is `{client_command_id, spec, binding_class}` where `spec` is a complete revision-pinned `HarnessSpec` document and `binding_class` is the closed `real|fault_injection_fake` enum; its response is a typed accepted/pending/refusal receipt carrying key, revision, composition digest, derived role table, and source watermark, never a credential value. The route delegates schema parsing, closed first-party admission, duplicate/revoked/digest checks, `harness-runtime-kind-invalid`, `harness-survey-answers-incomplete`, `pool-probe-classification-unknown`, and `harness-layer-roles-conflict` decisions to the kernel; it never accepts a caller-created harness authority, caller-supplied layer roles, generic runtime, or publication shortcut. This is a write over the existing kernel ceremony: it adds no new authority and no new seam capability; `HarnessRegistry.publication` remains the separate two-real-plus-fake gate and registration never dispatches a process. The four inventories for this operation are authored OpenAPI, generated Python/TypeScript clients, contract operation counters, and reference documentation, and they close together. Source: `docs/internal/design/ctower-app.md` §§1–3, proposal snapshot `origin/feat/cockpit-design@c3e4de48`, `packages/ctower-runner-sdk/src/ctower_runner_sdk/registry.py`, and `packages/ctower-runner-sdk/src/ctower_runner_sdk/spec.py`. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; registration schema, idempotency, closed-admission, refusal, role-table, authorization, and zero-dispatch traces reserved for the active CT-I2-014 implementation |
| <a id="ac-chat-13"></a>AC-CHAT-13 | R3129 adds the strict HTTP write `submitHarnessSurvey` (`POST /v1/harness/{harness_key}/survey`) over the existing `CredentialSurvey` admission input. Its request carries one client command identity, the pinned harness key/revision, and an answers object with exactly the kernel's `SURVEY_QUESTIONS`; partial, unknown, caller-invented, or layer-only answers are invalid. The kernel applies the survey-completeness gate before `derive_roles`, returns `harness-survey-answers-incomplete` with detail identifying the missing survey path and zero registration/role mutation when incomplete, and returns the derived role table plus an idempotent answer receipt when complete. This is a pre-registration write and never admits a binding by itself: `registerHarness` remains the sole registration command, rechecks the complete candidate and the kernel's `never both` rule, and returns `harness-layer-roles-conflict` rather than trusting a submitted layer. The operation records only the existing command/receipt path: no route-owned survey draft, role authority, or default answer is created; it creates no new authority and no new seam capability. The four inventories for this operation are authored OpenAPI, generated Python/TypeScript clients, contract operation counters, and reference documentation, and they close together. Source: `docs/internal/design/ctower-app.md` §§1–3, proposal snapshot `origin/feat/cockpit-design@c3e4de48`, `packages/ctower-runner-sdk/src/ctower_runner_sdk/survey.py`, and `packages/ctower-runner-sdk/src/ctower_runner_sdk/registry.py`. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; answer-shape, completeness, idempotency, derived-role, never-both, authorization, and zero-mutation refusal traces reserved for the active CT-I2-014 implementation |
| <a id="ac-chat-14"></a>AC-CHAT-14 | R3129 adds the strict HTTP write `bindHarnessCredentialReference` (`POST /v1/harness/{harness_key}/credential-reference`) over the shipped project-seat credential and runner binding ceremonies. Its request carries the client command identity, pinned harness key/revision, project/seat scope, an opaque `credential_ref`, its `credential_digest`, and only the declared `capture|transition|evidence` scopes; the reference is never a credential value and is not echoed into a public response, log, transcript, or telemetry row. The composition delegates project-seat issuance/reference resolution to the existing `SeatCredentialIssue`/`issue_seat_credential` kernel ceremony and passes only the typed `CredentialReference`/`CredentialRef` into the runner seam. Wrong scope, seat, project, or writeback scope returns the kernel's exact `credential-scope-denied`, `credential-seat-mismatch`, `project-scope-denied`, or `writeback-scope-denied` with zero mutation. The operation does not mint an external provider credential, add a pool or keep-or-evict authority, let the runner connect to Record/Postgres, or create a second credential store; it adds no new authority and no new seam capability. The four inventories for this operation are authored OpenAPI, generated Python/TypeScript clients, contract operation counters, and reference documentation, and they close together. Source: `docs/internal/design/ctower-app.md` §§1–3, proposal snapshot `origin/feat/cockpit-design@c3e4de48`, `packages/ctower-runner-sdk/src/ctower_runner_sdk/seam.py`, `packages/ctower-runner-sdk/src/ctower_runner_sdk/policy.py`, and `packages/ctower-kernel/src/ctower_kernel/record/credentials.py`. | `uv run pytest tests/contracts/evidence/test_runner_chat_spec.py -q`; not-yet-required structural contract check; reference-only secret scan, scope/seat/project refusal matrix, idempotency, authorization, and zero-mutation traces reserved for the active CT-I2-014 implementation |
...
ctower has no ticket API yet. The 52 stable IDs below—9 L0 preconditions, 31 I1 items, and 12 I2
...
| CT-I1-005 | Stable deferred alias to the `CT-I2-005` I2.4 browser product sub-checkpoint: realize D22's Home, Board, contextual/direct Ticket, narrow Fleet/Analytics, product routes, and product-surface Playwright evidence only after I1 API/CLI authority is proven. CT-I1-013 is the sole earlier exception and owns only login/callback/session/logout/auth-error routes plus auth-only evidence. D75 retires the separate `ctower-ui` Inbox dogfood boundary and its suite; neither supplies browser-product evidence or progress against this alias. | Deferred to CT-I2-005; no I1 critical dependency | Designer + UI QA; operator taste gate when material | `apps/ctower-web/src/surfaces/`; `routes.ts`; kernel `attention/`, `projections/` | I2.4 every-control UI QA, tenant isolation, product-route inventory, reconnect, <10 s Home, and unknown screenshots | `pnpm run test:e2e` at CT-I2-005 |
...
| CT-I1-021 | Deliver the Phase-1 contextual console viewer server foundation as one landable candidate: exact current-session allowance and `ConsoleViewGrant` authority; browser-session/CSRF/exact-Origin routes; registered read-only tmux/log Adapter; bounded durable SSE with typed gaps and closure; per-object RESTRICTED envelope custody through the dedicated output reader; persistent revocation, denial suspension, and global kill switch; and literal loopback/tailnet bind validation. It adds no browser UI, typing, pane write, generic process route, public listener, Record-tier adapter client, or authority claim for the pre-existing terminal reader. | CT-I1-013; CT-I1-019; CT-I1-020; D56; issue #437 | Engineer + independent QA/Review + CSO trigger test | Authored Console domain/HTTP/migration contracts; kernel `console/`; `ctower-api` browser/admin routes and registered Adapter; generated contracts; concept/reference/operator/security docs | AC-CON-01..07; RED-first phase manifest; real PostgreSQL/API/SSE trace; real private/tailnet granted-crew shadow view; typed expiry/revocation/fence proof; `ss -tlnp` and direct-path inventory; same-candidate docs | `uv run pytest tests/contracts/console tests/modules/console tests/acceptance/increment-1/test_console_authority_atomicity.py tests/acceptance/increment-1/test_console_authority_preflight.py tests/acceptance/increment-1/test_console_collection_lock.py tests/acceptance/increment-1/test_console_reader_role_adoption.py tests/acceptance/increment-1/test_console_transport_disconnect.py tests/acceptance/increment-1/test_console_view_grants.py tests/acceptance/increment-1/test_console_view_grant_races.py -q` |
| CT-I1-022 | Deliver fleet self-heal as one landable tower-feature candidate per D57: the versioned seat-and-crew registry with exactly one respawn owner per seat and no stored resume identities; typed probe/event-derived per-seat liveness that never reads pane rendering; respawn-policy revisions that local executors (systemd units, seat-guard, cron bootstrap) consume through the generated client as tower-driven agents under one spawn-lock protocol; the durable death/respawn/cap/drill event ledger with causes and claimant identities; and the operator answer read (API + protected CLI) folding registry, liveness, and ledger into "is everything moving, anything capped or blocked" with typed partial/unknown epistemics. Kill-drill and reboot-drill run as tower-run acceptance on an isolated substrate with ledger proof rows; the reboot drill additionally proves boot-enabled services and per-seat session CONTINUITY (pre/post session-identity equality, not merely discovery), with reboot-durable dynamic-crew respawn recipes as a registration-enforced prerequisite. Browser Fleet-surface deepening remains CT-I2-005-gated; no new public ingress, no sixth surface. | CT-I1-006; CT-I1-019; D57; operator orders R2928/R2929 | Engineer + DevOps + independent QA/Review; Commander drill verification; operator taste gate on the answer read | Authored fleet/runtime contracts; kernel `runtime/`, `record/`, `projections/`; `packs/policies/fleet/`; generated clients; `ctower-api`; `ctowerctl`; Mission Control executor consumers; runbooks | AC-FLEET-01..05; RED-first registry/liveness/ledger/read suites; isolated-substrate kill and reboot drill transcripts with ledger rows; dueling-claimant stand-down, stale-evidence-unknown, and pane-render misclassification fixtures; same-candidate docs | `uv run pytest tests/contracts/fleet tests/modules/runtime/test_fleet_selfheal.py tests/acceptance/increment-1/test_fleet_selfheal.py -q` |
| CT-I1-023 | Deliver harness capacity and credential tracking as one landable tower-feature candidate per D58, extending CT-I1-022 rather than duplicating it: triple-keyed (harness, account, model) capacity state with per-triple reset times over a versioned account inventory; one shared typed capacity Adapter Interface per harness — probe capacity, report credential completeness, describe supported models — proven by a single conformance suite every registered Adapter passes; credential facts as references and completeness states only, with typed `credential-incomplete` naming and degraded-pool reporting before the next cap; rotation keyed to the triple the seat runs, refusing windows that self-clear cheaper than the restart, with fallback-model substitution recorded as desired state and restored automatically when the triple clears; and the capped-seat operator answer served through the CT-I1-022 read (API + protected CLI) instead of a delivery receipt. Declared inventory: three Claude accounts, three Codex accounts, one z.ai, one Alibaba, one OpenRouter key. No secret value enters any record, log, event, export, or fixture; no new public ingress and no sixth surface. | CT-I1-022; D58; operator orders R2931, R2932 | Engineer + DevOps + CSO credential review + independent QA/Review; operator taste gate on the capped-seat answer | Authored capacity/credential contracts; kernel `runtime/`, `record/`, `projections/`; `packs/policies/capacity/`; harness Adapters; generated clients; `ctower-api`; `ctowerctl`; runbooks | AC-CAP-01..06; RED-first triple/adapter/credential/rotation/answer suites plus the three named fleet-layer regression classes (capped-model seat, empty-token account, unanswerable-seat injection); capped-model-alive-sibling and empty-credential fixtures; shared Adapter conformance across every registered harness; substitution-and-restore ledger trace; secret-absence scan; same-candidate docs | `uv run pytest tests/contracts/capacity tests/modules/runtime/test_capacity_tracking.py tests/acceptance/increment-1/test_capacity_answer.py -q` |
| CT-I1-024 | Deliver the Request-maintenance proposal queue as one landable tower-feature candidate per D59 after Request authority and the native digest exist: append-only proposal identities and lifecycle facts separate from their target Requests; proposer identity, source watermark, expected Request version, and typed evidence pointer on every row; proposal-only duplicate/completed/supersession/kill/keep maintenance; deterministic top-20 kill-or-keep ranking; operator-only confirm and reject, with confirmation causing the target mutation through its own ordinary recorded Request command; and digest counts plus a pointer, never a list. The generated API and protected CLI are the I1 review surface and one-action confirmation contract. Browser rendering remains gated by CT-I2-005 and joins the existing CT-I2-011 Request surface; this item adds no product UI, schedule, principal, ingress, egress, adapter, or Mission Control writer change. | CT-I1-015; CT-I1-018; D59; operator order R2947 | Engineer + independent QA/Review; operator taste gate on the review and confirmation read | Authored Request-proposal/event/HTTP contracts; kernel `work/`, `record/`, `projections/`; generated clients; `ctower-api`; `ctowerctl`; Request and digest docs | AC-PROP-01..07; RED-first proposal-separation/evidence/authority/rejection/ambiguity/digest/dedupe suites; real PostgreSQL proposal-to-command trace; exact-text and Request-zero-diff proofs; generated API/CLI parity; principal/route/schedule inventory; same-candidate docs | `uv run pytest tests/contracts/request_proposals tests/modules/work/test_request_proposals.py tests/acceptance/increment-1/test_request_maintenance_proposals.py -q` |
| CT-I1-025 | Deliver the recorded project-management system as one landable tower-feature candidate per D60 after Request authority, the native digest and fixed-Routine path, and the proposal queue exist: Request-rooted feature plans plus per-Request and per-Ticket current plan revisions; `PLANNED`/`dispatched=false` capture default and plan-before-dispatch gate; deterministic dated daily stack over recorded week-goal/Request/Ticket/proposal facts; operator-only one-line confirm or exact-permutation re-rank as an idempotent planning command separate from dispatch; zero-memory rolling-24-hour and Project-grouped seven-civil-date reminder views with strict inline-versus-pointer and accepted-relation dedupe rules; typed partial/unknown refusal; and one fixed 07:47 Europe/Vilnius `ctower.beat.project-management@1` Routine that cannot confirm or dispatch. After one live delivery, the director retires exactly the R2951/R2952/R2953 interim sections. The generated API and protected CLI are the I1 surface; browser rendering remains CT-I2-005-gated. This item adds no Feature aggregate, product UI, principal, public ingress, transport, egress, or non-kernel Record client, and explicitly excludes R2956 session/dream/context analysis. | CT-I1-015; CT-I1-018; CT-I1-019; CT-I1-024; D60; operator orders R2951, R2954, R2955 | Engineer + Engineering Manager + independent QA/Review; operator taste gate on the planning/reminder read and confirmation command | Authored project-management plan/reminder/event/HTTP contracts; kernel `work/`, `record/`, `projections/`, `runtime/`; generated clients; `ctower-api`; `ctowerctl`; `packs/routines/ctower.beat.project-management/v1.yaml` and closed registration; Request, Ticket, digest, Routine, and `docs/concepts/project-management.md` docs | AC-PM-01..07; RED-first default/plan/authority/determinism/window/dedupe/conflict suites; real PostgreSQL plan-to-confirm-to-dispatch-eligibility trace; generated API/CLI parity; Routine pack/ref exact-count, cadence, and cannot-confirm proof; one live delivery and director migration receipt; exact principal/route/schedule inventory; same-candidate outside-reader docs | `uv run pytest tests/contracts/project_management tests/modules/work/test_project_management.py tests/modules/projections/test_project_management.py tests/acceptance/increment-1/test_project_management.py -q` |
| CT-I1-026 | Deliver dream-cycle session mining as one landable MAP -> REDUCE -> DREAM candidate per D61 after the project-management projection exists: three runner-side harness readers behind one extractor Interface; deterministic reader-local redaction; one strict bounded Session Digest and Pattern Card contract; accepted per-harness cursors; an append-only Knowledge-owned cross-night pattern ledger; successor dream Routine revisions using the existing model-requirement shape; and DREAM consumption of Pattern Cards plus authorized Request/Ticket projections only. The generated API/protected CLI expose ledger, cursor, completeness, and failure truth; runner consumers have no Record-tier credential. R2951 may reuse the date-range Pattern Card projection but owns its report separately. The build follows R2954, requires the verified live dead-consumer alarm, and ships `docs/concepts/session-mining.md`; this item adds no product UI, principal, public ingress, raw-transcript store, or direct Request/Ticket mutation authority. | CT-I1-006; CT-I1-025; D16; D33; D59; D61; verified live dead-consumer alarm; operator order R2956 | Engineer + CSO redaction review + independent QA/Review; Director confirms live dream receipt | Authored session-mining/runtime/HTTP contracts; kernel `knowledge/`, `runtime/`, `projections/`; generated clients; `ctower-api`; `ctowerctl`; runner consumer and Claude/Codex/Hermes extractor Adapters; successor dream Routine packs and closed inventories; session-mining/operator/security docs | AC-MINE-01..07; RED-first all-reader conformance, ledger/cursor/replay, tokenizer-bound, parse/overflow/completeness, redaction-canary and raw-transcript-absence suites; real PostgreSQL and host-reader trace; model-routing and successor-pack parity; API/CLI equality; live dead-consumer alarm and dream receipt; same-candidate docs | `uv run pytest tests/contracts/session_mining tests/conformance/session_extractors tests/modules/knowledge/test_session_mining.py tests/acceptance/increment-1/test_session_mining.py -q` |

| CT-I1-027 | Deliver the canonical Ticket movement feed as one landable tower-feature candidate per D62 after Request-maintenance proposals: enrich the accepted `workflow.changed` transition branch in the existing stage-transition transaction with exact from/to stage and a stable transition-evaluation/evidence-manifest pointer; preserve exactly-once replay/refusal semantics; expose one authorization-preserving project cursor through generated API and protected CLI; add prior-Europe/Vilnius-civil-day counts plus pointer/watermark to the digest; and serve validator-clean authenticated Atom 1.0 from the same stream. Movement records remain Ticket-linked transition facts, not exhaustive Ticket snapshots. The first build creates and links `docs/concepts/ticket-movement.md`, marks the Stall clocks and Commander beat worklist sections planned, and adds no product UI, sixth route, principal, anonymous/query/feed-token auth, second ledger, schedule, or Ticket-text transport. | CT-I1-003; CT-I1-012; CT-I1-024; D59; D62; operator orders R2959, R2964 | Engineer + independent QA/Review + CSO exact-head Atom/auth/leak review; operator taste gate on the movement read | Authored workflow-event and movement HTTP contracts; kernel `workflow/`, `record/`, `projections/`; generated clients; `ctower-api`; `ctowerctl`; morning-digest movement projection; Atom endpoint; `docs/concepts/ticket-movement.md` and navigation/API/CLI/digest/security references | AC-MOVE-01..05; RED-first transition atomicity/replay/refusal and feed cursor/rebuild suites; real PostgreSQL/API/CLI/Atom trace; validator result; civil-day digest snapshots; cross-project, route/principal/credential, no-second-ledger, no-exhaustive-snapshot, and no-Ticket-text inventories; independent CSO verdict; same-candidate docs | `uv run pytest tests/contracts/movement_feed tests/modules/projections/test_ticket_movement.py tests/acceptance/increment-1/test_ticket_movement.py -q` |
| CT-I1-028 | Deliver typed Ticket stall clocks as one landable tower-feature candidate per D62 after movement and the project-management core: append-only wait episodes for `review`, `operator-decision`, `external`, and `blocked-by-ticket` with server time, owner, and reason-specific pointer; later recheck/resolution facts; exact 24-hour/48-hour/seven-day floors and event-driven blocking-Ticket reevaluation; D50 decision re-brief; typed breached/unknown folds; and one-row-per-current-episode batching into the owning Commander's next existing beat and D60 digest aging. Wait records remain Ticket-linked episode facts, not exhaustive Ticket snapshots. The build updates the shared ticket-movement documentation and adds no Ticket transition, automatic Attention/`human-waiting`, immediate page, Routine, cadence, or page storm. | CT-I1-003; CT-I1-025; CT-I1-027; D50; D60; D62; operator orders R2959, R2964 | Engineer + Engineering Manager + independent QA/Review; operator taste gate on breach and unknown aging | Authored wait-event/stall-clock/HTTP contracts; kernel `work/`, `record/`, `projections/`; generated clients; `ctower-api`; `ctowerctl`; existing project-management digest and beat reads; `docs/concepts/ticket-movement.md` plus API/CLI/digest/Routine references | AC-STALL-01..04; RED-first episode immutability/recheck/reason/clock/unknown/idempotency suites; real PostgreSQL threshold and blocking-Ticket movement trace; API/CLI/beat/digest equality; Ticket-transition/Attention/outbox/Routine/schedule zero-diff inventories; no-exhaustive-snapshot proof; same-candidate docs | `uv run pytest tests/contracts/stall_clocks tests/modules/work/test_stall_clocks.py tests/modules/projections/test_stall_clocks.py tests/acceptance/increment-1/test_stall_clocks.py -q` |
| CT-I1-029 | Deliver beat-from-ctower as one landable tower-feature candidate per D62: one generated authenticated Commander/custodian worklist read over the full current owned nonterminal Ticket set grouped by pinned stage, breached/unknown stalls, movement delta, per-source completeness, and accepted watermark; plus the D52 Mission Control sibling-consumer preflight that calls the generated client before every applicable DIRECTOR beat injection and records the watermark in the existing append-only external ledger. Every beat re-derives full membership; movement watermarks limit only deltas. Failed/partial/unknown input cannot become a contextless clean injection, empty claim, advanced watermark, or suppression. The build updates the shared ticket-movement documentation, imports no kernel persistence or Record client/credential into the runner, reuses existing Routine schedules unchanged, and does not claim D52's server-side lane-binding ceremony. | CT-I1-003; CT-I1-019; CT-I1-027; CT-I1-028; D52; D62; operator orders R2959, R2964 | Engineer + Commander + independent QA/Review; Director confirms live preflight/beat receipt | Authored Commander-worklist/runtime/HTTP contracts; kernel `projections/`; generated clients; `ctower-api`; `ctowerctl`; Mission Control sibling consumer and provisioning; existing claim/delivery/replay ledger integration; `docs/concepts/ticket-movement.md` plus API/CLI/Routine/consumer/operator references | AC-BEAT-01..04; RED-first ownership/stage/completeness/watermark/failure suites; real PostgreSQL/API/CLI/consumer transcript; compaction/restart/truncation/model/harness and no-drop fixtures; import/credential/lane-binding/Routine/schedule inventories; live DIRECTOR preflight receipt; same-candidate docs | `uv run pytest tests/contracts/commander_worklist tests/modules/projections/test_commander_worklist.py tests/acceptance/increment-1/test_beat_from_ctower.py -q` |

| CT-I1-030 | Deliver the public tool and knowledge catalog engine as one landable Catalog candidate per D63: extend the existing VersionedComponent and CompanyBundle path for COMPANY and PROJECT scoped tool, skill, and persona entries; immutable exact revision/digest indexing and resolution; existing-Actor publish/read/invoke authorization with visibility granting no execution; strict private-source cursor/watermark synchronization; typed runner-side Runtime/CommandGuard invocation and result custody; and one versioned-schema directory importer with canonical dry-run and idempotent re-import. Actual tenant catalog content and inventory facts remain in private per-company storage and never enter this repository. This item consumes none of CT-I1-024..029 and adds no second Catalog, bundle, pointer, principal, authority path, product UI, public source/provider Seam, Record-tier runner client, secret value, or active execution path; exercised arbitrary execution requires the standing Runtime/CommandGuard boundary. The build ships the outside-developer concepts -> quickstart -> reference path with zero internal references in the same candidate. | CT-I1-004; CT-I1-009; CT-I1-013; D12; D16; D31; D37; D63; operator order R2967; independent of CT-I1-024..029 | Engineer + Engineering Manager + CSO authorization/sync/isolation review + independent QA/Review; outside-developer documentation gate | Authored v2 catalog/tool/skill/persona/sync/invocation contracts when D37 requires them; kernel `catalog/`; generated clients; `ctower-api`; `ctowerctl`; runner-side source/materialization/execution code; bounded directory import helper; `docs/concepts/catalog.md`, `docs/quickstart.md`, `docs/reference/catalog.md`, navigation, and affected API/CLI/runner/security references | AC-CAT-01..08; RED-first scope/revision/permission/invocation/cursor/import/repository-content/docs suites; real PostgreSQL/API/CLI/runner trace; exact-pin and active-pointer proofs; source replay/partial/unknown and dry-run/re-import fixtures; tenant-content named negative; cross-tenant/Project, principal/scope, secret-value, Record-tier, and no-parallel-Catalog inventories; independent CSO exact-head verdict; fresh outside-reader walkthrough | `uv run pytest tests/contracts/catalog tests/modules/catalog tests/conformance/catalog_sources tests/conformance/catalog_execution tests/acceptance/increment-1/test_catalog_engine.py -q` |
| CT-I1-031 | Deliver first-class workspaces as one landable Work/Runtime candidate per D64: one append-only workspace record with immutable `WORKTREE|PROJECT|COMPANY` type, type-valid Ticket/Project/company binding, normalized start-directory intent, exact immutable Catalog mount-set revisions, and a reference to existing credential-scope authority; minimal `{workspace_id, workspace_type}` Ticket link facts; idempotent ticket-bound WORKTREE creation at accepted dispatch; PR-record merge truth causing an attributable `workspace.close_requested` fact with D62 transition causation where applicable; persistent PROJECT and COMPANY lifecycles; explicit remount and close commands; checkpoint-safe runner receipts; and fail-honest lifecycle folds. The control plane owns Record truth while only the runner materializes, remounts, and cleans host bytes through generated authenticated APIs under the standing Runtime/CommandGuard, lease, and fence boundary. This item depends on the canonical movement fact only, is independent of stall clocks and beat worklists, and adds no second workspace store, host-path authority, principal, grant kind, credential value, floating Catalog pointer, Record-tier runner client, product UI, sixth route, public provider Seam, or active execution path. The build ships the outside-developer concepts -> quickstart -> reference path with zero internal references in the same candidate. | CT-I1-003; CT-I1-009; CT-I1-027; CT-I1-030; D34; D62; D63; D64; operator order R2967 ticket B; independent of CT-I1-028/029 | Engineer + Engineering Manager + CSO credential/scope/isolation/cleanup review + independent QA/Review; outside-developer documentation gate | Authored workspace/lifecycle/HTTP/runner contracts when D37 requires them; kernel `work/`, `runtime/`, `record/`, `catalog/`, `projections/` behind existing Interfaces; generated clients; `ctower-api`; `ctowerctl`; one runner-side local workspace Adapter and conformance fake; `docs/concepts/workspaces.md`, `docs/quickstart.md`, `docs/reference/workspaces.md`, navigation, and affected Ticket/Catalog/API/CLI/runner/security references | AC-WS-01..09; RED-first type/binding/link/dispatch/merge/close/remount/credential/mount/unknown/docs suites; real PostgreSQL/API/CLI/runner lifecycle trace; exact-pin and active-pointer proofs; checkpoint/sole-work, restart/replay, lease/fence, partial/unknown, cross-tenant/Project, secret-value, principal/grant, Record-tier, and no-parallel-store inventories; independent CSO exact-head verdict; fresh outside-reader walkthrough | `uv run pytest tests/contracts/workspaces tests/modules/work/test_ticket_workspaces.py tests/modules/runtime/test_workspaces.py tests/conformance/workspace_adapters tests/acceptance/increment-1/test_workspaces.py -q` |

| CT-I1-032 | Deliver the external-estate record imports as one landable candidate per D62 and D66: provide manifest-generation and export handoff only; AC-REQ-06 execution remains CT-I1-015's separately gated proof and is not claimed by this candidate; add operator-authority manifest-bound imports for inbox history, agreed-decision Rulings, knowledge documents, and one new append-only company-record aggregate for accountability rows; every tier idempotent by its natural key with typed parity reports emitted before any source closure, unknown owners mapped to an explicit source-only state, and no principal, seat, or grant minted by any import. Source freezes, fence ordering, and the archive act stay with the external custodian. | CT-I1-003; D30; D62; D66; operator order R3000 | Engineer + independent QA/Review + CSO imported-tenant-content boundary review; director parity acknowledgment per surface | Authored import-manifest contracts; kernel `record/`, `knowledge/`, `work/` import paths behind existing Interfaces; `ctowerctl` migration commands; company-record aggregate + migration SQL; parity-report artifacts | AC-IMP-01..04; RED-first manifest/idempotency/refusal suites; real PostgreSQL import traces; parity reports per tier; the Request cutover proof remains in CT-I1-015's designated command | `uv run pytest tests/contracts/estate_imports tests/modules/migration/test_estate_imports.py tests/acceptance/increment-1/test_estate_imports.py -q` |
| CT-I1-033 | Deliver company/project-scoped activity-gated Routines as one landable candidate per D62 and D66: one append-only registration record with a closed typed gate set, evaluation that fires only on schedule-plus-gate-true with appended dispatch/skip/degraded facts standing on named watermarks, five representative external schedules migrated end-to-end with catch-parity evidence (both operator report rails included), and every unmigrated schedule enumerated with its intended gate. No expression language, product UI, new principal, or Routine-initiated host execution in this candidate. | CT-I1-003; CT-I1-012; D62; D66; operator order R2999; operator order R3000 | Engineer + independent QA/Review; director catch-parity acknowledgment per migrated schedule | Authored Routine/gate/HTTP contracts; kernel `runtime/`, `record/`, `projections/`; generated clients; `ctower-api`; `ctowerctl`; five Routine registrations; catch-parity report artifacts | AC-RTN-01..04; RED-first registration/gate/replay/degraded suites; real PostgreSQL fire traces; catch-parity reports | `uv run pytest tests/contracts/routines tests/modules/runtime/test_routines.py tests/acceptance/increment-1/test_routines.py -q` |
| CT-I1-034 | Deliver spawn-custody records as one landable candidate per D62 and D66: one append-only spawn record with substrate profile pin, task reference, worktree intent, null-until-built workspace reference, and appended lifecycle transition facts; record-before-drive integration in the external spawn driver with a mode-0600 pending-durability spool replayed on recovery; external spawn-history import by source identity with latest-status-effective semantics; and a reconcile read path that keeps reading the external twin until one recorded parity proof, the swap being a separately admitted act. No credential value in any payload. | CT-I1-003; CT-I1-009; D62; D66; operator order R2982; operator order R3000; workspace reference waits on CT-I1-031 | Engineer + independent QA/Review + CSO no-secret and authority review | Authored spawn-record/transition/HTTP contracts; kernel `runtime/`, `record/` behind existing Interfaces; generated clients; `ctower-api`; `ctowerctl`; external driver integration + spool; parity-proof artifact | AC-SPWN-01..04; RED-first lifecycle/spool/import/parity suites; real PostgreSQL traces; record-before-drive ordering proof | `uv run pytest tests/contracts/spawn_records tests/modules/runtime/test_spawn_records.py tests/acceptance/increment-1/test_spawn_records.py -q` |

| CT-I1-035 | Deliver Routine work items as one bounded candidate under D67 and operator order R3016: an accepted Routine fire appends exactly one Inbox work item for the owning seat carrying Routine identity, window, gate evidence and watermark, and one exact immutable Knowledge `document_id`, with the session-targeted dispatch path REMOVED rather than kept beside it; the complete current 17-reference Routine registry is enumerated and every reference is proven on the item path; policy changes use new Knowledge snapshots plus explicit Routine-revision supersession; the item remains unconsumed until an owning-seat receipt referencing the delivered artifact is accepted, regardless of Inbox read state; firing is suppressed with a typed fact while that receipt is absent; and a stable-key append-only alarm episode handles confirmed-unconsumed, degraded, and recovered-receipted outcomes with one ordinary alarm per window under repeated/restart/concurrent evaluation, using an immutable scoped Routine-revision `escalation_seat_ref` and refusing missing, stale, revoked, or foreign-scope bindings. The five CT-I1-033 host twins receive catch-parity evidence before retirement. Adds no product UI, principal, session write path, second Inbox store, or expression language. | CT-I1-003; CT-I1-033; D66; D67; operator orders R3000, R3016 | Engineer + independent QA/Review + CSO addressing/authorization review; director confirms live receipt and alarm | Authored routine-item/receipt/alarm contracts; kernel `runtime/`, `record/`, `knowledge/`, `projections/`; generated clients; `ctower-api`; `ctowerctl`; complete 17-reference Routine registry and five host-twin registrations; `docs/concepts/routines.md` and Inbox/CLI/API references | AC-RWI-01..06; RED-first fire/pointer/suppression/receipt/alarm/re-point suites; real PostgreSQL 17/17 registry and fire-to-item-to-receipt traces; immutable Knowledge supersession trace; session-write/target-session absence inventory; degraded/recovery/idempotency vectors; escalation binding/authorization/refusal matrix; five catch-parity reports; same-candidate docs | `uv run pytest tests/contracts/routine_items tests/modules/runtime/test_routine_items.py tests/acceptance/increment-1/test_routine_items.py -q` |
| CT-I1-037 | Deliver one authenticated Knowledge catalog discovery read as a derived, source-linked view over the existing immutable Knowledge-document facts, accepted Ruling facts, and D63 Catalog `skill` revisions: normalized term query, bounded deterministic results, exact source/provenance and recorded-at fields, typed currentness, one source watermark, and existing tenant/Project authorization. It reuses the existing Knowledge, Ruling, Catalog, Actor, generated-client, and protected-CLI authorities; playbooks and checklists remain Knowledge documents and decisions remain Rulings. It creates no authoritative body store, second Catalog, new component kind, principal, grant, importer, provider, execution path, or Record-tier runner client, and returns `unknown` rather than inferring freshness from missing or stale source facts. The build updates the existing Catalog/Knowledge/API/CLI documentation path for the operator outcome. | CT-I1-004; CT-I1-016; CT-I1-030; D49; D63 | Engineer + independent QA/Review + CSO scope/redaction review; operator verifies the one-place query outcome | Authored catalog-query/knowledge-result contracts; kernel derived read Interface over existing Knowledge projection, accepted Rulings, Catalog revisions, and Actor authority; generated clients; `ctower-api`; `ctowerctl`; existing Catalog/Knowledge/API/CLI docs | AC-KCAT-01..07; RED-first query/source/currentness/scope/watermark/no-second-store/docs suites; real PostgreSQL cross-source trace; generated API/CLI parity; exact-source and unknown-state matrix; same-candidate docs | `uv run pytest tests/contracts/knowledge_catalog tests/modules/projections/test_knowledge_catalog.py tests/acceptance/increment-1/test_knowledge_catalog.py -q` |
...
| CT-I1-040 | Deliver the tracked review-record rescue as one bounded documentation-and-policy candidate under D71: normalize the 31 selected CSO/security source verdicts into a searchable `docs/internal/review-records/` corpus with one file, one final verdict, and one exact head per record; split multi-subject evidence; name unavailable heads instead of inventing them; preserve a single signed-off accountability block; and prove the intended-tree redaction, source denominator, excluded-corpus disposition, and record-only commit scope. Review lanes remain read-only, the Commander lands the record as the last commit before merge, and a commit touching only this path does not invalidate prior product gate verdicts. This candidate adds no product route, principal, runtime authority, environment variable, feature flag, secret value, public navigation entry, or gate waiver. | CT-L0-007; D71 | Engineer + independent Review/CSO; Commander owns landing order | `docs/internal/review-records/`; `tests/repository/test_review_records.py` | AC-REV-01..04; RED-first one-file/one-verdict/one-head test; 31-source/34-record manifest; intended-tree secret/redaction scan with detector-shaped canary; exact record-only `git diff --name-only <sha>^ <sha>` proof; excluded-corpus and no-new-surface inventory | `uv run pytest tests/repository/test_review_records.py -q` |

| CT-I1-041 | Deliver the harness-adapter seam and its first real binding under D72: one revision-pinned `HarnessSpec` component — key, revision, artifact/config digests, input/output protocol, declared capabilities, ACK predicate, liveness evidence sources, declared context-window percentage, probe shape, and pool cache-invalidation hook — parsed as data without executing package code; the five verbs `spawn`, `liveness`, `collect`, `writeback`, `teardown` composed over D10's existing Supervisor Interface with no second process-control vocabulary; and the sibling `CredentialPool` Interface (`acquire`, `meter`, `limits`, `rotate`, `probe`, `request_mint`) resolved at `spawn` and carrying deliberately no copy verb. The `hermes` binding is the first real Adapter: because that harness already ships per-provider pools, rotation strategies, and an ordered fallback ladder, this binding CONFIGURES AND OBSERVES both layers and implements neither — it spawns through a profile directory whose own config owns model and reasoning effort, reads serving truth from the gateway/provider log while a footer proves only the request and is recorded as a conflict, classifies pool errors as dead auth, treats a declared fallback rung of the durable spawn intent as the never-stall ladder rather than substitution except on judgment lanes where tolerance is zero, projects the engine's per-entry fields into the ledger through a strict named-field allowlist, and never writes the engine's own auth state. It ships the deterministic fault-injection fake and the one shared conformance suite; with a single real binding the public Seam does not publish. It adds no operator or commander credential in an adapter, no rotation policy competing with the engine's, no cron, cadence, or backlog authority, no stage advancement, no remote or image Seam, and no record-tier connection from the runner. | CT-I1-034; CT-I1-036; CT-I2-003; CT-I2-004 — INV-58's CommandGuard at the final pre-dispatch boundary is a hard prerequisite that moves with the first dispatch rather than being waived, so this epic's activation ordering against Increment 2 is an operator/commander scope decision and not this item's; D10; D11; D13; D72; the landed credential-limits surface (kernel `pools/`, migration `0078`, `recordPoolObservation` and `readPoolLimits`, `ctowerctl pools observe` and `pools show`); the sealed harness-adapter seam design at head `c07c049c` of `feat/adapter-seam-design` | Engineer + Engineering Manager + CSO + independent Review/QA | Authored `contracts/runner/` HarnessSpec, capability, and credential-lease contracts; `apps/ctower-runner/`; `packages/ctower-runner-sdk/`; `tests/conformance/harness-adapter/`; generated clients; no new persistence, identity plane, or kernel authority | AC-HAD-01..09 proven on the `hermes` column plus the configure-and-observe half of AC-HAD-10, AC-HAD-11, and AC-HAD-12; RED-first ACK-predicate, request-versus-serving conflict, cap-and-saturation precedence, and uncommitted-tree fixtures; captured real hermes substrate corpus per classifier; publication refusal with one real binding; survey-completeness and never-both registration refusals; strict named-field projection scan proving no credential value reaches any ledger row, response, log, refusal, or telemetry event; guard-invocation trace with zero dispatch on block and needs-operator | `uv run pytest tests/conformance/harness-adapter tests/acceptance/increment-1/test_harness_seam.py -q` |

| CT-I1-042 | Deliver the `claude-code` binding as the second real Adapter — the one that earns the public Seam when it, `hermes`, and the deterministic fault-injection fake pass the same conformance suite with pre-existing cells' assertions frozen; later ticket row-named cells append through the `harness_subjects.py` registration entry, and any shared-surface growth is legal only when enumerated and reviewed under D74. This harness ships neither a credential pool nor an in-session fallback and holds one account per config home, so ctower PROVIDES both layers: topology A per-seat credential isolation with one config home per account, rotation by write-back-before-swap that refuses a snapshot older than the live refresh generation and enforces one live holder, minted-never-copied entries, and cache invalidation completed before any entry state is believed. Because no in-session rung exists, cross-provider failover is a checkpoint teardown followed by a respawn that is a NEW attempt with its own immutable pinned composition, never an in-session swap. Liveness classifies the limit menu and out-of-credits text as capped before any working marker, the context-bar percentage as saturated, and a pane-content hash delta as the marker-free fallback; served model comes from the session transcript under the pane's cwd because these panes carry no parseable model footer, with synthetic turns skipped and a transcript stale beyond one hour reported unknown. The wrapper-script spawn path carries its own refusal rather than relying on a check that inspects the launched program's own argument vector. It adds no second rotation policy, no operator or commander credential in an adapter, no copy path for credential material, and no new public ingress. | CT-I1-041; D72; D74; the sealed harness-adapter seam design at head `c07c049c` | Engineer + CSO + independent Review/QA | `apps/ctower-runner/`; `contracts/runner/`; `tests/conformance/harness-adapter/`; generated clients | AC-HAD-01's two-real-bindings publication proof on the unchanged existing cells plus the reviewed appended row cells; AC-HAD-02 through AC-HAD-09 on the `claude-code` column; the provided-pool half of AC-HAD-10 and AC-HAD-11; captured real Claude limit-menu and context-bar corpus including the cap-menu-matched-the-working-pattern regression; write-back-before-swap ordering with the reused-refresh-token regression; generation-guard and concurrent-holder tests; failover-as-new-attempt composition-digest diff plus the in-session-swap refusal; Interface inventory proving no copy path | `uv run pytest tests/conformance/harness-adapter tests/acceptance/increment-1/test_claude_code_binding.py -q` |

| CT-I1-043 | Deliver the `codex` binding and the runtime-under-a-harness distinction it exists to enforce: the seam pins the observed `harness_ref` AND the runtime/profile reference that carries credential lineage, because a model is not a harness and naming a launcher after a model is what produces a phantom harness category. Codex reached as a runtime under a `hermes` profile is not a separate harness and stays inside that engine's own pool — configure and observe, no special case. Codex as a direct CLI harness has a single active account per config home and no native pool or fallback, so ctower provides the layer by wrapping the fleet's existing enrolment, per-account re-mint, generation-guarded rotation, and multi-account cooldown ceremonies rather than writing a fifth rotation implementation. Pool exhaustion presents as a non-retryable 401 on the first real call while a separate liveness probe still reports the substrate alive, so `liveness` reports the pool fact rather than the substrate fact when the two disagree; a prepaid 402 is dead auth, because money is a liveness condition and revoked, capped, and unfunded are indistinguishable from the seat's side and identical in consequence; and a substrate that flips alive is not usable until it holds a full observation cycle, with briefs staged and nothing launched inside the verification window. Every mutating credential verb refuses an unknown flag and answers a question with usage rather than a side effect. It adds no new harness category for a routed model, no rotation implementation beside the wrapped ceremonies, and no credential value in any record, response, log, or event. | CT-I1-041; CT-I1-042; D72; the sealed harness-adapter seam design at head `c07c049c` | Engineer + CSO + independent Review/QA | `apps/ctower-runner/`; `contracts/runner/`; `tests/conformance/harness-adapter/`; generated clients | AC-HAD-01's survey-completeness and never-both refusals for both codex classes; AC-HAD-03's launch-argv serving-truth column; AC-HAD-10's pool-versus-substrate disagreement fixture and exhaustion-refusal body; AC-HAD-11's flap hold-one-cycle proof; an unknown-flag refusal vector for every mutating credential verb; a model-is-not-a-harness classification negative proving no second harness value is minted for a routed model | `uv run pytest tests/conformance/harness-adapter tests/acceptance/increment-1/test_codex_route.py tests/acceptance/increment-1/test_codex_pool.py tests/acceptance/increment-1/test_codex_sinks.py -q` *(EXPLICIT SPEC delta: the three authored row files are named because their combined logical test body is 602 lines, over the 600-line file budget; no aggregate or import shim.)* |

| CT-I1-044 | Deliver the later wave's honest classification and the credentials survey that blocks it, before any of its bindings is implemented. The ten survey questions — native pool, native fallback, config surface and whether it is authored-config-only, identity proof, reset/window semantics, rotation cache semantics, subagent credential inheritance, egress topology, probe target product/endpoint/model, and per-model per-direction credit weights — are answered per candidate as authored revision-pinned data, because the answers and never the harness's name decide whether ctower configures a layer or provides it; an unanswered survey leaves that choice undecidable, which is a refusal rather than a gap, and the candidate does not enter the conformance suite. The classification is itself part of the deliverable: `openclaw` is a gateway-routed harness with two approval planes whose readiness is proven by preflight assertions rather than inferred from a successful invite; `qwen-code` is already a legal baseline harness value with a weekly-plan quota and a known reset; `zcode` enters as a stub row and stays a stub until a real binding is written, because a table row is not an Adapter; and `deepseek` is a MODEL reached through a profile that already has both layers, so the correct amount of adapter work for it is zero. Each candidate's `HarnessSpec` declares its own liveness evidence sources or its `liveness` returns unknown by name, never a guess. It adds no adapter, no harness value for a model, and no rotation policy. | CT-I1-041; CT-I1-042; CT-I1-043; D72; the sealed harness-adapter seam design at head `c07c049c` | Engineer + Engineering Manager + CSO review | Authored `contracts/runner/` survey and capability-declaration data; `tests/conformance/harness-adapter/`; no runtime code for an unsurveyed candidate | AC-HAD-01's survey-completeness registration refusal per candidate; AC-HAD-03's declare-or-return-unknown proof for every declared liveness source; a per-candidate answered-survey census with the undecidable-role refusal; the zero-adapter disposition for a routed model and the stub-is-not-an-Adapter assertion | `uv run pytest tests/conformance/harness-adapter/test_survey_registration.py -q` |
...
| CT-I2-006 | The single owner of package-defined classification/overlays and Execution Policy evaluation, including stage-role membership over CT-I1-036 RoleFacts, the closed `participant_resolution` narrowing consumer, the delivery sprint's mandatory stage gates, required perspectives, configurable limits, non-waivable independence/conflict rules, the separate declared family-diversity placement rules and their per-tier waivability, the no-progress rule, protected waivers, and software/non-engineering fixtures. It consumes the authored Workflow base `role_keys` set and emits `workflow-stage-role-not-permitted` with the total precedence and zero-mutation contract; it does not resolve or mint RoleFacts. The `fleet-lifecycle@1` package (§"The fleet-lifecycle policy package") adds its close-gate lifecycle fixtures — a resolved ticket with a live bound crew session is DENIED close with `crew-session-still-live`; the same ticket closes once the reporter supplies the episode's three current facts; a reporter outage yields `substrate-unobservable:<probe>` and close stays denied. | CT-I2-002..003; CT-I1-036 | Engineering Manager + Engineer + CSO | Kernel `workflow/`, `access/`; Workflow and execution-policy packs | AC-ROLE-03..04 plus missing/invalid-bound/removal/client-count/independence/family-collapse denials, combined-refusal matrix, no-progress escalation, coherent current-digest traces, and the deny-then-close lifecycle fixtures | `uv run pytest tests/modules/workflow/test_execution_policy.py tests/acceptance/increment-2/test_workflow.py -q` |
...
| CT-I2-013 | Implement the registry-driven long-running runner service and its local target owner. Reconcile desired state to one systemd-delivered owner; bind each attempt to the effective HarnessSpec, Supervisor, target, workspace, telemetry, build digest, and credential-pool revision; observe independent liveness cycles; collect strict typed artifacts; transport typed transcript observations through a separate runner vocabulary; dispatch writeback only through the generated authenticated API and the seat's own project-seat credential; and checkpoint, park, reap, or recover through the typed teardown/fence protocol. Preserve record-tier isolation, the merged five-verb seam, CommandGuard at the final boundary, the sealed credential topology, and the explicit unknown/refusal laws. The sealed seam and cockpit document are proposal-stage inputs only; their transcript vocabulary, operator-principal write authority, managed-store boundary, ladder authority, and saturation policy remain activation prerequisites rather than guessed implementation choices. | CT-I1-041..044; accepted harness-adapter phase activation at `tools/checks/expected-suites.toml` (`active_phase = "CT-I1-043"`); CT-I2-004; D72; D76; sealed harness-adapter and operator-cockpit designs as proposal-stage sources | Engineer + DevOps + CSO + independent Review/QA | `apps/ctower-runner/`; `packages/ctower-runner-sdk/`; `contracts/runner/`; `deploy/private-vps/development/systemd/`; generated clients; conformance and Increment 2 acceptance suites | AC-RUN-16..21; AC-HAD-01..12; owner/restart/guard/liveness/collect/writeback/teardown/credential-topology traces; API-only record isolation; one `deferred` manifest-fixture disposition per new AC; no public page, route, or live-systemd mutation; R3114/R3117 review-round ledger proves at most three current-digest rounds and records any post-round-3 residue only as one-line tech debt unless it is security, data-loss/corruption, production-breaking, or gate-failing | `uv run pytest tests/conformance/runner tests/conformance/harness tests/acceptance/increment-2/test_runner_service.py -q && python -m tools.codegen --root . --check && just check && just verify` |
| CT-I2-014 | Implement the bounded chat/steer HTTP surface against the runner service. Keep the closed operation inventory to the cockpit proposal's justified crew/liveness/spec reads, typed transcript read/stream, composer input, and steer/ACK, plus the record-backed G2/G10/G11/G12 read inventory: HarnessSpec list/get, TicketResource workflow position, listTickets, and the four write-only queue reads; add only the three R3129 harness-setup writes `registerHarness`, `submitHarnessSurvey`, and `bindHarnessCredentialReference` over the shipped kernel ceremonies. Expose serving-model evidence, harness/spec/capability identity, liveness and watermarks; make command IDs, refusal reasons, cursor gaps, and pending-versus-folded durability explicit; and close authored OpenAPI, generated Python/TypeScript clients, contract operation counters, and reference documentation together for every operation. These reads and setup writes are HTTP faces over existing facts and kernel ceremonies and add no new authority and no new seam capability. No additional registration route, guard dry-run, provider credential mint/reference/keep-or-evict, workspace-change, pull-request, public product page, navigation entry, or browser UI is activated by this row. The operator-principal authority for steer and the typed transcript vocabulary remain named prerequisites. | CT-I1-041..044; accepted harness-adapter phase activation at `tools/checks/expected-suites.toml` (`active_phase = "CT-I1-043"`); CT-I2-004; CT-I2-013; D72; D76; `docs/internal/design/ctower-app.md` §§1–3 and `docs/internal/design/operator-cockpit.md` §8.2 as proposal-stage sources at `origin/feat/cockpit-design@c3e4de48` | Engineer + API/CLI + CSO + independent Review/QA | `contracts/http/openapi.yaml`; generated Python/TypeScript clients; `apps/ctower-api/`; `apps/ctowerctl/`; `docs/reference/http-api.md`; HTTP contract/counter/docs and Increment 2 acceptance suites | AC-CHAT-01..14; four-inventory parity per operation; G2 HarnessSpec list/get; G10 TicketResource `stage`/`workflow_ref`; G11 listTickets distinct from getBoard; G12 attention/intake/project_seats/poisoned-outbox reads; R3129 registration/survey/credential-reference writes over kernel ceremonies; project authorization/redaction; cursor/gap/replay/completeness; composer/steer ACK and refusal matrix; pending-to-folded projection trace; no credential values; no new authority and no new seam capability; no public-page or browser-surface claim; one `deferred` manifest-fixture disposition per new AC; R3114/R3117 review-round ledger proves at most three current-digest rounds and records any post-round-3 residue only as one-line tech debt unless it is security, data-loss/corruption, production-breaking, or gate-failing | `uv run pytest tests/contracts/http tests/acceptance/increment-2/test_chat_steer_http.py -q && python -m tools.codegen --root . --check && just check && just verify` |
