...
| Browser dogfood controls | Removed | The former server-mediated Inbox dogfood surface, required browser suite, and `ctower-ui` runtime (including `/setup`) were removed. The separate fresh `ctower-web` browser-app lane remains deferred to CT-I2-005 / I2.4. |
...
exception (D41/D44) and its runtime are removed; the separate fresh `ctower-web` browser-app lane has no
