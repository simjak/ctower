...
| HTTP/OpenAPI and generated clients | Development fixture | The authored OpenAPI has 86 operations with explicit CLI/query-mutation/spool metadata. It generates strict client models, methods, replay registry, and runtime schema resources. It is not a stable supported external API. |
...
| Browser dogfood controls | Removed | D75 retires the former server-mediated Inbox dogfood surface, required browser suite, and `ctower-ui` runtime (including `/setup`). The separate fresh `ctower-web` browser-app lane remains deferred to CT-I2-005 / I2.4. |
...
evidence, and browser E2E first activate at CT-I2-005 / I2.4. D75 retires the former `ctower-ui` Inbox
dogfood exception and its runtime; the separate fresh `ctower-web` browser-app lane has no
current product evidence. CT-I1-021 is a narrow server-only exception for the authenticated private Console
...
- Before operational or integration work, read
  [What is deliberately unavailable](../start-here/availability.md) and
