...
- **Native inbox delivery/read facts:** extend the existing two-party inbox with recipient-only monotonic
  ...
  atomically, while an explicit ticket preserves the existing link-only behavior. D75 removes the former
  separate server-mediated browser dogfood controls with this cut; no I1 product browser control is added.
