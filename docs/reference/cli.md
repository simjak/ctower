...
A listed entry quarantined by the server also carries `server_refusal` (`status` and an allowlisted `name`,
never response body text); an entry quarantined locally — `credential_identity_mismatch`, `expired`,
`corrupt_record` — carries its `reason_code` and no `server_refusal`.
