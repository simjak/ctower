...
| Persistent VPS shadow runtime | Development observation | A separately gated E2 candidate is running on this VPS with PostgreSQL 17.10 persistent primary/synchronous-development-ACK volumes, supervised API/worker units, ordinary finalization, and lifecycle replay. It remains `CP3_D_NOT_PROVEN`; the ACK is not an independently operated failure domain, and no supported install, backup, restore, or production claim follows. |
...
| Product installation, deployment, and recovery | Unsupported | A clean-wheel test proves the development artifact, and a secret-free private-VPS reference composition checks an alternate source-contract topology. The running E2 shadow instance is separate and does not turn either artifact into an operator path: there is no published/supported package, released container image, supported product Compose deployment, production backup/restore runbook, or production monitoring/incident path. |
