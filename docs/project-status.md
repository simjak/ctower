...
| Product installation, deployment, and recovery | Unsupported | A clean-wheel test proves the development artifact, and a secret-free private-VPS reference composition captures the intended one-host topology. Neither is an operator path: there is no published/supported package, released container image, supported product Compose deployment, production backup/restore runbook, or production monitoring/incident path. |
