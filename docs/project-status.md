...
| Persistent VPS shadow runtime | Dated development observation | Signed Mission Control artifact `coordination/2026-07-28_1030--engineer-r2164-e2-runtime--persistent-slice.status.md` recorded PostgreSQL 17.10 persistent primary/synchronous-development-ACK volumes, supervised API/worker units, ordinary finalization, and lifecycle replay on 2026-07-28. This is not current health; it remained `CP3_D_NOT_PROVEN`, the ACK was not an independently operated failure domain, and no supported install, backup, restore, or production claim follows. |
...
| Product installation, deployment, and recovery | Unsupported | A clean-wheel test proves the development artifact, and a secret-free private-VPS reference composition checks an alternate source-contract topology. The dated E2 observation is separate and does not turn either artifact into an operator path: there is no published/supported package, released container image, supported product Compose deployment, production backup/restore runbook, or production monitoring/incident path. |
...
- **Development observation** means an exact development candidate was exercised on a real host without
  creating an installation, compatibility, durability, recovery, or support promise.
