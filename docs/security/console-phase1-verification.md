...
|---|---|
| Exact identity join | Project, enabled non-Commander target and non-Commander Actor, seat, crew, assignment interval, work session, runtime attempt, runner/epoch, backend, live `@project`, and incarnation match at every decision point. Discovery, grant, renewal, first open, and each active poll rejoin the current target kind and disabled fact plus the exact current human session and role binding. Every Adapter call follows a side-effect-free durable authority preflight and precedes the final transactional recheck; a known refusal invokes the Adapter zero times. One fixed-search-path database primitive locks the exact authority anchors through grant, stream-open, encrypted-output, or custody-access persistence, so an overlapping authority mutation cannot commit inside the recheck-to-write handoff. The Adapter rereads the trusted current backend registry and live identity before and after each no-follow descriptor read; replacement bytes are never persisted. |
...
| Stream bounds | 16 KiB decoded chunks, 1 MiB/min delivery, 1 MiB/min replay, a transport-owned 256 KiB decoded pending queue, one-object-per-authority-check recovery, durable cursor before broadcast, per-allowance single-writer output and gap commits, source generations across truncation, and a typed gap before uncertainty. ASGI production runs ahead only to that queue bound, replaces unsent chunks with the durable gap, and enforces the authority-poll deadline on every network send. |
